...
public class EntitySeat extends Entity implements IControllable, IEntityAdditionalSpawnData {
  ...
  /**
   * A set of axes used to calculate where the player is looking, x axis is the direction of
   * looking, y is up
   */
  ...
  public void onUpdate() {
    ...
    boolean isThePlayer = riddenByEntity instanceof EntityPlayer && FlansMod.proxy
    ...
    if (worldObj.isRemote) {
      if (driver && isThePlayer && FlansMod.proxy.mouseControlEnabled() && driveable
          .hasMouseControlMode()) {
        looking = new RotatedAxes();
        playerLooking = new RotatedAxes();
      }
  ...
  public void updatePosition() {
    ...
    if (driveable != null && driveable.seats != null && driveable.seats[0] != null
        && driveable.seats[0].looking != null) {
      ...
      Vector3f
          .add(localPosition, new Vector3f(rotatedOffset.x, 0F, rotatedOffset.z), localPosition);
    }
    ...
    if (riddenByEntity != null) {
      ...
      Vec3 yOffset = driveable.axes
          .findLocalVectorGlobally(new Vector3f(0, riddenByEntity.getEyeHeight() * 3 / 4, 0))
          .toVec3().subtract(0, riddenByEntity.getEyeHeight(), 0);
      ...
      if (riddenByEntity instanceof EntityPlayer) {
        ...
        ((EntityPlayer) riddenByEntity).rotationYawHead = playerYaw;
      }
  ...
  public void onMouseMoved(int deltaX, int deltaY) {
    ...
    if (!driver || !FlansModClient.controlModeMouse || !driveable.hasMouseControlMode()) {
      ...
      float newPlayerPitch = playerLooking.getPitch() - deltaY / lookSpeed * Minecraft
          .getMinecraft().gameSettings.mouseSensitivity;
      ...
      float newPlayerYaw = playerLooking.getYaw() + deltaX / lookSpeed * Minecraft
          .getMinecraft().gameSettings.mouseSensitivity;
      ...
      if ((newPlayerYaw >= seatInfo.minYaw && newPlayerYaw <= seatInfo.maxYaw) || (
        ...
        float newPlayerYawDistFromRange = Math.min(Math.abs(newPlayerYaw - seatInfo.minYaw),
            Math.abs(newPlayerYaw - seatInfo.maxYaw));
        float otherPlayerNewYawDistFromRange = Math
            .min(Math.abs(otherNewPlayerYaw - seatInfo.minYaw),
      ...
      if ((newYaw >= seatInfo.minYaw && newYaw <= seatInfo.maxYaw) || (
        ...
        float newYawDistFromRange = Math
            .min(Math.abs(newYaw - seatInfo.minYaw), Math.abs(newYaw - seatInfo.maxYaw));
        float otherNewYawDistFromRange = Math
            .min(Math.abs(otherNewYaw - seatInfo.minYaw), Math.abs(otherNewYaw - seatInfo.maxYaw));
        //If the newYaw is closer to the range than the otherNewYaw, move newYaw into the range
      ...
      playYawSound = signDeltaX != 0 && seatInfo.traverseSounds == true;

      if (signDeltaY != 0 && seatInfo.yawBeforePitch == false && currentYawToMove < minYawToMove) {
        playPitchSound = true;
      } else
        playPitchSound = signDeltaY != 0 && seatInfo.yawBeforePitch == true && signDeltaX == 0;
    }
  }
...
//						item.onUpdateClient(player.getHeldItem(), 0, worldObj, player, false, false);
    ...
    if (key == 9) //Shoot
    {
      ...
      if (gun != null && gun.mode != EnumFireMode.MINIGUN || minigunSpeed > 2F) {
        if (gunDelay <= 0 && TeamsManager.bulletsEnabled) {
          ...
          if (gun != null && bulletItemStack != null && bulletItemStack
              .getItem() instanceof ItemShootable) {
            ShootableType bullet = ((ItemShootable) bulletItemStack.getItem()).type;
            if (gun.isAmmo(bullet)) {
              //Gun origin
              Vector3f gunOrigin = Vector3f
                  .add(driveable.axes.findLocalVectorGlobally(seatInfo.gunOrigin),
              ...
              Vector3f yOffset = driveable.axes.findLocalVectorGlobally(
                  new Vector3f(0F, (float) player.getMountedYOffset(), 0F));
              //Spawn a new bullet item
              worldObj.spawnEntityInWorld(
                  ((ItemShootable) bulletItemStack.getItem()).getEntity(worldObj,
                      Vector3f
                          .add(yOffset, new Vector3f(gunOrigin.x, gunOrigin.y, gunOrigin.z), null),
                      shootVec,
                      (EntityLivingBase) riddenByEntity,
                      bullet.bulletSpread / 2,
                      gun.damage,
                      gun.getBulletSpeed(bullet, null), // TODO : Fix nasty hack
  ...
  @Override
  public boolean interactFirst(
      EntityPlayer entityplayer) //interact : change back when Forge updates
  {
    if (isDead) {
      return false;
    }
    ...
    if (currentItem != null && currentItem.getItem() instanceof ItemLead) {
      ...
      List nearbyMobs = worldObj.getEntitiesWithinAABB(EntityLiving.class,
          new AxisAlignedBB(posX - checkRange, posY - checkRange, posZ - checkRange,
              posX + checkRange, posY + checkRange, posZ + checkRange));
