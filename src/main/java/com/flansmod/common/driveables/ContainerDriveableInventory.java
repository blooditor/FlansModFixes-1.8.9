...
public class ContainerDriveableInventory extends Container {
  ...
  public ContainerDriveableInventory(InventoryPlayer inventoryplayer, World worldy,
    ...
    switch (i) {
      ...
      case 2: {
        numItems =
            plane.getDriveableType().numFlareSlots > 0 ? plane.getDriveableType().numFlareSlots
                : plane.getDriveableType().numCargoSlots;
        maxScroll = (((numItems + 7) / 8) > 3 ? ((numItems + 7) / 8) - 3 : 0);
        break;
      }
      case 3: {
    ...
    switch (screen) {
      case 0: //Guns
      {
        ...
        for (int j = 0; j < plane.driveableData.numGuns; j++) {
          ...
          addSlotToContainer(
              new SlotNoVehicle(plane.driveableData, j, 29, yPos, ItemShootable.class));
          slotsDone++;
        }
        break;
      }
      ...
      case 3: //Missiles
      {
        ...
        for (int row = 0; row < m; row++) {
          ...
          for (int col = 0; col < ((row + scroll + 1) * 8 <= numItems ? 8 : numItems % 8); col++) {
            addSlotToContainer(
                new SlotNoVehicle(plane.driveableData, startSlot + row * 8 + col, 10 + 18 * col,
                    yPos, screen == 2 ? null : ItemShootable.class));
          }
        }
        break;
      }
    ...
    switch (screen) {
      case 0: {
        ...
        for (int i = 0; i < plane.driveableData.numGuns; i++) {
          ...
          inventorySlots.get(slotsDone).yDisplayPosition = yPos;
          slotsDone++;
        }
        break;
      }
      ...
      case 3: {
        ...
        for (int row = 0; row < m; row++) {
          ...
          for (int col = 0; col < ((row + 1) * 8 <= numItems ? 8 : numItems % 8); col++) {
            inventorySlots.get(row * 8 + col).yDisplayPosition = yPos;
          }
        }
        break;
      }
  ...
  public ItemStack transferStackInSlot(EntityPlayer player, int slotID) {
    ...
    if (currentSlot != null && currentSlot.getHasStack()) {
      ...
      if (stack.getItem() instanceof ItemPlane || stack.getItem() instanceof ItemVehicle) {
        return null;
      }
  ...
  public static class SlotNoVehicle extends Slot {
    ...
    public SlotNoVehicle(IInventory inventoryIn, int index, int xPosition, int yPosition,
        Class whitelistClass) {
    ...
    public boolean isItemValid(ItemStack stack) {
      ...
      if (whitelist != null) {
        return whitelist.item == it;
      }
      if (whitelistClass != null) {
        return whitelistClass.isInstance(it);
      }
      return !(stack.getItem() != null && (stack.getItem() instanceof ItemPlane || stack
          .getItem() instanceof ItemVehicle)) && super.isItemValid(stack);
    }
  }
}
