...
public abstract class EntityDriveable extends Entity implements IControllable, IExplodeable,
  ...
  /**
   * Ticks since last server update. Use to smoothly transition to new
   * posiGuiDriveableControllerion
   */
  ...
  /**
   * The throttle, in the range -1, 1 is multiplied by the maxThrottle (or maxNegativeThrottle) from
   * the plane type to obtain the thrust
   */
  ...
  protected void initType(DriveableType type, boolean clientSide) {
    ...
    CollisionBox core = this.getDriveableType().health.get(EnumDriveablePart.core);
  }
  ...
  public void setDead() {
    ...
    if (worldObj.isRemote && camera != null) {
      camera.setDead();
    }
  ...
  public void setPositionAndRotation2(double d, double d1, double d2, float f, float f1, int i,
    ...
    if (riddenByEntity instanceof EntityPlayer && FlansMod.proxy
        .isThePlayer((EntityPlayer) riddenByEntity)) {
    } else {
      if (syncFromServer) {
  ...
  public void shoot(boolean secondary) {
    ...
    if (getShootDelay(secondary) <= 0.0f) //hotfix for gamebreaking crash everyone complains about
    {
      ...
      int addGun = secondary ? type.shootPointsPrimary.size()
          : 0; //fix for vehicles with multiple primary guns
      ...
      if (type.alternate(secondary)) {
        ...
        shootEach(type, shootPoints.get(currentGun), currentGun + addGun, secondary, weaponType);
      } else {
  ...
  private boolean driverIsCreative() {
    return seats != null && seats[0] != null && seats[0].riddenByEntity instanceof EntityPlayer
  ...
  private void shootEach(DriveableType type, DriveablePosition shootPoint, int currentGun,
      boolean secondary, EnumWeaponType weaponType) {
    ...
    if (shootPoint instanceof PilotGun) {
      ...
      DriveableType d = getDriveableType();
      ...
      ItemStack bulletItemStack = driveableData.ammo[getDriveableType().numPassengerGunners
          + currentGun];
      ...
      if (gunType != null && bulletItemStack != null && bulletItemStack
        ...
        if (gunType.isAmmo(bullet)) {
          ...
          worldObj.spawnEntityInWorld(((ItemShootable) bulletItemStack.getItem())
              .getEntity(worldObj, Vector3f
                      .add(gunVec, new Vector3f((float) posX, (float) posY, (float) posZ), null),
                  lookVector, (EntityLivingBase) seats[0].riddenByEntity, gunType.bulletSpread / 2,
          ...
          if (damage + 1 == bulletItemStack.getMaxDamage()) {
            ...
            if (seats[0].riddenByEntity instanceof EntityPlayer
              ...
              driveableData
                  .setInventorySlotContents(getDriveableType().numPassengerGunners + currentGun,
                      bulletItemStack);
            }
          }
  ...
  public void onUpdate() {
    super.onUpdate();
    //if(worldObj.getBlockState(getPosition().up()).getBlock().getMaterial() == Material.water)
    if (inWater) {
      timeUnderWater++;
    } else {
      timeUnderWater = 0;
        ...
        for (float y = box.y; y <= box.y + box.h; y++) {
          for (float z = box.z; z <= box.z + box.d; z++) {
            ...
            if (seats[0] != null && seats[0].riddenByEntity instanceof EntityPlayerMP) {
              int eventOutcome = ForgeHooks.onBlockBreakEvent(worldObj,
                  ((EntityPlayerMP) seats[0].riddenByEntity).capabilities.isCreativeMode
                      ? GameType.CREATIVE
                      : ((EntityPlayerMP) seats[0].riddenByEntity).capabilities.allowEdit
                          ? GameType.SURVIVAL : GameType.ADVENTURE,
                  (EntityPlayerMP) seats[0].riddenByEntity, new BlockPos(blockX, blockY, blockZ));
              cancelled = eventOutcome == -1;
            }
            if (!cancelled) {
              if (type.materialsHarvested.contains(block.getMaterial())
                  && block.getBlockHardness(worldObj, new BlockPos(blockX, blockY, blockZ)) >= 0F) {
                //Add the itemstack to mecha inventory
                List<ItemStack> stacks = block
                    .getDrops(worldObj, new BlockPos(blockX, blockY, blockZ),
                        worldObj.getBlockState(new BlockPos(blockX, blockY, blockZ)), 0);
                for (int i = 0; i < stacks.size(); i++) {
                  ItemStack stack = stacks.get(i);
                  if (!InventoryHelper
                      .addItemStackToInventory(driveableData, stack, driverIsCreative())
                      && !worldObj.isRemote && worldObj.getGameRules().getBoolean("doTileDrops")) {
                    worldObj.spawnEntityInWorld(
                        new EntityItem(worldObj, blockX + 0.5F, blockY + 0.5F, blockZ + 0.5F,
                            stack));
                  }
                }
                //Destroy block
    ...
    for (DriveablePart part : getDriveableData().parts.values()) {
      if (part.box != null) {
        ...
        if (worldObj.isRemote) {
          if (part.onFire && part.health > 0) {
            int count = Math.min(part.maxHealth / part.health, 80);
            for (int i = 0; i < count * 10; i++) {

              //Pick a random position within the bounding box and spawn a flame there
              Vector3f pos = axes.findLocalVectorGlobally(new Vector3f(
                  part.box.x + rand.nextFloat() * part.box.w - part.box.x / 10
                      + rand.nextFloat() * part.box.w / 5,
                  part.box.y + rand.nextFloat() * part.box.h + rand.nextFloat() * part.box.h / 10,
                  part.box.z + rand.nextFloat() * part.box.d));
              worldObj
                  .spawnParticle(EnumParticleTypes.FLAME, posX + pos.x, posY + pos.y, posZ + pos.z,
                      0, 0, 0);
            }
          }
          if (part.health > 0 && part.health < part.maxHealth / 2) {
            ...
            Vector3f pos = axes.findLocalVectorGlobally(
                new Vector3f(part.box.x + rand.nextFloat() * part.box.w,
                    ...
                    part.box.z + rand.nextFloat() * part.box.d));
            worldObj.spawnParticle(part.health < part.maxHealth / 4 ? EnumParticleTypes.SMOKE_LARGE
        ...
        if (part.onFire) {
          ...
          if (worldObj.getBlockState(new BlockPos(MathHelper.floor_double(posX + pos.x),
              MathHelper.floor_double(posY + pos.y), MathHelper.floor_double(posZ + pos.z)))
              .getBlock().getMaterial() == Material.water) {
            part.onFire = false;
          }
        } else {
          Vector3f pos = axes.findLocalVectorGlobally(
              new Vector3f(part.box.x / 16F + part.box.w / 32F, part.box.y / 16F + part.box.h / 32F,
                  part.box.z / 16F + part.box.d / 32F));
          if (worldObj.getBlockState(new BlockPos(MathHelper.floor_double(posX + pos.x),
              MathHelper.floor_double(posY + pos.y), MathHelper.floor_double(posZ + pos.z)))
              .getBlock().getMaterial() == Material.lava) {
            part.onFire = true;
          }
        }
      }
    ...
    for (int i = 0; i < type.emitters.size(); i++) {
      ...
      canEmit = isPartIntact(EnumDriveablePart.getPart(emitter.part))
          && healthPercentage >= emitter.minHealth && healthPercentage <= emitter.maxHealth;
          ...
          if (seats != null && seats[0] != null) {
            if (EnumDriveablePart.getPart(emitter.part) != EnumDriveablePart.turret
              ...
              Vector3f localPosition = new Vector3f(
                  emitter.origin.x + rand.nextFloat() * emitter.extents.x
                      - emitter.extents.x * 0.5f,
                  emitter.origin.y + rand.nextFloat() * emitter.extents.y
                      - emitter.extents.y * 0.5f,
                  emitter.origin.z + rand.nextFloat() * emitter.extents.z
              ...
              RotatedAxes yawOnlyLooking = new RotatedAxes(
                  seats[0].looking.getYaw() + axes.getYaw(), axes.getPitch(), axes.getRoll());
    ...
    boolean canThrust = (seats[0] != null && seats[0].riddenByEntity instanceof EntityPlayer
        && ((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode)
        || driveableData.fuelInTank > 0 || getDriveableType().fuelTankSize == 0;

    //If there's no player in the driveable or it cannot thrust, slow the plane and turn off mouse held actions
    //fixed being able to drive backwards without fuel | fixed flak not shooting
    ...
    for (int i = 0; i < getDriveableData().getSizeInventory(); i++) {
			...
			*/
      {
        ...
        else if (FlansMod.hooks.BuildCraftLoaded && stack
  ...
  public void checkForCollisions() {
    ...
    for (DriveablePosition p : getDriveableType().collisionPoints) {
      ...
      Vec3 lastPos = new Vec3(prevPosX + lastRelPos.x, prevPosY + lastRelPos.y,
          prevPosZ + lastRelPos.z);

      Vector3f currentRelPos = axes.findLocalVectorGlobally(p.position);
      Vec3 currentPos = new Vec3(posX + currentRelPos.x, posY + currentRelPos.y,
          posZ + currentRelPos.z);

      if (FlansMod.DEBUG && worldObj.isRemote) {
        worldObj.spawnEntityInWorld(new EntityDebugVector(worldObj, new Vector3f(lastPos),
      ...
      if (hit != null && hit.typeOfHit == MovingObjectType.BLOCK) {
        ...
        //Attack the part
        if (!attackPart(p.part, DamageSource.inWall, blockHardness * blockHardness * (float) speed)
          ...
          //The part died!
          worldObj
              .createExplosion(this, currentPos.xCoord, currentPos.yCoord, currentPos.zCoord, 1F,
                  false);
        }
      }
  ...
  /**
   * Takes a vector (such as the origin of a seat / gun) and translates it from local coordinates to
   * global coordinates
   */
  ...
  public Entity getControllingEntity() {
    return seats[0] == null ? null : seats[0].getControllingEntity();
  }
  ...
  public boolean hasEnoughFuel() {
    ...
    if (!getDriveableType().floatOnWater && timeUnderWater > 60) {
      return false;
    }
    return driverIsCreative()
        || driveableData.fuelInTank > driveableData.engine.fuelConsumption * throttle;
  ...
  /**
   * Whether or not the plane is on the ground TODO : Replace with proper check based on wheels
   */
   ...
   */
  public float bulletHit(BulletType bulletType, float damage, DriveableHit hit,
      float penetratingPower) {
    DriveablePart part = getDriveableData().parts.get(hit.part);

    float applied = part.hitByBullet(bulletType, damage, !worldObj.isRemote);

    //pass damage to core
    if (applied > 0 && part.type != EnumDriveablePart.core) {
      DriveablePart core = driveableData.parts.get(EnumDriveablePart.core);
      //if the part dies, it deals 80% of the remaining dmg, if it survives it deals 60% of the initial damage
      //but it doesnt insta kill the core except the part is small
      float coreDmg = Math.min(Math.max((damage - applied) * 0.8f, damage * 0.6f),
          core.maxHealth * rand.nextFloat() * ((part.maxHealth <= 1000 ? 0.9f : 0.5f) + 0.25f));
      core.hitByBullet(bulletType, coreDmg, !worldObj.isRemote);
    }
  ...
  /**
   * Internal method for checking that all parts are ok, destroying broken ones, dropping items and
   * making sure that child parts are destroyed when their parents are
   */
  ...
  private void killPart(DriveablePart part) {
    ...
    if (!worldObj.isRemote) {
      ...
      if (part.box != null) {
        pos = axes.findLocalVectorGlobally(
            new Vector3f(part.box.x / 16F + part.box.w / 32F, part.box.y / 16F + part.box.h / 32F,
                part.box.z / 16F + part.box.d / 32F));
      }
      ...
      if (part.type == EnumDriveablePart.core) {
        for (int i = 0; i < getDriveableData().getSizeInventory(); i++) {
          ...
          if (stack != null) {
            worldObj.spawnEntityInWorld(
                new EntityItem(worldObj, posX + rand.nextGaussian(), posY + rand.nextGaussian(),
