...
public class ChunkProviderApocalypse implements IChunkProvider {
  ...
  public static List<BiomeGenBase> runwaySpawnBiome = Arrays
      .asList(BiomeGenApocalypse.highPlateau);
  private WorldGenResearchLab researchLabGenerator = new WorldGenResearchLab();
  public static List<BiomeGenBase> labSpawnBiome = Arrays
      .asList(BiomeGenApocalypse.highPlateau);
...
*/
    ...
    if (TerrainGen.populate(p_73153_1_, worldObj, rand, p_73153_2_, p_73153_3_, flag, ANIMALS)) {
      SpawnerAnimals
          .performWorldGenSpawning(this.worldObj, biomegenbase, k + 8, l + 8, 16, 16, this.rand);
      //this.generateEntities(chunkcoordintpair);
    }
    blockpos = blockpos.add(8, 0, 8);

    MinecraftForge.EVENT_BUS.post(
        new PopulateChunkEvent.Post(p_73153_1_, worldObj, rand, p_73153_2_, p_73153_3_, flag));
    ...
    if (rand.nextInt(FlansModApocalypse.AIRPORT_RARITY) == 0) {
      ...
      for (int i = 0; i < 4; i++) {
        if (!worldObj.getWorldChunkManager()
            .areBiomesViable((xOrigin * 4 + i) * 16 + 8, p_73153_3_ * 16 + 8, 0,
                runwaySpawnBiome)) {
          canSpawn = false;
        }
      }
      ...
      if (canSpawn) {
        runwayGenerator
            .generate(worldObj, rand, new BlockPos(xOrigin * 4 * 16, 0, p_73153_3_ * 16));
      }
    }
    ...
    if (rand.nextInt(FlansModApocalypse.LAB_RARITY) == 0) {
      ...
      for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 5; j++) {
          if (!worldObj.getWorldChunkManager()
              .areBiomesViable((xOrigin * 3 - 1 + i) * 16 + 8, (zOrigin * 3 - 1 + j) * 16 + 8, 0,
                  labSpawnBiome)) {
            canSpawn = false;
          }
        }
      ...
      if (canSpawn) {
        ...
        j1 = this.rand.nextLong() / 2L * 2L + 1L;
        this.rand
            .setSeed((long) p_73153_2_ * i1 + (long) p_73153_3_ * j1 ^ this.worldObj.getSeed());
        researchLabGenerator
            .generate(worldObj, rand, new BlockPos(p_73153_2_ * 16, 0, p_73153_3_ * 16));
      }
    }
    ...
    if (rand.nextInt(FlansModApocalypse.VEHICLE_RARITY) == 0) {
      ...
      for (EnumDriveablePart part : EnumDriveablePart.values()) {
        tags.setInteger(part.getShortName() + "_Health",
            type.health.get(part) == null ? 0 : rand.nextInt(type.health.get(part).health));
        tags.setBoolean(part.getShortName() + "_Fire", false);
      }
      for (int i = 0; i < 10; i++) {
        EnumDriveablePart randomPart = EnumDriveablePart.values()[rand
            .nextInt(EnumDriveablePart.values().length)];
        if (randomPart != EnumDriveablePart.core) {
          killPart(randomPart, tags);
        }
      }

      EntityDriveable entity = type
          .createDriveable(worldObj, x, surface.getY() + 1, z, new DriveableData(tags));

      if (rand.nextBoolean()) {
        entity.driveableData.fuel = new ItemStack(
            FlansModApocalypse.getLootGenerator().getRandomFuel(rand).item, 1);
      }
...
*/
  ...
  public void setBlocksInChunk(int p_180518_1_, int p_180518_2_, ChunkPrimer p_180518_3_) {
    this.biomesForGeneration = this.worldObj.getWorldChunkManager()
        .getBiomesForGeneration(this.biomesForGeneration, p_180518_1_ * 4 - 2, p_180518_2_ * 4 - 2,
            10, 10);
    ...
    for (int subChunkX = 0; subChunkX < 4; ++subChunkX) {
      ...
      for (int subChunkZ = 0; subChunkZ < 4; ++subChunkZ) {
        ...
        for (int subChunkY = 0; subChunkY < 32; ++subChunkY) {
          ...
          int nextSquishedSubChunkY =
              subChunkY + 1;//subChunkY == 31 ? subChunkY : heightScaling[(subChunkY + 1) * 8] / 8;
          ...
          for (int blockY = 0; blockY < 8; ++blockY) {
            ...
            for (int blockX = 0; blockX < 4; ++blockX) {
              ...
              for (int blockZ = 0; blockZ < 4; ++blockZ) {
                if ((d15 += d16) > 0.0D) {
                  p_180518_3_.setBlockState(subChunkX * 4 + blockX, subChunkY * 8 + blockY,
                      subChunkZ * 4 + blockZ, Blocks.stone.getDefaultState());
                } else if (subChunkY * 8 + blockY < this.seaLevel) {
                  p_180518_3_.setBlockState(subChunkX * 4 + blockX, subChunkY * 8 + blockY,
                      subChunkZ * 4 + blockZ, Blocks.water.getDefaultState());
                }
              }
  ...
  public void func_180517_a(int p_180517_1_, int p_180517_2_, ChunkPrimer p_180517_3_,
      BiomeGenBase[] p_180517_4_) {
    ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(this,
        p_180517_1_, p_180517_2_, p_180517_3_, this.worldObj);
    ...
    this.stoneNoise = this.noiseGen4
        .func_151599_a(this.stoneNoise, p_180517_1_ * 16, p_180517_2_ * 16,
  ...
  private void generateNoisemap(int xSize, int ySize, int zSize) {
    this.field_147426_g = this.noiseGen6
        .generateNoiseOctaves(this.field_147426_g, xSize, zSize, 5, 5,
            this.depthNoiseScaleX, this.depthNoiseScaleZ,
            this.depthNoiseScaleExponent);
    float f = this.coordinateScale;
    float f1 = this.heightScale;
    this.field_147427_d = this.noiseGen3
        .generateNoiseOctaves(this.field_147427_d, xSize, ySize, zSize, 5, 33, 5,
            f / this.mainNoiseScaleX, f1 / this.mainNoiseScaleY,
            f / this.mainNoiseScaleZ);
    this.field_147428_e = this.noiseGen2
        .generateNoiseOctaves(this.field_147428_e, xSize, ySize, zSize, 5, 33, 5, f,
            f1, f);
    this.field_147425_f = this.noiseGen1
        .generateNoiseOctaves(this.field_147425_f, xSize, ySize, zSize, 5, 33, 5, f,
            f1, f);
    boolean flag1 = false;
    boolean flag = false;
    int l = 0;
      ...
      for (int noiseZ = 0; noiseZ < 5; ++noiseZ) {
        ...
        for (int l1 = -b0; l1 <= b0; ++l1) {
          for (int i2 = -b0; i2 <= b0; ++i2) {
            BiomeGenBase biomegenbase1 = this.biomesForGeneration[noiseX + l1 + 2
                + (noiseZ + i2 + 2) * 10];
        ...
        for (int noiseY = 0; noiseY < 33; ++noiseY) {
          double d1 = ((double) noiseY - d0) * this.stretchY * 128.0D / 256.0D / d9;
          ...
          if (noiseY > 29) {
            double d6 = (float) (noiseY - 29) / 3.0F;
