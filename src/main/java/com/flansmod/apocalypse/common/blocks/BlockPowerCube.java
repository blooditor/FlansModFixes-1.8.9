...
public class BlockPowerCube extends Block implements ITileEntityProvider {
  ...
  public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer,
    ...
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 2; j++) {
        if (world.provider.getDimensionId() == FlansModApocalypse.dimensionID && isPortal(world,
            pos.add(-3 * i, 0, -3 * j))) {
  ...
  private boolean isPortal(World world, BlockPos pos) {
		if (world.getBlockState(pos).getBlock() != FlansModApocalypse.blockPowerCube
				|| world.getBlockState(pos.add(3, 0, 0)).getBlock() != FlansModApocalypse.blockPowerCube
				|| world.getBlockState(pos.add(0, 0, 3)).getBlock() != FlansModApocalypse.blockPowerCube
				|| world.getBlockState(pos.add(3, 0, 3)).getBlock() != FlansModApocalypse.blockPowerCube) {
			return false;
		}
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				if (world.getBlockState(pos.add(i * 3, -1, j * 3)).getBlock() != Blocks.obsidian
						|| world.getBlockState(pos.add(1 + i, -1, 1 + j)).getBlock() != Blocks.obsidian) {
					return false;
				}
			}
		}
