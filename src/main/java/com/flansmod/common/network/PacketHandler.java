 ...
 *
 * @author Flan With much inspiration from http://www.minecraftforge.net/wiki/Netty_Packet_Handling
 */
...
public class PacketHandler extends MessageToMessageCodec<FMLProxyPacket, PacketBase> {
  ...
  public boolean registerPacket(Class<? extends PacketBase> cl) {
    if (packets.size() > 256) {
      FlansMod.log(
          "Packet limit exceeded in Flan's Mod packet handler by packet " + cl.getCanonicalName()
              + ".");
      return false;
    }
    ...
    if (modInitialised) {
      FlansMod
          .log("Tried to register packet " + cl.getCanonicalName() + " after mod initialisation.");
      return false;
    }
  ...
  protected void encode(ChannelHandlerContext ctx, PacketBase msg, List<Object> out)
    ...
    try {
      ...
      FMLProxyPacket proxyPacket = new FMLProxyPacket(new PacketBuffer(encodedData.copy()),
    ...
    try {
      ...
      if (cl == null) {
        throw new NullPointerException(
            "Packet not registered for discriminator : " + discriminator);
      }
  ...
  public void handleClientPackets() {
    for (PacketBase packet = receivedPacketsClient.poll(); packet != null;
        packet = receivedPacketsClient.poll()) {
      packet.handleClientSide(getClientPlayer());
    }
  }
  ...
  public void handleServerPackets() {
    for (String playerName : receivedPacketsServer.keySet()) {
      ConcurrentLinkedQueue<PacketBase> receivedPacketsFromPlayer = receivedPacketsServer
          .get(playerName);
      EntityPlayerMP player = MinecraftServer.getServer().getConfigurationManager()
          .getPlayerByUsername(playerName);
      for (PacketBase packet = receivedPacketsFromPlayer.poll(); packet != null;
  ...
  public void initialise() {
    ...
    registerPacket(PacketVehicleFireMode.class);
  }

  /**
   * Post-Initialisation method called from FMLPostInitializationEvent in FlansMod Logically sorts
   * the packets client and server side to ensure a matching ordering
   */
        ...
        new Comparator<Class<? extends PacketBase>>() {
          ...
          public int compare(Class<? extends PacketBase> c1, Class<? extends PacketBase> c2) {
            int com = String.CASE_INSENSITIVE_ORDER
                .compare(c1.getCanonicalName(), c2.getCanonicalName());
            if (com == 0) {
  ...
  public void sendToAllAround(Packet packet, NetworkRegistry.TargetPoint point) {
    MinecraftServer.getServer().getConfigurationManager()
        .sendToAllNear(point.x, point.y, point.z, point.range, point.dimension, packet);
  }
  ...
  public void sendToDimension(Packet packet, int dimensionID) {
    MinecraftServer.getServer().getConfigurationManager()
        .sendPacketToAllPlayersInDimension(packet, dimensionID);
  }
