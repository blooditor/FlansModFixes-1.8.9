...
public class EntityFakePlayer extends EntityFlansModShooter {
  ...
  public EntityFakePlayer(World world, EntityPlayer player) {
    ...
    for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
      inventory
          .setInventorySlotContents(i, ItemStack.copyItemStack(player.inventory.getStackInSlot(i)));
    }
  }
      ...
      for (int j = 0; j < inventory.getSizeInventory(); j++) {
				if (inventory.getStackInSlot(j) != null) {
					worldObj.spawnEntityInWorld(
							new EntityItem(worldObj, posX, posY, posZ, inventory.getStackInSlot(j)));
				}
      }
    }
  }
  ...
  public void readEntityFromNBT(NBTTagCompound tags) {
    ...
    for (int i = 0; i < 40; i++) {
      inventory.setInventorySlotContents(i,
          ItemStack.loadItemStackFromNBT(tags.getCompoundTag("S" + i)));
    }
  }
