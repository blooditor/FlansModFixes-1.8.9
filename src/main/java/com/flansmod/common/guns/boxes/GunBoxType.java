...
public class GunBoxType extends BoxType {
  ...
  public void read(String[] split, TypeFile file) {
    ...
    try {
      ...
      else if (split[0].equals("AddAmmo") || split[0].equals("AddAltType") || split[0]
          .equals("AddAltAmmo") || split[0].equals("AddAlternateAmmo")) {
  ...
  private List<ItemStack> getRecipe(String[] split) {
    ...
    for (int i = 0; i < (split.length - 2) / 2; i++) {
      if (split[i * 2 + 3].contains(".")) {
        recipe.add(
            getRecipeElement(split[i * 2 + 3].split("\\.")[0], Integer.parseInt(split[i * 2 + 2]),
                Integer.valueOf(split[i * 2 + 3].split("\\.")[1]), shortName));
      } else {
        recipe.add(
            getRecipeElement(split[i * 2 + 3], Integer.parseInt(split[i * 2 + 2]), 0, shortName));
      }
    }
  ...
  public static class GunBoxPage {
    ...
    /**
     * Points to the gun box entry we are currently reading from file. Allows for the old format to
     * write in ammo on a separate line to the gun.
     */
  ...
  /**
   * Represents a top level entry, normally a gun. This has child entries, normally ammo that are
   * listed below in the GUI.
   */
