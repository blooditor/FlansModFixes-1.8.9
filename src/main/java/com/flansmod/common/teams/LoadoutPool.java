...
public class LoadoutPool extends InfoType {
  ...
  public void read(String[] split, TypeFile file) {
    ...
    if (KeyMatches(split, "MaxLevel")) {
      ...
      for (int i = 0; i < EnumLoadoutSlot.values().length; i++) {
        if (2 + i < split.length) {
          ...
          if (type == null) {
            ...
            defaultLoadouts[index].slots[i] = new ItemStack(type.getItem());
          }
        }
      }
      ...
      FlansMod.Assert(slotAvailable,
          "Trying to insert more than 3 reward box types. No support for this yet");
