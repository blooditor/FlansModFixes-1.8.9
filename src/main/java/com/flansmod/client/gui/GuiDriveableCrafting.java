...
public class GuiDriveableCrafting extends GuiScreen {
  ...
  public void drawScreen(int i, int j, float f) {
    ...
    for (int m = 0; m < 2; m++) {
      for (int n = 0; n < 8; n++) {
        ...
        if (blueprintNumber < DriveableType.types.size()) {
          ...
          drawSlotInventory(new ItemStack(type.item), guiOriginX + 8 + n * 18,
              guiOriginY + 18 + m * 18);
        }
      }
    }
    ...
    if (selectedType != null) {
      ...
      GL11.glScalef(-50F * selectedType.modelScale / selectedType.cameraDistance,
      ...
      drawString(fontRendererObj, "Cargo Slots : " + selectedType.numCargoSlots, guiOriginX + 82,
          guiOriginY + 74, 0xffffff);
      drawString(fontRendererObj, "Bomb Slots : " + selectedType.numBombSlots, guiOriginX + 82,
          guiOriginY + 84, 0xffffff);
      drawString(fontRendererObj, "Passengers : " + selectedType.numPassengers, guiOriginX + 82,
          guiOriginY + 94, 0xffffff);
      drawString(fontRendererObj, "Guns : " + (selectedType.ammoSlots()), guiOriginX + 82,
          guiOriginY + 104, 0xffffff);
      drawString(fontRendererObj, selectedType.numEngines() + "x", guiOriginX + 100,
          guiOriginY + 141, 0xffffff);
        ...
        for (int c = 0; c < 4; c++) {
          ...
          if (recipeItemNumber < selectedType.driveableRecipe.size()) {
            ...
            for (int n = 0; n < temporaryInventory.getSizeInventory(); n++) {
              ...
              if (stackInSlot != null && stackInSlot.getItem() == recipeStack.getItem()
                  && stackInSlot.getItemDamage() == recipeStack.getItemDamage()) {
                //Work out the amount to take from the stack
                int amountFound = Math
                    .min(stackInSlot.stackSize, recipeStack.stackSize - totalAmountFound);
                //Take it
            ...
            if (totalAmountFound < recipeStack.stackSize) {
              mc.renderEngine.bindTexture(texture);
              drawTexturedModalRect(guiOriginX + 8 + c * 18, guiOriginY + 138 + r * 18, 195, 11, 16,
                  16);
              canCraft = false;
            }
      ...
      for (int n = 0; n < temporaryInventory.getSizeInventory(); n++) {
        ...
        if (stackInSlot != null && stackInSlot.getItem() instanceof ItemPart) {
          ...
          if (partType.category == EnumPartCategory.ENGINE && partType.worksWith
              .contains(EnumType.getFromObject(selectedType))) {
      ...
      for (PartType part : engines.keySet()) {
        ...
        if (part.engineSpeed > bestEngineSpeed && engines.get(part).stackSize >= selectedType
            .numEngines()) {
