...
public class GuiMissionResults extends GuiTeamsBase {
  ...
  public void initGui() {
    ...
    buttonList.add(
        new GuiButton(0, width / 2 - WIDTH / 2 + 214, height / 2 - HEIGHT / 2 + 6, 36, 20, "Done"));
  }
  ...
  public void updateScreen() {
    ...
    switch (state) {
      ...
      case INCREASE_XP_BAR: {
        ...
        displayXP = MathHelper.floor_float(
            lastXP + ((float) (currentTarget - lastXP) * (float) (timeInState - 1)
                / (float) stateTimes[state.ordinal()]));
  ...
  public void drawScreen(int i, int j, float f) {
    ...
    if (teamInfo == null || teamInfo.gametype == null || teamInfo.gametype.equals("")
        || teamInfo.teamData == null || teamInfo.teamData.length < 1 || !teamInfo.roundOver()) {
    ...
    drawModalRectWithCustomSizedTexture(guiOriginX + 7, guiOriginY + 109, 259, 109,
        (int) (242 * XPProgress), 10, textureX, textureY);

    if (state.ordinal() >= EnumResultsState.LEVEL_UP.ordinal()
        && state.ordinal() <= EnumResultsState.REVEAL_UNLOCK4.ordinal()) {
      drawModalRectWithCustomSizedTexture(guiOriginX + 5, guiOriginY + 120, 266, 120, 246, 38,
          textureX, textureY);
    }

    if (XPForNextLevel > 0) {
      drawCenteredString(fontRendererObj, displayXP + " / " + XPForNextLevel, guiOriginX + 128,
          guiOriginY + 110, 0xffffff);
    } else {
      drawCenteredString(fontRendererObj, "" + displayXP, guiOriginX + 128, guiOriginY + 110,
          0xffffff);
    }

    // Draw text
    drawString(fontRendererObj, (ClientTeamsData.timeLeftInStage / 20) + "", guiOriginX + 12,
        guiOriginY + 12, 0xffffff);
    drawCenteredString(fontRendererObj, "ROUND OVER", guiOriginX + 128, guiOriginY + 12, 0xffffff);

    if (state.ordinal() >= EnumResultsState.LEVEL_UP.ordinal()
        && state.ordinal() <= EnumResultsState.REVEAL_UNLOCK4.ordinal()) {
      drawCenteredString(fontRendererObj, "RANK INCREASED", guiOriginX + 128, guiOriginY + 135,
          0xffffff);
    } else {
      drawString(fontRendererObj, "Rank " + displayRank, guiOriginX + 44, guiOriginY + 135,
          0xffffff);
    ...
    if (state.ordinal() >= EnumResultsState.SHOW_LINE_1_XP.ordinal()) {
      ...
      drawString(fontRendererObj, "" + earnedXP,
          guiOriginX + 244 - fontRendererObj.getStringWidth("" + earnedXP), guiOriginY + 31,
          0xffffff);
    }
    ...
    if (state.ordinal() >= EnumResultsState.SHOW_LINE_5_TOTAL.ordinal()) {
      ...
      drawString(fontRendererObj, "" + earnedXP,
          guiOriginX + 244 - fontRendererObj.getStringWidth("" + earnedXP), guiOriginY + 91,
          0xffffff);
    }
