...
public class TeamsMap {
  ...
  public void addBaseFirstTime(ITeamBase base) {
    ...
    ForgeChunkManager.forceChunk(chunkLoadingTicket,
        new ChunkCoordIntPair((int) base.getPosX() >> 4, (int) base.getPosZ() >> 4));
    FlansMod.log(
        "Added chunk at " + ((int) base.getPosX() >> 4) + ",  " + ((int) base.getPosZ() >> 4)
            + " to chunk loading ticket for base " + name);
  }
  ...
  public void removeBase(ITeamBase base) {
    ...
    ForgeChunkManager.unforceChunk(chunkLoadingTicket,
        new ChunkCoordIntPair((int) base.getPosX() >> 4, (int) base.getPosZ() >> 4));
    FlansMod.log(
        "Removed chunk at " + ((int) base.getPosX() >> 4) + ",  " + ((int) base.getPosZ() >> 4)
            + " from chunk loading ticket for base " + name);
  }
  ...
  public void forceChunkLoading(Ticket ticket) {
    for (ChunkCoordIntPair coord : ticket.getChunkList()) {
      FlansMod.log(
          "Loading chunk at " + coord.chunkXPos + ", " + coord.chunkZPos + " for map : " + name);
