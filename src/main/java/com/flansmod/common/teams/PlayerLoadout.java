...
public class PlayerLoadout {
      ...
      switch (EnumLoadoutSlot.values()[i]) {
        ...
        case secondary: {
          ...
          if (type instanceof GunType) {
            ...
            if (!VerifyType(gun.getBarrel(stack), pool.unlocks[i], currentLevel)) {
              return false;
            }
            if (!VerifyType(gun.getScope(stack), pool.unlocks[i], currentLevel)) {
              return false;
            }
            if (!VerifyType(gun.getStock(stack), pool.unlocks[i], currentLevel)) {
              return false;
            }
            if (!VerifyType(gun.getGrip(stack), pool.unlocks[i], currentLevel)) {
              return false;
            }
            if (!VerifyType(gun.getGeneric(stack, 0), pool.unlocks[i], currentLevel)) {
              return false;
            }

            Paintjob paint = gun.getPaintjob(stack.getItemDamage());
            if (!VerifyPaint(paint, rewardBoxData)) {
              return false;
            }
          }
          break;
  ...
  private boolean VerifyType(InfoType target, ArrayList<LoadoutEntryInfoType> list,
      int currentLevel) {
    if (target == null) {
      return true;
    }
