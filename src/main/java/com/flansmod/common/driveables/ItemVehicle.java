...
import com.flansmod.client.ColorUtils;
...
public class ItemVehicle extends ItemMapBase implements IPaintableItem {
  ...
  private NBTTagCompound getOldTagCompound(ItemStack stack, World world) {
    try {
      ...
      NBTTagCompound tags = CompressedStreamTools.readCompressed(fileinputstream)
          .getCompoundTag("data");
  ...
  public void addInformation(ItemStack stack, EntityPlayer player, List lines,
    ...
    String col = ColorUtils.getInstance().getColorNameFromNbt(tags);
    if (col != null) {
      lines.add(EnumChatFormatting.GOLD + "Color " + col);
    }
  }
  ...
  public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer) {
    ...
    double length = 5D;
    Vec3 posVec = new Vec3(entityplayer.posX, entityplayer.posY + 1.62D - entityplayer.getYOffset(),
        entityplayer.posZ);
    Vec3 lookVec = posVec
        .addVector(sinYaw * cosPitch * length, sinPitch * length, cosYaw * cosPitch * length);
    MovingObjectPosition movingobjectposition = world
        .rayTraceBlocks(posVec, lookVec, type.placeableOnWater);
      ...
      if (type.placeableOnLand || block instanceof BlockLiquid) {
        if (!world.isRemote) {
          world.spawnEntityInWorld(
              new EntityVehicle(world, (double) pos.getX() + 0.5F, (double) pos.getY() + 2.5F,
                  (double) pos.getZ() + 0.5F, entityplayer, type, getData(itemstack, world)));
        }
  ...
  public int getColorFromItemStack(ItemStack par1ItemStack, int par2) {
    ...
    if (tag != null && tag.hasKey("color_r")) {
