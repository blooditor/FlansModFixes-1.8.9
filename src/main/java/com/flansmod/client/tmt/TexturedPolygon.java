...
public class TexturedPolygon {
  ...
  public TexturedPolygon(PositionTextureVertex[] apositionTexturevertex, int par2, int par3,
      int par4, int par5, float par6, float par7) {
  ...
  public void draw(TmtTessellator tessellator, float f) {
    ...
    if (iNormals.size() == 0) {
      if (normals.length == 3) {
        ...
        if (invertNormal) {
          tessellator
              .setNormal(-(float) Vec32.xCoord, -(float) Vec32.yCoord, -(float) Vec32.zCoord);
        } else {
    ...
    for (int i = 0; i < nVertices; i++) {
      ...
      if (i < iNormals.size()) {
        if (invertNormal) {
          tessellator.setNormal(-(float) iNormals.get(i).xCoord, -(float) iNormals.get(i).yCoord,
      ...
      tessellator.addVertexWithUVW((float) positionTexturevertex.vector3D.xCoord * f,
          (float) positionTexturevertex.vector3D.yCoord * f,
          (float) positionTexturevertex.vector3D.zCoord * f, positionTexturevertex.texturePositionX,
          positionTexturevertex.texturePositionY, positionTexturevertex.texturePositionW);
    }
