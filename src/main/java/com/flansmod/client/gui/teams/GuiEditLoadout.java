...
public class GuiEditLoadout extends GuiTeamsBase {
   ...
   */
  private static final ResourceLocation texture = new ResourceLocation("flansmod",
      "gui/LoadoutEditor.png");
  ...
  public void initGui() {
    ...
    buttonList.add(
        new GuiButton(0, width / 2 - WIDTH / 2 + 10, height / 2 - HEIGHT / 2 + 143, 82, 20,
            "Confirm"));
    buttonList.add(
        new GuiButton(1, width / 2 - WIDTH / 2 + 10, height / 2 - HEIGHT / 2 + 165, 82, 20,
            "Cancel"));
  }
  ...
  public void drawScreen(int i, int j, float f) {
    ...
    //Draw the background
    drawModalRectWithCustomSizedTexture(guiOriginX, guiOriginY, 0, 0, WIDTH, HEIGHT, textureX,
        textureY);

    // Draw title text
    drawCenteredString(fontRendererObj, "Edit Loadout " + (selectedLoadout + 1),
        guiOriginX + WIDTH / 2, guiOriginY + 4, 0xffffff);

    // Draw loadout slots panel
    {
      mc.renderEngine.bindTexture(texture);
      drawModalRectWithCustomSizedTexture(guiOriginX + 70,
          guiOriginY + 32 + 22 * selectedSlot.ordinal(), 70, 203, 36, 22, textureX, textureY);

      drawCenteredString(fontRendererObj, "Loadout", guiOriginX + 51, guiOriginY + 18, 0xffffff);
      for (int n = 0; n < EnumLoadoutSlot.values().length; n++) {
        drawCenteredString(fontRendererObj, EnumLoadoutSlot.values()[n].name, guiOriginX + 39,
            guiOriginY + 38 + 22 * n, 0xffffff);
    ...
    {
      mc.renderEngine.bindTexture(texture);
      drawModalRectWithCustomSizedTexture(guiOriginX + 169, guiOriginY + 32 + 22 * selectedCategory,
          70, 203, 36, 22, textureX, textureY);

      drawCenteredString(fontRendererObj, selectedSlot.name, guiOriginX + 150, guiOriginY + 18,
          0xffffff);
      if (selectedSlot.isWeapon) {
        for (int n = 0; n < WEAPON_COMPONENT_NAMES.length; n++) {
          ...
          InfoType type =
              (stack != null && stack.getItem() instanceof IFlanItem) ? ((IFlanItem) stack
                  .getItem()).getInfoType() : null;
          int numUnlocks = type != null ? data.GetNumUnlocksForType(type) : 0;
          if (n == 1 && type != null && numUnlocks > 0) {
            drawCenteredString(fontRendererObj, WEAPON_COMPONENT_NAMES[n] + " (" + numUnlocks + ")",
                guiOriginX + 138, guiOriginY + 38 + 22 * n, 0xffffff);
          } else {
            drawCenteredString(fontRendererObj, WEAPON_COMPONENT_NAMES[n], guiOriginX + 138,
                guiOriginY + 38 + 22 * n, 0xffffff);
          }
          ...
          switch (n) {
            ...
            default: {
              if (stack != null && stack.getTagCompound() != null) {
                NBTTagCompound attachmentTags = stack.getTagCompound()
                    .getCompoundTag("attachments");
                  ...
                  switch (n) {
                    case 2:
                      attachmentStack = ItemStack
                          .loadItemStackFromNBT(attachmentTags.getCompoundTag("scope"));
                      break;
                    case 3:
                      attachmentStack = ItemStack
                          .loadItemStackFromNBT(attachmentTags.getCompoundTag("barrel"));
                      break;
                    case 4:
                      attachmentStack = ItemStack
                          .loadItemStackFromNBT(attachmentTags.getCompoundTag("stock"));
                      break;
                    case 5:
                      attachmentStack = ItemStack
                          .loadItemStackFromNBT(attachmentTags.getCompoundTag("grip"));
                      break;
                    case 6:
                      attachmentStack = ItemStack
                          .loadItemStackFromNBT(attachmentTags.getCompoundTag("generic_0"));
                      break;
                  }
        ...
        for (int n = 0; n < NON_WEAPON_COMPONENT_NAMES.length; n++) {
          drawCenteredString(fontRendererObj, NON_WEAPON_COMPONENT_NAMES[n], guiOriginX + 138,
              guiOriginY + 38 + 22 * n, 0xffffff);
          ItemStack stack = data.loadouts[selectedLoadout].slots[selectedSlot.ordinal()];
          switch (n) {
            case 0: // Main item
      ...
      if (stack != null && stack.getItem() instanceof ItemGun) {
        ...
        if (mainAmmo != null) {
          drawCenteredString(fontRendererObj,
              String.format("%.0f", type.damage * mainAmmo.damageVsLiving * mainAmmo.numBullets),
              guiOriginX + 290, guiOriginY + 60, 0xffffff);
          drawCenteredString(fontRendererObj,
              String.format("%.0f", (50.0f - type.bulletSpread) * 2.0f), guiOriginX + 290,
              guiOriginY + 70, 0xffffff);
          drawCenteredString(fontRendererObj,
              String.format("%d", mainAmmo.roundsPerItem * numClips), guiOriginX + 290,
              guiOriginY + 80, 0xffffff);
        }
      }
    }
    ...
    {
      drawCenteredString(fontRendererObj,
          "Choose " + WEAPON_COMPONENT_NAMES[selectedCategory].toLowerCase(), guiOriginX + 262,
          guiOriginY + 95, 0xffffff);
      ...
      for (int row = 0; row < 4; row++) {
        for (int col = 0; col < 6; col++) {
          ...
          if (entry instanceof LoadoutEntryInfoType) {
            drawSlotInventory(new ItemStack(((LoadoutEntryInfoType) entry).type.getItem()),
                guiOriginX + 209 + col * 18, guiOriginY + 107 + row * 18);
          } else if (entry instanceof LoadoutEntryPaintjob) {
            Paintjob paintjob = ((LoadoutEntryPaintjob) entry).paintjob;

            DrawRarityBackground(paintjob.rarity, guiOriginX + 209 + col * 18,
                guiOriginY + 107 + row * 18);

            drawSlotInventory(new ItemStack(paintjob.parent.getItem(), 1, paintjob.ID),
                guiOriginX + 209 + col * 18, guiOriginY + 107 + row * 18);
          }
          ...
          if (!entry.available) {
            ...
            drawModalRectWithCustomSizedTexture(guiOriginX + 209 + col * 18,
                guiOriginY + 107 + row * 18, 332, 161, 16, 16, textureX, textureY);
            if (entry.unlockLevel > 0) {
              drawCenteredString(fontRendererObj, "" + entry.unlockLevel,
                  guiOriginX + 218 + col * 18, guiOriginY + 112 + row * 18, 0xffffff);
            }
    ...
    if (k == 0 || k == 1) {
      ...
      for (int row = 0; row < 4; row++) {
        for (int col = 0; col < 6; col++) {
          ...
          if (!availableComponents.get(index).available) {
            continue;
          }
  ...
  public void SelectItem(LoadoutEntry entry) {
    ...
    switch (selectedCategory) {
      ...
      {
        if (entry instanceof LoadoutEntryInfoType) {
          data.loadouts[selectedLoadout].slots[selectedSlot.ordinal()] = new ItemStack(
              ((LoadoutEntryInfoType) entry).type.getItem());
        } else if (entry != null) {
        ...
        if (entry instanceof LoadoutEntryPaintjob) {
          if (data.loadouts[selectedLoadout].slots[selectedSlot.ordinal()] != null) {
            data.loadouts[selectedLoadout].slots[selectedSlot.ordinal()]
                .setItemDamage(((LoadoutEntryPaintjob) entry).paintjob.ID);
          } else {
            FlansMod.log("Applying paintjob to null item!");
          }
        } else if (entry != null) {
      ...
      default: // Attachments
      {
        if (entry instanceof LoadoutEntryInfoType || entry == null) {
          ...
          if (stack != null) {
            ...
            if (entry != null) {
              ItemStack attachmentStack = new ItemStack(
                  ((LoadoutEntryInfoType) entry).type.getItem());
            ...
            FlansMod.log("Applying attachment to null item!");
          }
        } else {
          FlansMod.log("Loadout entry doesn't match for slot");
        }
  ...
  public class LoadoutComparator implements Comparator<LoadoutEntry> {
    ...
    public int compare(LoadoutEntry a, LoadoutEntry b) {
      ...
      if (a instanceof LoadoutEntryPaintjob && b instanceof LoadoutEntryPaintjob) {
        if (((LoadoutEntryPaintjob) a).paintjob.rarity.ordinal()
    ...
    if (selectedCategory == 1) // Paint
    {
      ...
      for (LoadoutEntryInfoType entry : pool.unlocks[selectedSlot.ordinal()]) {
        switch (selectedCategory) {
          case 0: // Main item
          {
            if (entry.type instanceof AttachmentType) {
              continue;
            }
            break;
          }
          ...
          default: // Attachments. Check sub-type
          {
            ...
            GunType gunType = ((ItemGun) data.loadouts[selectedLoadout].slots[selectedSlot
                .ordinal()].getItem()).GetType();
            if (!gunType.allowAllAttachments && !gunType.allowedAttachments.contains(entry.type)) {
              continue;
            }
            ...
            switch (selectedCategory) {
              case 2:
                if (attachType != EnumAttachmentType.sights) {
                  continue;
                } else {
                  break;
                }
              case 3:
                if (attachType != EnumAttachmentType.barrel) {
                  continue;
                } else {
                  break;
                }
              case 4:
                if (attachType != EnumAttachmentType.stock) {
                  continue;
                } else {
                  break;
                }
              case 5:
                if (attachType != EnumAttachmentType.grip) {
                  continue;
                } else {
                  break;
                }
              case 6:
                if (attachType != EnumAttachmentType.generic) {
                  continue;
                } else {
                  break;
                }
