...
public class EntityTeamItem extends EntityItemCustomRender implements IEntityAdditionalSpawnData {
  ...
  public EntityTeamItem(TileEntitySpawner te, int i) {
    super(te.getWorld(), te.getPos().getX() + 0.5F, te.getPos().getY() + 0.5F,
        te.getPos().getZ() + 0.5F, te.stacksToSpawn.get(i).copy());
  ...
  public void onUpdate() {
    ...
    if (worldObj.isRemote) {
      angle += 0.05D;
      setPosition(xCoord + 0.5F + Math.cos(angle) * 0.3F, yCoord + 0.5F,
          zCoord + 0.5F + Math.sin(angle) * 0.3F);
    }
  ...
  public void onCollideWithPlayer(EntityPlayer player) {
    if (!worldObj.isRemote) {
      ...
      if ((event.getResult() == Result.ALLOW || var2 <= 0 || player.inventory
          .addItemStackToInventory(getEntityItem()))) {
