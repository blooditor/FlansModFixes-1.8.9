  ...
  public void onUpdate() {
    ...
    if (ammo == null && gunner != null) {
      ...
      if (slot >= 0) {
        ...
        PacketPlaySound
            .sendSoundPacket(posX, posY, posZ, FlansMod.soundRange, dimension, type.reloadSound,
                false);
      }
    }
    if (worldObj.isRemote && gunner != null && gunner == FMLClientHandler.instance()
        .getClient().thePlayer && type.mode == EnumFireMode.FULLAUTO) {
      //Send a packet!
      checkForShooting();
    }
    if (!worldObj.isRemote && isShooting) {
      ...
      if (soundDelay <= 0) {
        ...
        PacketPlaySound
            .sendSoundPacket(posX, posY, posZ, FlansMod.soundRange, dimension, type.shootSound,
                type.distortSound);
      }
    }
    if (soundDelay > 0) {
  ...
  public boolean interactFirst(EntityPlayer player) //interact : change back when Forge updates
  {
    ...
    if (!worldObj.isRemote) {
      ...
      if (gunner == player) {
        ...
        FlansMod.getPacketHandler()
            .sendToAllAround(new PacketMGMount(player, this, false), posX, posY, posZ,
                FlansMod.driveableUpdateRange, dimension);
        return true;
      }

      //If this person is already mounting a gun, dismount it first
      if (PlayerHandler.getPlayerData(player).mountingGun != null && !PlayerHandler
          .getPlayerData(player).mountingGun.isDead) {
      ...
      FlansMod.getPacketHandler()
          .sendToAllAround(new PacketMGMount(player, this, true), posX, posY, posZ,
              FlansMod.driveableUpdateRange, dimension);
      if (ammo == null) {
  ...
  public void setDead() {
    ...
    if (!worldObj.isRemote) {
      if (TeamsManager.weaponDrops == 2) {
        EntityGunItem gunEntity = new EntityGunItem(worldObj, posX, posY, posZ,
            new ItemStack(type.getItem()), Arrays.asList(ammo));
