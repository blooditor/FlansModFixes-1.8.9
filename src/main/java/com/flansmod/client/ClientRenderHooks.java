...
public class ClientRenderHooks {
  ...
  public static final ResourceLocation offHand = new ResourceLocation("flansmod",
      "gui/offHand.png");
  public static final ResourceLocation hitMarker = new ResourceLocation("flansmod",
      "gui/hitMarker.png");
  ...
  /**
   * When Minecraft would render a 2D gun item, instead cancel it and render the gun properly.
   * Render the offhand gun too.
   */
  public void renderHeldItem(RenderHandEvent event) {
    ...
    if (itemToRender != null && itemToRender.getItem() instanceof ItemGun
        && ((ItemGun) itemToRender.getItem()).GetType().model != null) {
      ...
      //fixes shaders GlStateManager.clear(256);
      ...
      Project.gluPerspective(getFOVModifier(partialTicks),
      ...
      boolean flag = mc.getRenderViewEntity() instanceof EntityLivingBase && ((EntityLivingBase) mc
          .getRenderViewEntity()).isPlayerSleeping();
  ...
  private void renderItemInFirstPerson(ItemStack stack, ItemRenderer renderer, float partialTicks) {
    float f1 =
        1.0F - (prevEquippedProgress + (equippedProgress - prevEquippedProgress) * partialTicks);
    ...
    //Do lighting
    int i = this.mc.theWorld.getCombinedLight(new BlockPos(entityplayersp.posX,
    ...
    ClientProxy.gunRenderer
        .renderItem(ItemRenderType.EQUIPPED_FIRST_PERSON, stack, mc.theWorld, mc.thePlayer);
  ...
  private void setupViewBobbing(float partialTicks) {
    if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
      float mod = this.mc.getRenderViewEntity().isSprinting() ? 0.25f : 0.2f;
			if (FlansModClient.zoomProgress > 0.1f) {
				mod *= 0.1f;
			}
      ...
      GlStateManager.translate(MathHelper.sin(f2 * (float) Math.PI) * f3 * 0.5F * mod,
          -Math.abs(MathHelper.cos(f2 * (float) Math.PI) * f3) * mod, 0.0F);
      GlStateManager
          .rotate(MathHelper.sin(f2 * (float) Math.PI) * f3 * 3.0F * mod, 0.0F, 0.0F, 1.0F);
      GlStateManager
          .rotate(Math.abs(MathHelper.cos(f2 * (float) Math.PI - 0.2F) * f3) * 5.0F * mod, 1.0F,
              0.0F, 0.0F);
  ...
  public void update() {
    ...
    if (mc.getRenderViewEntity() instanceof AbstractClientPlayer) {
      AbstractClientPlayer abstractclientplayer = (AbstractClientPlayer) this.mc
          .getRenderViewEntity();
      ...
      if (itemToRender != null && itemstack != null) {
        if (!itemToRender.getIsItemStackEqual(itemstack)) {
          ...
          equippedGun = true;
				...
				equippedGun = itemToRender != null || itemstack != null;

      float maxChange = 0.4F;
      float targetProgress = equippedGun ? 0.0F : 1.0F;
      float difference = MathHelper
          .clamp_float(targetProgress - equippedProgress, -maxChange, maxChange);
    ...
    if (FlansMod.DEBUG && Minecraft.getMinecraft().theWorld != null) {

      for (Object o : Minecraft.getMinecraft().theWorld.playerEntities) {
        EntityPlayer p = (EntityPlayer) o;
        if (o != thePlayer) {
          PlayerData data = PlayerHandler.getPlayerData(p);
          if (data != null) {
						if (data.snapshots[0] != null) {
							data.snapshots[0].renderSnapshot();
						}
  ...
  public void renderThirdPersonWeapons(RenderLivingEvent.Post event) {
    ...
    if (entity.getEquipmentInSlot(0) != null && entity.getEquipmentInSlot(0)
        .getItem() instanceof ItemGun && mainModel instanceof ModelBiped) {
      ...
      float f2 = this
          .interpolateRotation(entity.prevRenderYawOffset, entity.renderYawOffset, partialTicks);
      float f3 = this
          .interpolateRotation(entity.prevRotationYawHead, entity.rotationYawHead, partialTicks);
      float f4 = f3 - f2;
      float f5;
      ...
      float f9 = entity.prevRotationPitch
  ...
  public void renderPlayer(RenderPlayerEvent.Pre event) {
    ...
    if (event.entity instanceof EntityPlayer && FlansModClient.teamInfo != null
        && FlansModClient.teamInfo.gametype != null && !"No Gametype"
        .equals(FlansModClient.teamInfo.gametype)) {
      PlayerScoreData rendering = PacketTeamInfo
          .getPlayerScoreData(event.entity.getName());
      PlayerScoreData thePlayer = PacketTeamInfo
          .getPlayerScoreData(mc.thePlayer.getName());
  ...
  public void cameraSetup(CameraSetup event) {
    if (mc.thePlayer.ridingEntity instanceof IControllable) {
      ...
      float roll = interpolateRotation(cont.getPrevPlayerRoll(), cont.getPlayerRoll(),
...
//        && (FlansModClient.currentScope != null
  ...
  private void RenderScopeOverlay(Tessellator tessellator, int i, int j) {
    ...
    if (FlansModClient.currentScope != null && FlansModClient.currentScope.hasZoomOverlay()
        ...
        && FlansModClient.zoomProgress > 0.8F) {
    ...
    if (overlayTexture != null) {
      ...
      FlansModClient.getWorldRenderer().addVertexWithUV(i / 2 - 2 * j, j, -90D, 0.0D, 1.0D);
      FlansModClient.getWorldRenderer().addVertexWithUV(i / 2 + 2 * j, j, -90D, 1.0D, 1.0D);
      FlansModClient.getWorldRenderer().addVertexWithUV(i / 2 + 2 * j, 0.0D, -90D, 1.0D, 0.0D);
      FlansModClient.getWorldRenderer().addVertexWithUV(i / 2 - 2 * j, 0.0D, -90D, 0.0D, 0.0D);
      tessellator.draw();
  ...
  private void RenderOffHandHighlights(Tessellator tessellator, int i, int j) {
    ...
    if (currentStack != null && currentStack.getItem() instanceof ItemGun && ((ItemGun) currentStack
        .getItem()).GetType().oneHanded) {
      for (int n = 0; n < 9; n++) {
        if (data.offHandGunSlot == n + 1) {
          FlansModClient.getWorldRenderer().startDrawingQuads();
          FlansModClient.getWorldRenderer()
              .addVertexWithUV(i / 2 - 88 + 20 * n, j - 3, zLevel, 16D / 64D, 16D / 32D);
          FlansModClient.getWorldRenderer()
              .addVertexWithUV(i / 2 - 72 + 20 * n, j - 3, zLevel, 32D / 64D, 16D / 32D);
          FlansModClient.getWorldRenderer()
              .addVertexWithUV(i / 2 - 72 + 20 * n, j - 19, zLevel, 32D / 64D, 0D / 32D);
          FlansModClient.getWorldRenderer()
              .addVertexWithUV(i / 2 - 88 + 20 * n, j - 19, zLevel, 16D / 64D, 0D / 32D);
          tessellator.draw();
        } else if (data.isValidOffHandWeapon(mc.thePlayer, n + 1)) {
          FlansModClient.getWorldRenderer().startDrawingQuads();
          FlansModClient.getWorldRenderer()
              .addVertexWithUV(i / 2 - 88 + 20 * n, j - 3, zLevel, 0D / 64D, 16D / 32D);
          FlansModClient.getWorldRenderer()
              .addVertexWithUV(i / 2 - 72 + 20 * n, j - 3, zLevel, 16D / 64D, 16D / 32D);
          FlansModClient.getWorldRenderer()
              .addVertexWithUV(i / 2 - 72 + 20 * n, j - 19, zLevel, 16D / 64D, 0D / 32D);
          FlansModClient.getWorldRenderer()
              .addVertexWithUV(i / 2 - 88 + 20 * n, j - 19, zLevel, 0D / 64D, 0D / 32D);
          tessellator.draw();
        }
      }
    }
      ...
      if (stack != null && stack.getItem() instanceof ItemGun) {
        ...
        for (int n = 0; n < gunType.numAmmoItemsInGun; n++) {
          ...
          if (bulletStack != null && bulletStack.getItem() != null
            ...
            RenderHelper.disableStandardItemLighting();
            String s =
                (bulletStack.getMaxDamage() - bulletStack.getItemDamage()) + "/" + bulletStack
                    .getMaxDamage();
        ...
        if (gunType.oneHanded && data.offHandGunSlot != 0) {
          ...
          if (offHandStack != null && offHandStack.getItem() instanceof ItemGun) {
            ...
            for (int n = 0; n < offHandGunType.numAmmoItemsInGun; n++) {
              ItemStack bulletStack = ((ItemGun) offHandStack.getItem())
                  .getBulletItemStack(offHandStack, n);
              if (bulletStack != null && bulletStack.getItem() != null
                  && bulletStack.getItemDamage() < bulletStack.getMaxDamage()) {
  ...
  private void RenderTeamInfo(Tessellator tessellator, int i, int j) {
    ...
    if (teamInfo != null && FlansModClient.minecraft.thePlayer != null && (teamInfo.numTeams > 0
        || !teamInfo.sortedByTeam)
        && PacketTeamInfo.getPlayerScoreData(FlansModClient.minecraft.thePlayer.getName()) != null) {
      GL11.glEnable(3042 /* GL_BLEND */);
      GL11.glDisable(2929 /* GL_DEPTH_TEST */);
      GL11.glDepthMask(false);
      GL11.glBlendFunc(770, 771);
      ...
      FlansModClient.getWorldRenderer().startDrawingQuads();
      FlansModClient.getWorldRenderer()
          .addVertexWithUV(i / 2 - 43, 27, -90D, 85D / 256D, 27D / 256D);
      FlansModClient.getWorldRenderer()
          .addVertexWithUV(i / 2 + 43, 27, -90D, 171D / 256D, 27D / 256D);
      FlansModClient.getWorldRenderer()
          .addVertexWithUV(i / 2 + 43, 0D, -90D, 171D / 256D, 0D / 256D);
      FlansModClient.getWorldRenderer()
          .addVertexWithUV(i / 2 - 43, 0D, -90D, 85D / 256D, 0D / 256D);
      tessellator.draw();
      ...
      if (teamInfo.numTeams == 2 && teamInfo.sortedByTeam) {
        if (teamInfo.teamData == null || teamInfo.teamData[0] == null
            || teamInfo.teamData[0].team == null || teamInfo.teamData[1] == null
            || teamInfo.teamData[1].team == null) {
        ...
        GL11.glColor4f(((colour >> 16) & 0xff) / 256F, ((colour >> 8) & 0xff) / 256F,
            (colour & 0xff) / 256F, 1.0F);
        FlansModClient.getWorldRenderer().startDrawingQuads();
        FlansModClient.getWorldRenderer()
            .addVertexWithUV(i / 2 - 43, 27, -90D, 0D / 256D, 125D / 256D);
        FlansModClient.getWorldRenderer()
            .addVertexWithUV(i / 2 - 19, 27, -90D, 24D / 256D, 125D / 256D);
        FlansModClient.getWorldRenderer()
            .addVertexWithUV(i / 2 - 19, 0D, -90D, 24D / 256D, 98D / 256D);
        FlansModClient.getWorldRenderer()
            .addVertexWithUV(i / 2 - 43, 0D, -90D, 0D / 256D, 98D / 256D);
        tessellator.draw();
        ...
        FlansModClient.getWorldRenderer().startDrawingQuads();
        FlansModClient.getWorldRenderer()
            .addVertexWithUV(i / 2 + 19, 27, -90D, 62D / 256D, 125D / 256D);
        FlansModClient.getWorldRenderer()
            .addVertexWithUV(i / 2 + 43, 27, -90D, 86D / 256D, 125D / 256D);
        FlansModClient.getWorldRenderer()
            .addVertexWithUV(i / 2 + 43, 0D, -90D, 86D / 256D, 98D / 256D);
        FlansModClient.getWorldRenderer()
            .addVertexWithUV(i / 2 + 19, 0D, -90D, 62D / 256D, 98D / 256D);
        tessellator.draw();
        ...
        if (teamInfo.teamData[0] != null && teamInfo.teamData[1] != null) {
          ...
          mc.fontRendererObj.drawString(teamInfo.teamData[1].score + "",
              i / 2 + 35 - mc.fontRendererObj.getStringWidth(teamInfo.teamData[1].score + ""), 9,
              0x000000);
          mc.fontRendererObj.drawString(teamInfo.teamData[1].score + "",
              i / 2 + 34 - mc.fontRendererObj.getStringWidth(teamInfo.teamData[1].score + ""), 8,
              0xffffff);
        }
      }
      ...
      mc.fontRendererObj.drawString(teamInfo.map + "",
          i / 2 - 47 - mc.fontRendererObj.getStringWidth(teamInfo.map + ""), 9, 0x000000);
      mc.fontRendererObj.drawString(teamInfo.map + "",
          i / 2 - 48 - mc.fontRendererObj.getStringWidth(teamInfo.map + ""), 8, 0xffffff);
      ...
      mc.fontRendererObj
          .drawString(timeLeft, i / 2 - mc.fontRendererObj.getStringWidth(timeLeft) / 2 - 1, 29,
              0x000000);
      mc.fontRendererObj
          .drawString(timeLeft, i / 2 - mc.fontRendererObj.getStringWidth(timeLeft) / 2, 30,
              0xffffff);
  ...
  private void RenderKillMessages(int i, int j) {
    //addKillMessage(false, InfoType.getType("m16a4"), "4blooditor", "faboy");
    for (int n = 0; n < killMessages.size(); n++) {
      KillMessage killMessage = killMessages.get(n);
      mc.fontRendererObj.drawString(
          "\u00a7" + killMessage.killerName + "    " + (killMessage.headshot
              ? "\u00a74\u00a7l\u271b" : "  ") + "\u00a7" + killMessage.killedName,
          i - mc.fontRendererObj
              .getStringWidth(killMessage.killerName + "     " + killMessage.killedName) - 6,
          j - 46 - killMessage.line * 16, 0xffffff);
    }
    ...
    for (int n = 0; n < killMessages.size(); n++) {
      KillMessage killMessage = killMessages.get(n);
      drawSlotInventory(mc.fontRendererObj,
          new ItemStack(killMessage.weapon.item, 1, killMessage.paint),
          i - mc.fontRendererObj.getStringWidth("     " + killMessage.killedName) - 12,
          j - 50 - killMessage.line * 16);
    }
  ...
  private void RenderVehicleDebug() {
    ...
    if (mc.thePlayer.ridingEntity instanceof EntitySeat && FlansMod.DEBUG) {
      EntityDriveable ent = ((EntitySeat) mc.thePlayer.ridingEntity).driveable;
      ...
      float speed = ent.getRealSpeedXYZ();

      //speed = (int)(speed * 10F) / 10F;

      mc.fontRendererObj.drawString("Speed: " + speed + "", 2, 2, 0xffffff);
      ...
      boolean onGround = false;
			for (Entity wh : ent.wheels) {
				if (wh != null && wh.onGround) {
					onGround = true;
				}
			}
      mc.fontRendererObj.drawString("OnGround : " + onGround, 2, 22, 0xffffff);
      //	mc.fontRendererObj.drawString("Pitch : " + ent.prevRotationPitch, 2, 32, 0xffffff);
      //	mc.fontRendererObj.drawString("Roll : " + ent.prevRotationRoll, 2, 42, 0xffffff);
      //}
    }
  }
  ...
  private void drawSlotInventory(FontRenderer fontRenderer, ItemStack itemstack, int i, int j) {
    ...
    itemRenderer.renderItemOverlayIntoGUI(fontRenderer, itemstack, i, j,
        null); //May be something other than null
  }

  public static void addKillMessage(boolean headshot, InfoType infoType, String killer,
      String killed) {
