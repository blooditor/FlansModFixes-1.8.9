...
public class GuiVoting extends GuiScreen {
  ...
  public void initGui() {
    ...
    for (int i = 0; i < ClientTeamsData.roundFinishedData.votingOptions.length; i++) {
      buttonList.add(
          new GuiButton(i, width / 2 + 128 - 50, height / 2 - guiHeight / 2 + 24 + 24 * i, 40, 20,
              "Vote"));
    }
  }
  ...
  public void drawScreen(int i, int j, float f) {
    ...
    drawString(fontRendererObj, (ClientTeamsData.timeLeftTotal / 20) + "", m + 256 - 20, n + 8,
        0xffffff);

    for (int p = 0; p < ClientTeamsData.roundFinishedData.votingOptions.length; p++) {
      drawString(fontRendererObj, ClientTeamsData.roundFinishedData.votingOptions[p].mapName,
          m + 10, n + 25 + 24 * p, 0xffffff);
      drawString(fontRendererObj,
          ClientTeamsData.roundFinishedData.votingOptions[p].gametype + " : \u00a7"
              + ClientTeamsData.roundFinishedData.votingOptions[p].teamNames[0] + ", \u00a7"
              + ClientTeamsData.roundFinishedData.votingOptions[p].teamNames[1], m + 10,
          n + 35 + 24 * p, 0xffffff);

      drawCenteredString(fontRendererObj, (myVote == p + 1 ? "\u00a72" : "")
              + ClientTeamsData.roundFinishedData.votingOptions[p].numVotes, m + 196, n + 31 + 24 * p,
          0xffffff);
    }
