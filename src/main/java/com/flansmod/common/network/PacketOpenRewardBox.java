...
public class PacketOpenRewardBox extends PacketBase {
  ...
  public void handleServerSide(EntityPlayerMP playerEntity) {
    ...
    if (box == null) {
      FlansMod.Assert(false, "Recieved invalid reward box open packet from player " + playerEntity
          .getDisplayNameString());
    } else {
      FlansMod.log(
          "Recieved reward box open packet from player " + playerEntity.getDisplayNameString()
              + " for box " + box.shortName);
