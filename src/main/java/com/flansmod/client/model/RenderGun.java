...
public class RenderGun implements IItemRenderer {
  ...
  public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
    ...
    //Render main hand gun
    {
      GunAnimations animations =
          (type == ItemRenderType.ENTITY || type == ItemRenderType.INVENTORY) ? new GunAnimations(
              null) : FlansModClient.getGunAnimations((EntityLivingBase) data[1], false);
      if (type == ItemRenderType.EQUIPPED && data[1] instanceof EntityLivingBase
          && data[1] != Minecraft.getMinecraft().thePlayer) {
        animations.lookAt =
            ((EntityLivingBase) data[1]).isSneaking() ? GunAnimations.LookAtState.LOOK1
                : GunAnimations.LookAtState.NONE;
      }
    ...
    if (gunType.oneHanded && type == ItemRenderType.EQUIPPED_FIRST_PERSON) {
      ...
      if (entity instanceof EntityPlayer) {
        ...
        if (playerData.offHandGunSlot != 0) {
          ...
          if (animations == null) {
            animations = new GunAnimations(entity);
  ...
  public void renderOffHandGun(EntityPlayer player, ItemStack offHandItemStack) {
    ...
    if (animations == null) {
      animations = new GunAnimations(player);
  ...
  private void renderGun(ItemRenderType type, ItemStack item, GunType gunType,
      GunAnimations animations, boolean offHand, Object... data) {
    //The model scale
    float f = 1F / 16F;
    ModelGun model = gunType.model;
    ...
    GL11.glPushMatrix();
    {
      ...
      switch (type) {
        case ENTITY: {
          ...
          GL11.glTranslatef(-0.45F + model.itemFrameOffset.x, -0.05F + model.itemFrameOffset.y,
              model.itemFrameOffset.z);
          break;
        }
        case INVENTORY: {
          GL11.glTranslatef(model.itemFrameOffset.x, model.itemFrameOffset.y,
              model.itemFrameOffset.z);
          break;
        }
        case EQUIPPED: {
          if (offHand) {
            ...
            GL11.glTranslatef(0.12F, 0F, 0F);
            GL11.glScalef(1F, 1F, -1F);

            if (animations.lookAt == GunAnimations.LookAtState.LOOK1) {
            ...
            if (animations.reloading) {
              ...
              float effectiveReloadAnimationProgress = animations.lastReloadAnimationProgress
                  + (animations.reloadAnimationProgress - animations.lastReloadAnimationProgress)
                  * smoothing;
              reloadRotate = 1F;
							if (effectiveReloadAnimationProgress < model.tiltGunTime) {
								reloadRotate = effectiveReloadAnimationProgress / model.tiltGunTime;
							}
							if (effectiveReloadAnimationProgress
									> model.tiltGunTime + model.unloadClipTime + model.loadClipTime) {
								reloadRotate = 1F -
										(effectiveReloadAnimationProgress - (model.tiltGunTime + model.unloadClipTime
												+ model.loadClipTime)) / model.untiltGunTime;
							}
              ...
              switch (model.animationType) {
                ...
                case REVOLVER: {
                  ...
                  GL11.glRotatef(-10F * reloadRotate * flip, 1F, 0F, 0F);
                  GL11.glTranslatef(0.1F * reloadRotate, -0.2F * reloadRotate,
                      -0.1F * reloadRotate);
                  break;
                }
                ...
                case CUSTOM: {
                  ...
                  GL11.glTranslatef(model.translateGun.x * reloadRotate,
                      model.translateGun.y * reloadRotate, model.translateGun.z * reloadRotate);
                  break;
                }
                default:
                  break;
          ...
          GL11.glTranslatef(model.thirdPersonOffset.x, model.thirdPersonOffset.y,
              model.thirdPersonOffset.z);
					/*
        ...
        case EQUIPPED_FIRST_PERSON: {
          ...
          float adsSwitch = FlansModClient.lastZoomProgress
              + (FlansModClient.zoomProgress - FlansModClient.lastZoomProgress)
              * smoothing;//0F;//((float)Math.sin((FlansMod.ticker) / 10F) + 1F) / 2F;

          AttachmentType sc = gunType.getScope(item);
          float modY = 0.90f;
          float modX = 0.3f;
          if (sc != null) {
            switch (sc.shortName) {
          ...
          if (modY != 0.9f) {
            modY += (0.3125f - model.scopeAttachPoint.y * gunType.modelScale) * 6.0f;
          } else if (model.scopeAttachPoint.y != 0) {
						modY += (0.2 - model.scopeAttachPoint.y * gunType.modelScale);
					}

          //	if(FlansModClient.zoomProgress > 0.99f)
          //		Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText( model.scopeAttachPoint.y + " " + gunType.modelScale));
          ...
          if (offHand) {
            ...
            GL11.glTranslatef(-1F - adsSwitch * modX, 0.675F + 0.180F * adsSwitch * modY,
                -1F - 0.3978F * adsSwitch);
          ...
          if (animations.switchAnimationProgress > 0 && animations.switchAnimationLength > 0) {
            ...
            //	endAngles = new Vector3f(startAngles.x + (endAngles.x - startAngles.x) * interp, startAngles.y + (endAngles.y - startAngles.y) * interp, startAngles.z + (endAngles.z - startAngles.z) * interp);
            //	pos2 = new Vector3f(pos1.x + (pos2.x - pos1.x) * interp, pos1.y + (pos2.y - pos1.y) * interp, pos1.z + (pos2.z - pos1.z) * interp);
          ...
          if (animations.meleeAnimationProgress > 0
            ...
            Vector3f nextMeleePos =
                animations.meleeAnimationProgress + 1 < gunType.meleePath.size() ? gunType.meleePath
                    .get(animations.meleeAnimationProgress + 1) : new Vector3f();
            ...
            Vector3f nextMeleeAngles =
                animations.meleeAnimationProgress + 1 < gunType.meleePathAngles.size()
                    ? gunType.meleePathAngles.get(animations.meleeAnimationProgress + 1)
                    : new Vector3f();
            GL11.glRotatef(meleeAngles.y + (nextMeleeAngles.y - meleeAngles.y) * smoothing, 0F, 1F,
                0F);
            GL11.glRotatef(meleeAngles.z + (nextMeleeAngles.z - meleeAngles.z) * smoothing, 0F, 0F,
                1F);
            GL11.glRotatef(meleeAngles.x + (nextMeleeAngles.x - meleeAngles.x) * smoothing, 1F, 0F,
                0F);
          }
          ...
          GL11.glRotatef(
              -animations.recoilAngle * (float) Math.sqrt(gunType.recoil.vertical) * 0.4f, 0F, 0F,
              1F);
          GL11.glRotatef(
              0.1f * (FlansModClient.playerRecoil.horizontal < 0 ? -1 : 1) * animations.recoilAngle
                  * (float) Math.sqrt(gunType.recoil.horizontal) * 1.5f, 0F, 1F, 0F);
          GL11.glTranslatef(animations.recoilOffset.x, animations.recoilOffset.y,
              animations.recoilOffset.z);

          if (model.spinningCocking) {
            GL11.glTranslatef(model.spinPoint.x, model.spinPoint.y, model.spinPoint.z);
            float pumped = (animations.lastPumped
                + (animations.pumped - animations.lastPumped) * smoothing);
   ...
   */
  public void renderGun(ItemStack item, GunType type, float f, ModelGun model,
      GunAnimations animations, float reloadRotate) {
    ...
    GL11.glPushMatrix();
    {
      ...
      GL11.glPushMatrix();
      {
        GL11.glTranslatef(-(1 - Math
            .abs(animations.lastPumped + (animations.pumped - animations.lastPumped) * smoothing))
      ...
      GL11.glPushMatrix();
      {
        ...
        switch (model.animationType) {
          ...
          case BACK_LOADED: {
						if (empty) {
							shouldRender = animations.reloading;
						}
            break;
          }
          default:
        ...
        if (shouldRender && animations.reloading
          ...
          float clipPosition = 0F;
					...
					if (effectiveReloadAnimationProgress >= model.tiltGunTime + model.unloadClipTime
							&& effectiveReloadAnimationProgress
							< model.tiltGunTime + model.unloadClipTime + model.loadClipTime) {
						clipPosition = 1F
								- (effectiveReloadAnimationProgress - (model.tiltGunTime + model.unloadClipTime))
								/ model.loadClipTime;
          ...
          float loadOnlyClipPosition = Math.max(0F, Math.min(1F,
              1F - ((effectiveReloadAnimationProgress - model.tiltGunTime) / (model.unloadClipTime
                  + model.loadClipTime))));
          ...
          switch (model.animationType) {
            ...
            case CUSTOM: {
              ...
              GL11.glTranslatef(model.translateClip.x * clipPosition,
                  model.translateClip.y * clipPosition, model.translateClip.z * clipPosition);
              break;
            }
            case END_LOADED: {
    ...
    if (scopeAttachment != null) {
      GL11.glPushMatrix();
      {
        Paintjob scopepaintjob = scopeAttachment.getPaintjob(scopeItemStack == null? scopeAttachment == type.defaultSight? type.defaultSightId : 0 : scopeItemStack.getItemDamage());
        ...
        GL11.glTranslatef(model.scopeAttachPoint.x * type.modelScale,
            model.scopeAttachPoint.y * type.modelScale, model.scopeAttachPoint.z * type.modelScale);

				if (model.scopeIsOnSlide) {
					GL11.glTranslatef(-(animations.lastGunSlide
							+ (animations.gunSlide - animations.lastGunSlide) * smoothing)
							* model.gunSlideDistance, 0F, 0F);
				}
        GL11.glScalef(scopeAttachment.modelScale, scopeAttachment.modelScale,
            scopeAttachment.modelScale);
    ...
    if (gripAttachment != null) {
      GL11.glPushMatrix();
      {
        ...
        GL11.glTranslatef(model.gripAttachPoint.x * type.modelScale,
            model.gripAttachPoint.y * type.modelScale, model.gripAttachPoint.z * type.modelScale);
				if (model.gripIsOnPump) {
					GL11.glTranslatef(-(1 - Math
							.abs(animations.lastPumped + (animations.pumped - animations.lastPumped) * smoothing))
							* model.pumpHandleDistance, 0F, 0F);
				}
        GL11.glScalef(gripAttachment.modelScale, gripAttachment.modelScale,
    ...
    if (barrelAttachment != null) {
      GL11.glPushMatrix();
      {
        ...
        GL11.glScalef(barrelAttachment.modelScale, barrelAttachment.modelScale,
            barrelAttachment.modelScale);
    ...
    if (stockAttachment != null) {
      GL11.glPushMatrix();
      {
        ...
        GL11.glTranslatef(model.stockAttachPoint.x * type.modelScale,
            model.stockAttachPoint.y * type.modelScale, model.stockAttachPoint.z * type.modelScale);
        GL11.glScalef(stockAttachment.modelScale, stockAttachment.modelScale,
            stockAttachment.modelScale);
