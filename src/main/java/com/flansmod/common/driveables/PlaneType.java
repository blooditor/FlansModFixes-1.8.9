...
public class PlaneType extends DriveableType {
  ...
  /**
   * The positions, parent parts and recipe items of the propellers, used to calculate forces and
   * render the plane correctly
   */
  public ArrayList<Propeller> propellers = new ArrayList<Propeller>();
  /**
   * The positions, parent parts and recipe items of the helicopter propellers, used to calculate
   * forces and render the plane correctly
   */
  ...
  public void read(String[] split, TypeFile file) {
    ...
    try {
      ...
      if (split[0].equals("Propeller")) {
        Propeller propeller = new Propeller(Integer.parseInt(split[1]), Integer.parseInt(split[2]),
            Integer.parseInt(split[3]), Integer.parseInt(split[4]),
            EnumDriveablePart.getPart(split[5]), PartType.getPart(split[6]));
  ...
  @Override
  public EntityDriveable createDriveable(World world, double x, double y, double z,
      DriveableData data) {
