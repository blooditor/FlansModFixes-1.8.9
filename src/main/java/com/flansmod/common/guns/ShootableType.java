...
public abstract class ShootableType extends InfoType {
   ...
   */
  public float damageVsLiving = 1, damageVsDriveable = 1, damageVsPlane = 1;
  /**
  ...
  /**
   * If 0, then the grenade will last until some other detonation condition is met, else the grenade
   * will detonate after this time (in ticks)
   */
  public int fuse = 0;
  /**
  ...
  public void read(String[] split, TypeFile file) {
    ...
    try {
      ...
      //Hit stuff
      else if (split[0].equals("HitEntityDamage") || split[0].equals("DamageVsLiving") || split[0]
          .equals("DamageVsPlayer")) {
        damageVsLiving = Float.parseFloat(split[1]);
      } else if (split[0].equals("DamageVsVehicles")) {
        damageVsDriveable = Float.parseFloat(split[1]);
        damageVsPlane = damageVsDriveable;
      } else if (split[0].equals("Damage")) {
        damageVsLiving = damageVsDriveable = damageVsPlane = Float.parseFloat(split[1]);
