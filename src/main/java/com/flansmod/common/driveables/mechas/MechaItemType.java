...
public class MechaItemType extends InfoType {
  ...
  /**
   * This makes the mecha float towards the surface when underwater, because apparently people
   * prefer limited functionality
   */
  ...
  /**
   * The following are a ton of upgrade flags and modifiers. The mecha will iterate over all
   * upgrades in its inventory multiplying multipliers and looking for true booleans in order to
   * decide if things should happen or what certain values should take
   */
  ...
  /**
   * The drop rate of these items are multiplied by this float. They stack between items too. Once
   * dropRate has been calculated, each block then gives floor(dropRate) items with a dropRate -
   * floor(dropRate) chance of getting one more
   */
