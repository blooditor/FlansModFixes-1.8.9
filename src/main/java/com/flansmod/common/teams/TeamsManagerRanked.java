...
public class TeamsManagerRanked extends TeamsManager {
  ...
  public void tick() {
    ...
    if (interRoundTimeLeft > 0 && time % 10 == 0
        && roundFinishedTemplateData.votingOptions != null
        && roundFinishedTemplateData.votingOptions.length > 0) {
      for (int i = 0; i < roundFinishedTemplateData.votingOptions.length; i++) {
        roundFinishedTemplateData.votingOptions[i].numVotes = 0;
      }
      for (EntityPlayer player : getPlayers()) {
        PlayerData data = PlayerHandler.getPlayerData(player);
        if (!data.builder && data.vote != 0
            && data.vote - 1 < roundFinishedTemplateData.votingOptions.length) {
  ...
  public static void OpenRewardBox(EntityPlayerMP player, RewardBox box) {
    ...
    for (RewardBoxInstance instance : data.rewardBoxData) {
      if (!instance.opened
        ...
        FlansMod.getPacketHandler()
            .sendTo(new PacketOpenRewardBox(box.hashCode(), unlockHash), player);
        return;
      }
    }

    FlansMod.Assert(false,
        "Player " + player.getDisplayNameString() + " tried to open box they don't have");
  }

  public static PlayerRankData GetRankData(EntityPlayer player) {
    return rankData.get(player.getUniqueID());
  }
