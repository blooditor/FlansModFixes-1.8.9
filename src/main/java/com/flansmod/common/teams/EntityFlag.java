...
public class EntityFlag extends Entity implements ITeamObject {
    ...
    if (ridingEntity != null && ridingEntity.isDead) {
      if (ridingEntity instanceof EntityPlayerMP) {
        ...
        TeamsManager.messageAll(
            "\u00a7f" + player.getName() + " dropped the \u00a7" + team.textColour + team.name
                + "\u00a7f flag");
      }
      mountEntity(null);
    ...
    if (timeUntilReturn > 0) {
      if (ridingEntity != null || isHome) {
        ...
        if (timeUntilReturn == 0) {
          ...
          TeamsManager.messageAll("\u00a7fThe \u00a7" + flagTeam.textColour + flagTeam.name
              + "\u00a7f flag returned itself");
        }
      }
    }
  ...
  public void mountEntity(Entity entity) {
    if (entity == null) {
      if (TeamsManager.getInstance().currentRound != null && TeamsManager
          .getInstance().currentRound.gametype instanceof GametypeCTF) {
        timeUntilReturn =
            ((GametypeCTF) TeamsManager.getInstance().currentRound.gametype).flagReturnTime * 20;
      } else {
        timeUntilReturn = 600; //30 seconds
      }
    }
