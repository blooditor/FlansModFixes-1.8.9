...
import com.flansmod.apocalypse.client.model.ModelSurvivor;
...
public class ClientProxyApocalypse extends CommonProxyApocalypse {
  ...
  public void init(FMLInitializationEvent event) {
    super.init(event);
    registerVanillaItemModel(FlansModApocalypse.sulphur,
        Item.getItemFromBlock(FlansModApocalypse.blockSulphur),
        Item.getItemFromBlock(FlansModApocalypse.blockSulphuricAcid),
        Item.getItemFromBlock(FlansModApocalypse.blockLabStone),
        Item.getItemFromBlock(FlansModApocalypse.blockPowerCube));
    ClientRegistry.bindTileEntitySpecialRenderer(TileEntityPowerCube.class, new RenderPowerCube());
    RenderingRegistry.registerEntityRenderingHandler(EntitySurvivor.class, new RenderSurvivor(Minecraft.getMinecraft().getRenderManager(), new ModelSurvivor(), 0));
    RenderingRegistry.registerEntityRenderingHandler(EntityFakePlayer.class,
        new RenderFakePlayer(Minecraft.getMinecraft().getRenderManager(), new ModelBiped(), 0));
    RenderingRegistry.registerEntityRenderingHandler(EntityTeleporter.class,
        new RenderTeleporter(Minecraft.getMinecraft().getRenderManager()));
    RenderingRegistry.registerEntityRenderingHandler(EntityAIMecha.class,
        new RenderMecha(Minecraft.getMinecraft().getRenderManager()));
    RenderingRegistry.registerEntityRenderingHandler(EntityNukeDrop.class,
        new RenderNukeDrop(Minecraft.getMinecraft().getRenderManager()));
  }
    ...
    for (Item item : items) {
      Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(item, 0,
          new ModelResourceLocation(
              FlansModApocalypse.MODID + ":" + item.getUnlocalizedName().split("\\.")[1],
              "inventory"));
    }
  }
  ...
  public void eventHandler(RenderGameOverlayEvent event) {
    ...
    if (apocalypseCountdown > 0 && FlansMod.DEBUG) {
      mc.fontRendererObj
          .drawString("Seconds to the apocalypse: " + (apocalypseCountdown / 20), 2, 2, 0xffffff);
    }
        ...
        if (obj instanceof EntityNukeDrop) {
          ...
          if (mc.thePlayer.getDistanceSqToEntity(nuke) < scale * scale) {
            ...
            FlansModClient.getWorldRenderer()
                .addVertexWithUV(i / 2 + 2 * j, 0.0D, -90D, 1.0D, 0.0D);
            FlansModClient.getWorldRenderer()
                .addVertexWithUV(i / 2 - 2 * j, 0.0D, -90D, 0.0D, 0.0D);
            tessellator.draw();
