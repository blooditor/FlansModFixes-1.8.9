...
public class ArmourBoxType extends BoxType {
          ...
          for (int j = 0; j < (lineSplit.length - 1) / 2; j++) {
            ...
            if (lineSplit[j * 2 + 1].contains(".")) {
              stack = getRecipeElement(lineSplit[j * 2 + 1].split("\\.")[0],
                  Integer.valueOf(lineSplit[j * 2 + 2]),
                  Integer.valueOf(lineSplit[j * 2 + 1].split("\\.")[1]), shortName);
            } else {
              stack = getRecipeElement(lineSplit[j * 2 + 1], Integer.valueOf(lineSplit[j * 2 + 2]),
                  0, shortName);
            }
  ...
  /**
   * Each instance of this class refers to one page full of recipes, that is, one full set of
   * armour
   */
