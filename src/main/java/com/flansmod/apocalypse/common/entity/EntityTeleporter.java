...
public class EntityTeleporter extends Entity {
  ...
  public void onUpdate() {
    ...
    if (lowerLeftCornerPowerCube == null) {
      lowerLeftCornerPowerCube = new BlockPos(MathHelper.floor_double(posX - 1.5D),
          MathHelper.floor_double(posY + 0.5D), MathHelper.floor_double(posZ - 1.5D));
    }

    if (!worldObj.isRemote) {
      for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
          if (worldObj.getBlockState(lowerLeftCornerPowerCube.add(3 * i, 0, 3 * j)).getBlock()
              != FlansModApocalypse.blockPowerCube) {
            setDead();
    ...
    for (int i = 0; i < 10; i++) {
      ...
      worldObj
          .spawnParticle(EnumParticleTypes.PORTAL, posX + dX, posY + 1 + dY, posZ + dZ, dX, dY, dZ);
    }
  }
  ...
  public void onCollideWithPlayer(EntityPlayer player) {
    if (!worldObj.isRemote) {
      if (targetTeleporter == null
          && worldObj.provider.getDimensionId() == FlansModApocalypse.dimensionID) {
        findPortal(player);
      }
        ...
        if (worldObj.provider.getDimensionId() == 0) {
          MinecraftServer.getServer().getConfigurationManager()
              .transferPlayerToDimension((EntityPlayerMP) player, FlansModApocalypse.dimensionID,
                  new TeleporterApocalypse(MinecraftServer.getServer()
                      .worldServerForDimension(FlansModApocalypse.dimensionID),
                      this.targetTeleporter));
        } else {
          MinecraftServer.getServer().getConfigurationManager()
              .transferPlayerToDimension((EntityPlayerMP) player, 0,
                  new TeleporterApocalypse(MinecraftServer.getServer().worldServerForDimension(0),
                      this.targetTeleporter));
        }
      }
    }
  ...
  private void findPortal(EntityPlayer player) {
    ...
    for (int i = overworld.rand.nextInt(100); i < 300; i++) {
      ...
      for (; pos.getY() >= 0; pos = pos.down()) {
        if (overworld.isAirBlock(pos) && World
            .doesBlockHaveSolidTopSurface(overworld, pos.down())) {
  ...
  private boolean createPortal(World world, BlockPos pos) {
    ...
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 2; j++) {
        ...
        world.setBlockState(pos.add(i * 3, 0, j * 3),
    ...
    for (int i = 0; i < 4; i++) {
      for (int k = 0; k < 4; k++) {
        for (int j = -1; j >= 1 && world.getBlockState(pos.add(i, j, k)).getBlock() == Blocks.air;
            j--) {
  ...
  protected void readEntityFromNBT(NBTTagCompound tags) {
    lowerLeftCornerPowerCube = new BlockPos(tags.getInteger("X"), tags.getInteger("Y"),
        tags.getInteger("Z"));
    this.setPosition(lowerLeftCornerPowerCube.getX() + 2D, lowerLeftCornerPowerCube.getY() + 1D,
        lowerLeftCornerPowerCube.getZ() + 2D);
    if (tags.hasKey("targetX")) {
      targetTeleporter = new BlockPos(tags.getInteger("targetX"), tags.getInteger("targetY"),
          tags.getInteger("targetZ"));
    }
  }
