...
/**
 * The Bone class makes it possible to create skeletons, which should help you out in animating your
 * mobs a little bit more easy. However, since you won't work with a graphical interface, creating
 * bones will be different from what you are probably used to. <br /><br /> First, you will need to
 * instantiate every Bone in the constructor of your model file. The default orientation, when all
 * angles are set to zero, will be in the vector (0, 0, length), meaning it will always point
 * backwards on a regular model. You can also set what its parent node is. If a Bone does not have a
 * parent node, it is assumed it is the root node. Each Bone can only have one parent, but several
 * children. Also, all children will inherit the offset position of the root node. <br /><br /> The
 * neutral position basically defines in what direction the Bone normally faces when in rest. This
 * will not affect the rotation of any model currently attached to it or the rotation of the child
 * nodes, but will affect the position of the child nodes when recalculating their positions. The
 * length always defines how far each child Bone will be placed, since child Bones are always placed
 * at the end of their parent Bone. <br /><br /> Once you're ready to render, you can call the
 * prepareDraw method. You only need to apply it to one Bone, since it will always search for the
 * root node to execute the code there. It will then automatically rotate every child Bone and
 * places them at the right position. Finally, use the setAnglesToModels method to rotate each model
 * and place them at the correct spot. Note that if you also apply custom rotation for the
 * individual models that you should apply that after you've run setAnglesToModels, since this will
 * override the settings the model originally had. The best way to solve this is to make a separate
 * method to rotate the Bones. <br /><br /> The following would be an example of a biped with a
 * skeleton. It takes ModelBiped as an example and extends it with a skeleton. First, we have the
 * part that goes in the constructor.
 * <pre>
 ...
 * </pre>
 * <br /><br /> This should generate the same animation of the regular biped. Don't forget to add
 * the individual render methods for each model though, as it won't automatically render them. <br
 * /><br />
 *
 * @author GaryCXJk
 */
public class Bone {
  ...
  /**
   * Constructor to create a bone. This attaches the bone to a parent bone, and will calculate its
   * current position relative to the origin.
   *
   * @param x      the x-rotation of the bone
   * @param y      the y-rotation of the bone
   * @param z      the z-rotation of the bone
  ...
  /**
   * Sets the current offset of the parent root Bone. Note that this will always set the parent root
   * Bone, not the current Bone, as its offset is determined by the offset, rotation and length of
   * its parent.
   *
  ...
  /**
   * Attaches a model to the bone. Its base rotation will be set to the neutral rotation of the
   * model.
   *
  ...
  /**
   * Attaches a model to the bone. If inherit is true, it sets the base rotation to the neutral
   * rotation of the Bone, otherwise it's set to the neutral rotation of the model.
   *
   * @param model   the model to attach
  ...
  /**
   * Attaches a model to the bone. If inherit is true, it sets the base rotation to the neutral
   * rotation of the Bone, otherwise it's set to the neutral rotation of the model. When isUpright
   * is set, the model will be rotated (-PI / 2, 0, 0).
   *
   * @param model     the model to attach
   * @param inherit   whether the model should inherit the Bone's base rotations
  ...
  /**
   * Attaches a model to the bone with a given base rotation. When inherit is true, it will add the
   * Bone's neutral rotation to the given angles.
   *
   * @param model   the model to attach
   * @param x       the base x-rotation
   * @param y       the base y-rotation
  ...
  /**
   * Attaches a model to the bone with a given base rotation. When inherit is true, it will add the
   * Bone's neutral rotation to the given angles. When isUpright is set, the model will be rotated
   * (-PI / 2, 0, 0).
   *
   * @param model     the model to attach
   * @param x         the base x-rotation
  ...
  /**
   * Removes the given model from the Bone. Always detach the model before adding it to another
   * Bone. The best thing however is to just keep the model to one bone.
   *
  ...
  /**
   * Gets the current absolute angles. The absolute angle is calculated by getting the sum of all
   * parent Bones' relative angles plus the current relative angle. This must be called after using
   * the prepareDraw method.
   *
  ...
  /**
   * Gets the current position of the bone. You should call this after all rotations and positions
   * are applied, e.g. after prepareDraw has been called.
   *
  ...
  protected void setAbsoluteRotations() {
    ...
    for (Bone childNode : childNodes) {
      childNode.setAbsoluteRotations(absoluteAngles.angleX, absoluteAngles.angleY,
          absoluteAngles.angleZ);
    }
  }
