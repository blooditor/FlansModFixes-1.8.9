...
public class GuiChooseLoadout extends GuiTeamsBase {
  ...
  public void initGui() {
    ...
    for (int i = 0; i < 5; i++) {
      if (data.currentLevel >= pool.slotUnlockLevels[i]) {
        buttonList.add(
            new GuiButton(i, width / 2 - 128 + 12 + 49 * i, height / 2 - 99 + 117, 36, 20,
                "Select"));
      }
    }

    buttonList.add(
        new GuiButton(5, width / 2 - 128 + 7, height / 2 - 99 + 144, 88, 20, "<< Change Team"));
  }
  ...
  public void drawScreen(int i, int j, float f) {
    ...
    drawModalRectWithCustomSizedTexture(guiOriginX, guiOriginY + 143, 256, 180, 256, 76, textureX,
        textureY);

    // Draw text
    drawCenteredString(fontRendererObj, "Choose a loadout", guiOriginX + 128, guiOriginY + 12,
        0xffffff);
