...
public class GuiDriveableInventory extends GuiContainer {
  ...
  public GuiDriveableInventory(InventoryPlayer inventoryplayer, World world1,
      EntityDriveable entPlane, int i) {
  ...
  protected void drawGuiContainerForegroundLayer(int x, int y) {
    String title = " - Guns";
    if (screen == 1) {
      title = " - " + driveable.getBombInventoryName();
    }
    if (screen == 2) {
      title = driveable.getDriveableType().numFlareSlots > 0 ? " - Flares" : " - Cargo";
    }
    if (screen == 3) {
      title = " - " + driveable.getMissileInventoryName();
    }
    ...
    if (screen == 0) {
      ...
      for (int i = 0; i < driveable.getDriveableType().seats.length; i++) {
        ...
        if (driveable.getDriveableType().seats[i].gunType != null) {
          if (slotsDone >= scroll) {
            fontRendererObj.drawString(driveable.getDriveableType().seats[i].gunName, 53,
                29 + 19 * (slotsDone - scroll), 0x000000);
            drawStack(new ItemStack(driveable.getDriveableType().seats[i].gunType.getItem()), 10,
                25 + 19 * (slotsDone - scroll));
          }
          slotsDone++;
        }
      }
      for (int i = 0; i < driveable.getDriveableType().pilotGuns.size(); i++) {
        ...
        if (driveable.getDriveableType().pilotGuns.get(i).type != null) {
          if (slotsDone >= scroll) {
            fontRendererObj
                .drawString("Driver's gun " + (i + 1), 53, 29 + 19 * (slotsDone - scroll),
                    0x000000);
            drawStack(new ItemStack(driveable.getDriveableType().pilotGuns.get(i).type.getItem()),
                10, 25 + 19 * (slotsDone - scroll));
          }
          slotsDone++;
        }
      }
    ...
    switch (screen) {
      ...
      case 3: {
        ...
        for (int row = 0; row < (m > 3 ? 3 : m); row++) {
          drawTexturedModalRect(j + 9, k + 24 + 19 * row, 7, 97,
              18 * ((row + scroll + 1) * 8 <= numItems ? 8 : numItems % 8), 18);
        }
        break;
      }
    }
  ...
  protected void mouseClicked(int i, int j, int k) throws IOException {
    ...
    if (m > 161 && m < 171 && n > 5 && n < 15) {
      if (driveable instanceof EntityMecha) {
        ...
        (inventory.player)
            .openGui(FlansMod.INSTANCE, 10, world, driveable.chunkCoordX, driveable.chunkCoordY,
                driveable.chunkCoordZ);
      } else {
