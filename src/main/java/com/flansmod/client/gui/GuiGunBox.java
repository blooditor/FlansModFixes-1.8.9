...
public class GuiGunBox extends GuiContainer {
  ...
  public void initGui() {
    ...
    for (int i = 0; i < numCategories; i++) {
      if (pageScroller * numCategories + i < type.pages.size()) {
        categories[i] = new GuiButton(4 + i, width / 2 - numCategories * 30 + i * 60,
            height / 2 - 100, 60, 20, type.pages.get(pageScroller * numCategories + i).name);
        buttonList.add(categories[i]);
      } else {
        categories[i] = new GuiButton(4 + i, width / 2 - numCategories * 30 + i * 60,
            height / 2 - 100, 60, 20, "NONE");
  ...
  protected void drawGuiContainerBackgroundLayer(float f, int i1, int j1) {
    ...
    if (currentPage != null) {
      if (currentEntry != null) {
        ...
        drawModalRectWithCustomSizedTexture(originX + 130, originY + 54, 290, 4, 24, 112, textureX,
            textureY);
        drawModalRectWithCustomSizedTexture(originX + 95, originY + 57 + currentEntryIndex * 22,
            318, 28, 38, 18, textureX, textureY);
        ...
        for (int i = 0; i < 5; i++) {
          ...
          drawModalRectWithCustomSizedTexture(originX + 133, originY + 57 + i * 22, 319, 8, 18, 18,
              textureX, textureY);
        }
        ...
        if (currentSubEntry != null) {
          ...
          drawModalRectWithCustomSizedTexture(originX + 132,
              originY + 55 + currentSubEntryIndex * 22, 327, 48, 29, 22, textureX, textureY);
    ...
    mc.fontRendererObj
        .drawString(type.name, originX + xSize / 2 - stringWidth / 2, originY + 8, 0x00000000);
    mc.fontRendererObj
        .drawString(type.name, originX + xSize / 2 - stringWidth / 2 + 1, originY + 7, 0xffffffff);
  }
  ...
  private void renderPanelBackground(GunBoxEntry entry, int x, int y) {
    ...
    if (numPartsOnLine1 > 0) {
      drawModalRectWithCustomSizedTexture(x + 5, y + 44, 276, 122, 18 + 20 * (numPartsOnLine1 - 1),
          18, textureX, textureY);
    }
    if (numPartsOnLine2 > 0) {
      drawModalRectWithCustomSizedTexture(x + 5, y + 64, 276, 122, 18 + 20 * (numPartsOnLine2 - 1),
          18, textureX, textureY);
    }
  }
  ...
  protected void mouseClicked(int i, int j, int k) throws IOException {
    ...
    if (k == 0 || k == 1) {
      if (currentPage != null) {
        for (int e = 0; e < 5; e++) {
          if (e < currentPage.entries.size() && 105 < m && m < 123 && 57 + e * 22 < n
              && n < 76 + e * 22) {
            currentEntry = currentPage.entries.get(e);
            currentSubEntry =
                currentEntry.childEntries.size() > 0 ? currentEntry.childEntries.get(0) : null;
          }
        }
      }
      ...
      if (currentEntry != null) {
        for (int e = 0; e < 5; e++) {
          if (e < currentEntry.childEntries.size() && 133 < m && m < 151 && 57 + e * 22 < n
              && n < 76 + e * 22) {
