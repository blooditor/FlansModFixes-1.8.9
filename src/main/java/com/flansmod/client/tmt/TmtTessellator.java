...
public class TmtTessellator extends Tessellator {
  ...
  /**
   * Boolean used to check whether quads should be drawn as two triangles. Initialized to false and
   * never changed.
   */
  ...
  /**
   * The number of vertices manually added to the given draw call. This differs from vertexCount
   * because it adds extra vertices when converting quads to triangles.
   */
  ...
  /**
   * The index of the last VBO used. This is used in round-robin fashion, sequentially, through the
   * vboCount vertex buffers.
   */
    ...
    if (!this.isDrawing) {
      ...
      while (offs < vertexCount) {
        ...
        if (TmtTessellator.useVBO) {
          ...
          ARBBufferObject
              .glBufferDataARB(ARBVertexBufferObject.GL_ARRAY_BUFFER_ARB, TmtTessellator.byteBuffer,
  ...
  /**
   * Sets the RGB values as specified, converting from floats between 0 and 1 to integers from
   * 0-255.
   */
  ...
  /**
   * Sets the RGBA values for the color, converting from floats between 0 and 1 to integers from
   * 0-255.
   */
  public void setColorRGBA_F(float par1, float par2, float par3, float par4) {
    this.setColorRGBA((int) (par1 * 255.0F), (int) (par2 * 255.0F), (int) (par3 * 255.0F),
        (int) (par4 * 255.0F));
  }
  ...
  public void addVertexWithUVW(double par1, double par3, double par5, double par7, double par9,
      double par10) {
  ...
  public void addVertex(double par1, double par3, double par5) {
    ...
    this.rawBuffer[this.rawBufferIndex + 1] = Float
        .floatToRawIntBits((float) (par3 + this.yOffset));
    this.rawBuffer[this.rawBufferIndex + 2] = Float
        .floatToRawIntBits((float) (par5 + this.zOffset));
    this.rawBufferIndex += 10;
    ++this.vertexCount;
  }
