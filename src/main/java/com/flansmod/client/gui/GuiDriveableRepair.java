...
public class GuiDriveableRepair extends GuiScreen {
  ...
  public void drawScreen(int i, int j, float f) {
    ...
    //Render the title
    drawString(fontRendererObj, driving.getDriveableType().name + " - Health", guiOriginX + 7,
        guiOriginY + 7, 0xffffff);
    ...
    for (DriveablePart part : partsToDraw) {
      ...
      float percentHealth = Math
          .max((float) part.health / (float) part.maxHealth, 0); //no negative health
      GL11.glColor3f(1 - percentHealth, percentHealth, 0F);
      drawTexturedModalRect(guiOriginX + 121, guiOriginY + y + 2, 0, 73, (int) (70 * percentHealth),
          16);

      //Write the part name and percent health
      drawString(fontRendererObj, part.type.getName(), guiOriginX + 10, guiOriginY + y + 6,
          0xffffff);
      drawCenteredString(fontRendererObj, (int) (percentHealth * 100F) + "%", guiOriginX + 158,
          guiOriginY + y + 6, 0xffffff);
  ...
  protected void mouseClicked(int i, int j, int k) throws IOException {
    ...
    if (m > 185 && m < 195 && n > 5 && n < 15) {
      if (driving instanceof EntityMecha) {
        ...
        (driver).openGui(FlansMod.INSTANCE, 10, driver.worldObj, driving.chunkCoordX,
            driving.chunkCoordY, driving.chunkCoordZ);
      } else {
