...
public class TeamsManager {
  ...
  /**
   * The current round in play. This class replaces the old set of 3 fields "currentGametype",
   * "currentMap" and "teams"
   */
  public TeamsRound currentRound;
  /**
   * This contains a list of all the valid rounds, similar to the old RotationEntry and map
   * rotation
   */
  ...
  public void tick() {
    ...
    if (currentRound != null && roundTimeLeft > 0) {
      ...
      if (autoBalance() && time % autoBalanceInterval == autoBalanceInterval - 200
          && needAutobalance()) {
  ...
  public void autobalance() {
    ...
    if (membersTeamA - membersTeamB > 1) {
      for (int i = 0; i < (membersTeamA - membersTeamB) / 2; i++) {
        ...
        EntityPlayerMP playerToKick = getPlayer(
            currentRound.teams[1].addPlayer(currentRound.teams[0].removeWorstPlayer()));
    ...
    if (membersTeamB - membersTeamA > 1) {
      for (int i = 0; i < (membersTeamB - membersTeamA) / 2; i++) {
        EntityPlayerMP playerToKick = getPlayer(
            currentRound.teams[0].addPlayer(currentRound.teams[1].removeWorstPlayer()));
        messagePlayer(playerToKick, "You were moved to the other team by the autobalancer.");
        sendClassMenuToPlayer(playerToKick);
      }
    }
  ...
  public void pickVoteOptions() {
    ...
    //Wildcard option!
    voteOptions[Gametype.rand.nextInt(voteOptions.length)] = rounds
        .get(Gametype.rand.nextInt(rounds.size()));
  }
  ...
  /**
   * Called at the start of a round. Shows all players the team selection menu. Exludes people on
   * the building / op team
   */
  ...
  public void onPlayerInteract(EntityInteractEvent event) {
    if (event.entityPlayer.inventory.getCurrentItem() != null && event.entityPlayer.inventory
        .getCurrentItem().getItem() instanceof ItemOpStick) {
  ...
  /**
   * Stop damage being taken when it shouldn't N - NoTeam, S - Spectator, 1 - Team 1, 2 - Team 2, O
   * - Other (mobs and world inflicted damage etc)
   * <p>
  ...
  public void onEntityHurt(LivingAttackEvent event) {
    ...
    if (event.entity instanceof EntityPlayerMP) {
      ...
      if (source instanceof EntityDamageSource && source.getEntity() instanceof EntityPlayerMP) {
        ...
        if ((attackerData.team == null && data.team != null) || (attackerData.team != null
            && data.team == null)) {
          event.setCanceled(true);
          return;
        }
        ...
        if (attackerData.team != null && data.team != null) {
          ...
          if (roundTimeLeft > 0 && !currentRound.gametype
              .playerCanAttack(attacker, attackerData.team, player, data.team)) {
            event.setCanceled(true);
          }
        }
      } else {
  ...
  public void playerUseEntity(EntityInteractEvent event) {
    ...
    if (currentItem != null && currentItem.getItem() != null && currentItem
        .getItem() instanceof ItemOpStick) {
      ...
      if (event.target instanceof ITeamObject) {
        currentRound.gametype
            .objectClickedByPlayer((ITeamObject) event.target, (EntityPlayerMP) event.entityPlayer);
      }
      if (event.target instanceof ITeamBase) {
        currentRound.gametype
            .baseClickedByPlayer((ITeamBase) event.target, (EntityPlayerMP) event.entityPlayer);
      }
    }
  }
  ...
  public void playerInteracted(PlayerInteractEvent event) {
    ...
    if (event.action == Action.LEFT_CLICK_BLOCK
        && event.entityPlayer.getCurrentEquippedItem() != null && event.entityPlayer
        .getCurrentEquippedItem().getItem() instanceof ItemGun) {
      event.setCanceled(true);
      return;
    }
    ...
    if (te != null) {
      ...
      if (currentItem != null && currentItem.getItem() != null && currentItem
        ...
        if (te instanceof ITeamObject) {
          ((ItemOpStick) currentItem.getItem())
              .clickedObject(event.entityPlayer.worldObj, (EntityPlayerMP) event.entityPlayer,
                  (ITeamObject) te);
        }
        if (te instanceof ITeamBase) {
          ((ItemOpStick) currentItem.getItem())
              .clickedBase(event.entityPlayer.worldObj, (EntityPlayerMP) event.entityPlayer,
                  (ITeamBase) te);
        }
      } else if (currentRound != null) {
        if (te instanceof ITeamObject) {
          currentRound.gametype
              .objectClickedByPlayer((ITeamObject) te, (EntityPlayerMP) event.entityPlayer);
        }
        if (te instanceof ITeamBase) {
          currentRound.gametype
              .baseClickedByPlayer((ITeamBase) te, (EntityPlayerMP) event.entityPlayer);
        }
      }
    }
  }
  ...
  public void playerDrops(PlayerDropsEvent event) {
        ...
        if (stack != null && stack.getItem() != null) {
          if (stack.getItem() instanceof ItemGun) {
            ...
            for (EntityItem check : dropsToThrow) {
              if (((ItemGun) stack.getItem()).GetType() == ((ItemGun) check.getEntityItem()
                  .getItem()).GetType()) {
                alreadyAdded = true;
              }
            }
    ...
    for (EntityItem entity : event.drops) {
      ...
      if (stack != null && stack.getItem() != null && stack.stackSize > 0) {
        if (stack.getItem() instanceof ItemGun || stack.getItem() instanceof ItemPlane || stack
            .getItem() instanceof ItemVehicle || stack.getItem() instanceof ItemAAGun || stack
            .getItem() instanceof ItemBullet) {
          if (weaponDrops != 1) {
    ...
    if (event.entity instanceof EntityPlayer) {
      ...
      if (enabled && currentRound != null && data != null) {
        if (data.team == Team.spectators || !currentRound.gametype
            .playerCanLoot(itemStack, InfoType.getType(itemStack), event.entityPlayer, data.team)) {
          event.setCanceled(true);
        }
      }
    }
    ...
    if (firstSpawn) {
      ...
      if (spawnPoint != null) {
        ...
        playerMP.playerNetServerHandler
            .setPlayerLocation(spawnPoint.xCoord, spawnPoint.yCoord, spawnPoint.zCoord, 0F, 0F,
                enumset);
  ...
  private void setPlayersNextSpawnpoint(EntityPlayerMP player) {
    ...
    if (spawnPoint != null) {
      setPlayersNextSpawnpoint(player, new BlockPos(MathHelper.floor_double(spawnPoint.xCoord),
          MathHelper.floor_double(spawnPoint.yCoord) + 1,
          MathHelper.floor_double(spawnPoint.zCoord)), 0);
    } else {
  ...
  public void sendClassMenuToPlayer(EntityPlayerMP player) {
    ...
    if (team == null) {
      ...
      sendPacketToPlayer(
          new PacketTeamSelect(team.classes.toArray(new PlayerClass[team.classes.size()])), player);
    }
  }
  ...
  public void playerSelectedClass(EntityPlayerMP player, IPlayerClass playerClass) {
    ...
    if (data.team == data.newTeam && data.playerClass != null && !data.playerClass.GetShortName()
        .equals(playerClass.GetShortName())) {
      currentRound.gametype.playerChoseNewClass(player, playerClass);
      data.newPlayerClass = playerClass;
      player.addChatMessage(
          new ChatComponentText("You will respawn with the " + playerClass.GetName() + " class"));
    }
    //2 : Player switched team
    else if (data.team != null && data.team != data.newTeam) {
      messageAll(
          player.getName() + " switched to \u00a7" + data.newTeam.textColour + data.newTeam.name);
    ...
    //3 : Player has only just joined
      ...
      if (data.newTeam == null) {
        ...
        messageAll(
            player.getName() + " joined \u00a7" + data.newTeam.textColour + data.newTeam.name);
    ...
    for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
      ...
      if (stack != null && stack.getItem() instanceof ItemGun) {
        ((ItemGun) stack.getItem())
            .Reload(stack, player.worldObj, player, player.inventory, false, false, true, false, true);
      }
    }
  }
      ...
      while (it.hasNext()) {
        ...
        if (entity instanceof ITeamBase) {
          ...
          if (((ITeamBase) entity).getBaseID() > nextBaseID) {
            FlansMod.log(
                "Loaded base with ID higher than the supposed highest ID. Adjusted highest ID");
  ...
  private void loadPerWorldData(Event event, World world) {
    ...
    try {
      NBTTagCompound tags = CompressedStreamTools
  ...
  protected void ReadFromNBT(NBTTagCompound tags, World world) {
    ...
    if (maps.size() == 0) {
      maps.put("default" + dimension, new TeamsMap(world, "default" + dimension,
  ...
  public void SelectTeam(Team team) {
    FlansMod.getPacketHandler()
        .sendToServer(new PacketTeamSelect(team == null ? "null" : team.shortName, false));
