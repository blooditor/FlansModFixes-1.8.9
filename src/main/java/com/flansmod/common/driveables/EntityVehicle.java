...
public class EntityVehicle extends EntityDriveable implements IExplodeable {
  ...
  int handbrakeTimer = 0; //timer for reversing
  ...
  public EntityVehicle(World world, double x, double y, double z, VehicleType type,
      DriveableData data) {
  ...
  public EntityVehicle(World world, double x, double y, double z, EntityPlayer placer,
      VehicleType type, DriveableData data) {
    super(world, type, data);
    stepHeight = 1.0F;
    setPosition(x, y, z);
  ...
  public boolean pressKey(int key, EntityPlayer player) {
    ...
    switch (key) {
      case 0: //Accelerate : Increase the throttle, up to 1.
      {
        ...
        float breakSpeed = 0.05f;
        if (throttle < 0) {
          throttle = Math.min(0, throttle += breakSpeed);
          handbrakeTimer = 0;
        } else {
          if (throttle == 0 && handbrakeTimer++ < 20) {

          } else {
            throttle += throttle >= 0 ? 0.02F : 0.1f;
            handbrakeTimer = 0;
            if (throttle > 1F) {
              throttle = 1F;
            }
      ...
      case 1: //Decelerate : Decrease the throttle, down to -1, or 0 if the vehicle cannot reverse
      {
        float breakSpeed = 0.05f;
        if (throttle > 0) {
          throttle = Math.max(0, throttle -= breakSpeed);
          handbrakeTimer = 0;
        } else {
          if (throttle == 0 && handbrakeTimer++ < 20) {
  ...
  public void onUpdate() {
    ...
    if (!worldObj.isRemote && TeamsManager.vehicleLife > 0
        && ticksSinceUsed > TeamsManager.vehicleLife * 20) {
      setDead();
    }
    ...
    for (EntityWheel wheel : wheels) {
      ...
      boolean canThrustCreatively =
          !TeamsManager.vehiclesNeedFuel || (seats != null && seats[0] != null
      ...
      if (canThrustCreatively
          ...
          type.floatOnWater || timeUnderWater < 60)) {
        if (getVehicleType().tank) {
          ...
          if (type.tank) {
            if (left) {
              if (driveableData.parts.get(EnumDriveablePart.leftTrack) != null && !isPartIntact(
                  EnumDriveablePart.leftTrack)) {
                broken = true;
              }
            } else {
              if (driveableData.parts.get(EnumDriveablePart.rightTrack) != null && !isPartIntact(
                  EnumDriveablePart.rightTrack)) {
                broken = true;
              }
            }
          }
          float wYaw = wheelsYaw;

          float turningDrag = 0.02F;
          wheel.motionX *= 1F - (Math.abs(wYaw) * turningDrag);
          wheel.motionZ *= 1F - (Math.abs(wYaw) * turningDrag);
          float tankModifier = 1.9f;
          float velocityScale = 0.04F * (throttle > 0 ? type.maxThrottle * tankModifier
              : type.maxNegativeThrottle * tankModifier) * data.engine.engineSpeed;
          float steeringScale =
              0.1F * (wheelsYaw > 0 ? type.turnLeftModifier : type.turnRightModifier);
          float effectiveWheelSpeed =
              (throttle + (wheelsYaw * (left ? 1 : -1) * steeringScale)) * velocityScale;
          if (broken) {
            effectiveWheelSpeed *= 0.4f;
          }
          ...
          if (wheel.ID == 2 || wheel.ID == 3) {
            float velocityScale =
                0.01F * (wheelsYaw > 0 ? type.turnLeftModifier : type.turnRightModifier) * (
                    throttle > 0 ? 1 : -1);
      ...
      Vector3f targetWheelPos = axes
          .findLocalVectorGlobally(getVehicleType().wheelPositions[wheel.ID].position);
  ...
  private Vec3 crossProduct(Vec3 a, Vec3 b) {
    return new Vec3(a.yCoord * b.zCoord - a.zCoord * b.yCoord,
