...
public class CommandTeams extends CommandBase {
  ...
  public void processCommand(ICommandSender sender, String[] split) throws CommandException {
    ...
    if (split[0].equals("plane")) {
      ...
      PlaneType type = PlaneType.getPlane("f22");
      EntityPlayer p = getCommandSenderAsPlayer(sender);
      NBTTagCompound tags = new NBTTagCompound();
      tags.setString("Engine", "rotaryEngine");
      ...
      EntityFlyByPlane plane = new EntityFlyByPlane(p.worldObj,  - 50, 50, 0,
          type, new DriveableData(tags));
      plane.worldObj.spawnEntityInWorld(plane);
      for (int i = 0; i < plane.driveableData.cargo.length; i++) {
        plane.driveableData.cargo[i] = new ItemStack(InfoType.getType("flareGunAmmo").item, 16);
      }
      plane.motionX = 1;
      return;
    }

    if (split[0].equals("vammo")) {
      EntityDriveable d = ((EntitySeat) getCommandSenderAsPlayer(sender).ridingEntity).driveable;
      for (BulletType t : d.getDriveableType().ammo) {
        getCommandSenderAsPlayer(sender).inventory.addItemStackToInventory(new ItemStack(t.item));
      }
      return;
    }
    ...
    if (split[0].equals("heal")) {
      if (split.length > 1) {
        MinecraftServer.getServer().getConfigurationManager().getPlayerByUsername(split[1])
            .heal(100);
      }
      getCommandSenderAsPlayer(sender).heal(100);
      return;
    }
    if (split[0].equals("repair")) {
      for (int i = 0; i < 4; i++) {
        getCommandSenderAsPlayer(sender).inventory.armorItemInSlot(i).setItemDamage(0);
      }
      return;
    }

    if (split[0].equals("armor")) {
      Team t = Team.getTeam(split[1]);
      if (t == null) {

        getCommandSenderAsPlayer(sender)
            .setCurrentItemOrArmor(4, new ItemStack(InfoType.getType(split[1] + "Helmet").item));
        getCommandSenderAsPlayer(sender)
            .setCurrentItemOrArmor(3, new ItemStack(InfoType.getType(split[1] + "Body").item));
        try {

          getCommandSenderAsPlayer(sender)
              .setCurrentItemOrArmor(2, new ItemStack(InfoType.getType(split[1] + "Legs").item));
        } catch (Exception e) {
          getCommandSenderAsPlayer(sender)
              .setCurrentItemOrArmor(2, new ItemStack(InfoType.getType(split[1] + "Pants").item));
        }
        getCommandSenderAsPlayer(sender)
            .setCurrentItemOrArmor(1, new ItemStack(InfoType.getType(split[1] + "Boots").item));
      } else {

        getCommandSenderAsPlayer(sender).setCurrentItemOrArmor(4, new ItemStack(t.hat.getItem()));
        getCommandSenderAsPlayer(sender).setCurrentItemOrArmor(3, new ItemStack(t.chest.getItem()));
        getCommandSenderAsPlayer(sender).setCurrentItemOrArmor(2, new ItemStack(t.legs.getItem()));
        getCommandSenderAsPlayer(sender).setCurrentItemOrArmor(1, new ItemStack(t.shoes.getItem()));
      }
      return;
    }

    if (split[0].equals("ammo")) {
      for (int i = 0; i < getCommandSenderAsPlayer(sender).inventory.getSizeInventory(); i++) {
        ItemStack st = getCommandSenderAsPlayer(sender).inventory.getStackInSlot(i);
        if (st != null && st.getItem() instanceof ItemGun) {
          getCommandSenderAsPlayer(sender).inventory.addItemStackToInventory(
              new ItemStack(((ItemGun) st.getItem()).type.ammo.get(0).item));
        }
      }
      return;
    }
    if (split[0].equals("renderMode")) {
      FlansMod.ENTITY_RENDER_MODE = Integer.parseInt(split[1]);
      return;
    }
    //On / off
    ...
    if (split[0].equals("listGametypes")) {
      ...
      for (Gametype gametype : Gametype.gametypes.values()) {
        sender.addChatMessage(
            new ChatComponentText("\u00a7f" + gametype.name + " (" + gametype.shortName + ")"));
      }
      return;
    }
		/*
    ...
    if (split[0].equals("listMaps")) {
      ...
      for (TeamsMap map : teamsManager.maps.values()) {
        sender.addChatMessage(new ChatComponentText(
            (teamsManager.currentRound != null && map == teamsManager.currentRound.map ? "\u00a74"
    ...
    if (split[0].equals("addMap")) {
      ...
      sender.addChatMessage(
          new ChatComponentText("Added new map : " + name + " (" + shortName + ")"));
      return;
    }
    ...
    if (split[0].equals("setRound")) {
      if (split.length != 2) {
        sender.addChatMessage(
            new ChatComponentText("You need to specify the round index (see /teams listRounds)"));
        return;
      }
      ...
      if (round != null) {
        ...
        TeamsManager.messageAll(
            "\u00a72Next round will be " + round.gametype.shortName + " in " + round.map.name);
      }
      return;
    }
		/*
    ...
    if (split[0].equals("listTeams") || split[0].equals("listAllTeams")) {
      ...
      for (Team team : Team.teams) {
        sender.addChatMessage(new ChatComponentText(
            "\u00a7" + team.textColour + team.name + " (" + team.shortName + ")"));
      }
      return;
    }
		/*
		...
		*/
    if (split[0].equals("getSticks") || split[0].equals("getOpSticks") || split[0]
        .equals("getOpKit")) {
    ...
    if (split[0].toLowerCase().equals("autobalance")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Autobalance is now " + (TeamsManager.autoBalance ? "enabled" : "disabled")));
      return;
    }
    if (split[0].equals("useRotation")) {
      ...
      sender.addChatMessage(
          new ChatComponentText("Voting is now " + (TeamsManager.voting ? "enabled" : "disabled")));
      return;
    }
    ...
    if (split[0].equals("removeRound") || split[0].equals("removeMapFromRotation") || split[0]
        .equals("removeFromRotation") || split[0].equals("removeRotation")) {
      if (split.length != 2) {
        sender.addChatMessage(
            new ChatComponentText("Incorrect Usage : Should be /teams " + split[0] + " <ID>"));
        return;
      }
      int map = Integer.parseInt(split[1]);
      sender.addChatMessage(new ChatComponentText(
          "Removed map " + map + " (" + TeamsManager.getInstance().rounds.get(map).map.shortName
              + ") from rotation"));
      TeamsManager.getInstance().rounds.remove(map);
      return;
    }
    if (split[0].equals("addMapToRotation") || split[0].equals("addToRotation") || split[0]
        .equals("addRotation") || split[0].equals("addRound")) {
      if (split.length < 7) {
        sender.addChatMessage(new ChatComponentText("Incorrect Usage : Should be /teams " + split[0]
            + " <Map> <Gametype> <Team1> <Team2> ... <TimeLimit> <ScoreLimit>"));
        return;
      }
      ...
      if (split.length != 5 + gametype.numTeamsRequired) {
        sender.addChatMessage(new ChatComponentText("Incorrect Usage : Should be /teams " + split[0]
            + " <Map> <Gametype> <Team1> <Team2> ... <ScoreLimit> <TimeLimit>"));
        return;
      }
      ...
      TeamsManager.getInstance().rounds.add(new TeamsRound(map, gametype, teams,
          Integer.parseInt(split[3 + gametype.numTeamsRequired]),
    ...
    if (split[0].equals("forceAdventure") || split[0].equals("forceAdventureMode")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Adventure mode will " + (TeamsManager.forceAdventureMode ? "now" : "no longer")
              + " be forced"));
      return;
    }
    if (split[0].equals("overrideHunger") || split[0].equals("noHunger")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Players will " + (TeamsManager.overrideHunger ? "no longer" : "now")
    ...
    if (split[0].equals("explosions")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Expolsions are now " + (TeamsManager.explosions ? "enabled" : "disabled")));
      return;
    }
    if (split[0].equals("bombs") || split[0].equals("allowBombs")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Bombs are now " + (TeamsManager.bombsEnabled ? "enabled" : "disabled")));
      return;
    }
    if (split[0].equals("bullets") || split[0].equals("bulletsEnabled")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Bullets are now " + (TeamsManager.bulletsEnabled ? "enabled" : "disabled")));
      return;
    }
    if (split[0].equals("canBreakGuns")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "AAGuns and MGs can " + (TeamsManager.canBreakGuns ? "now" : "no longer")
              + " be broken"));
      return;
    }
    if (split[0].equals("canBreakGlass")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Glass and glowstone can " + (TeamsManager.canBreakGlass ? "now" : "no longer")
              + " be broken"));
      return;
    }
    if (split[0].equals("armourDrops") || split[0].equals("armorDrops")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Armour will " + (TeamsManager.armourDrops ? "now" : "no longer") + " be dropped"));
      return;
    }
    if (split[0].equals("weaponDrops")) {
      if (split.length != 2) {
        sender.addChatMessage(new ChatComponentText(
            "Incorrect Usage : Should be /teams " + split[0] + " <on/off/smart>"));
        return;
      }
    ...
    if (split[0].equals("fuelNeeded")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Vehicles will " + (TeamsManager.vehiclesNeedFuel ? "now" : "no longer")
              + " require fuel"));
      return;
    }
    if (split[0].equals("mgLife")) {
      if (split.length != 2) {
        sender.addChatMessage(
            new ChatComponentText("Incorrect Usage : Should be /teams " + split[0] + " <time>"));
        return;
      }
      TeamsManager.mgLife = Integer.parseInt(split[1]);
      if (TeamsManager.mgLife > 0) {
        sender.addChatMessage(
            new ChatComponentText("MGs will despawn after " + TeamsManager.mgLife + " seconds"));
      } else {
    ...
    if (split[0].equals("planeLife")) {
      ...
      if (TeamsManager.planeLife > 0) {
        sender.addChatMessage(new ChatComponentText(
            "Planes will despawn after " + TeamsManager.planeLife + " seconds"));
      } else {
    ...
    if (split[0].equals("vehicleLife")) {
      ...
      if (TeamsManager.vehicleLife > 0) {
        sender.addChatMessage(new ChatComponentText(
            "Vehicles will despawn after " + TeamsManager.vehicleLife + " seconds"));
      } else {
    ...
    if (split[0].equals("mechaLife")) {
      ...
      if (TeamsManager.mechaLove > 0) {
        sender.addChatMessage(new ChatComponentText(
            "Mechas will despawn after " + TeamsManager.mechaLove + " seconds"));
      } else {
    ...
    if (split[0].equals("aaLife")) {
      ...
      if (TeamsManager.aaLife > 0) {
        sender.addChatMessage(new ChatComponentText(
            "AA Guns will despawn after " + TeamsManager.aaLife + " seconds"));
      } else {
    ...
    if (split[0].equals("vehiclesBreakBlocks")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Vehicles will " + (TeamsManager.driveablesBreakBlocks ? "now" : "no longer")
              + " break blocks"));
      return;
    }
    if (split[0].equals("scoreDisplayTime")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Score summary menu will appear for " + TeamsManager.scoreDisplayTime / 20 + " seconds"));
      return;
    }
    if (split[0].equals("rankUpdateTime")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Rank update menu will appear for " + TeamsManager.rankUpdateTime / 20 + " seconds"));
      return;
    }
    if (split[0].equals("votingTime")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Voting menu will appear for " + TeamsManager.votingTime / 20 + " seconds"));
      return;
    }
    if (split[0].toLowerCase().equals("autobalancetime")) {
      ...
      sender.addChatMessage(new ChatComponentText(
          "Autobalance will now occur every " + TeamsManager.autoBalanceInterval / 20
              + " seconds"));
      return;
    }
    if (split[0].equals("setVariable")) {
      ...
      if (TeamsManager.getInstance().currentRound.gametype.setVariable(split[1], split[2])) {
        sender.addChatMessage(new ChatComponentText(
            "Set variable " + split[1] + " in gametype " + TeamsManager
                .getInstance().currentRound.gametype.shortName + " to " + split[2]));
      } else {
        sender.addChatMessage(new ChatComponentText(
            "Variable " + split[1] + " did not exist in gametype " + TeamsManager
    ...
    if (split[0].toLowerCase().equals("xpmultiplier")) {
      ...
      if (target < 0.5f || target > 2.0f) {
        sender.addChatMessage(
            new ChatComponentText("Not going to allow that for now. Keep it within 0.5 to 2.0"));
      } else {
    ...
    sender.addChatMessage(
        new ChatComponentText(split[0] + " is not a valid teams command. Try /teams help"));
  }

  public void sendHelpInformation(ICommandSender sender, int page) {
    if (page > 3 || page < 1) {
      ChatComponentText text = new ChatComponentText(
          "Invalid help page, should be in the range (1-3)");
    ...
    sender.addChatMessage(
        new ChatComponentText("\u00a72Listing teams commands \u00a7f[Page " + page + " of 3]"));
    switch (page) {
      case 1: {
      ...
      case 2: {
        ...
        sender.addChatMessage(new ChatComponentText(
            "/teams addRound <map> <gametype> <team1> <team2> <TimeLimit> <ScoreLimit>"));
