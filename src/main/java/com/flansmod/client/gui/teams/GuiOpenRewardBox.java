...
public class GuiOpenRewardBox extends GuiTeamsBase {
  ...
  public void updateScreen() {
    ...
    switch (state) {
      ...
      case SLOWING_DOWN: {
        ...
        spinner = target + timeInState * InitialVelocity()
            + 0.5f * Acceleration() * timeInState * timeInState;
  ...
  public void drawScreen(int i, int j, float f) {
    ...
    drawModalRectWithCustomSizedTexture(guiOriginX + 9, guiOriginY + 101, 239 + pixelOffset + 10,
        101, 180, 18, textureX, textureY);
    ...
    for (int n = 0; n < 10; n++) {
      ...
      DrawRarityBackground(paintjob.rarity, guiOriginX + 18 - 18 - pixelOffset + 18 * n,
          guiOriginY + 102);
    }

    mc.renderEngine.bindTexture(texture);
    GlStateManager.disableDepth();
    drawModalRectWithCustomSizedTexture(guiOriginX + 0, guiOriginY + 93, 0, 93, 196, 34, textureX,
        textureY);
    GlStateManager.enableDepth();

    int currentIndex = MathHelper.floor_float(spinner) % options.size();
    ItemStack gunStack = new ItemStack(options.get(currentIndex).parent.item, 1,
        options.get(currentIndex).ID);
    DrawGun(gunStack, guiOriginX + 98, guiOriginY + 65, 60.0f);

    if (state == EnumPageState.STOPPED) {
      drawCenteredString(fontRendererObj, "New paintjob unlocked!", guiOriginX + 98,
          guiOriginY + 130, 0xffffff);
      drawCenteredString(fontRendererObj, options.get(target).parent.name, guiOriginX + 98,
          guiOriginY + 142, 0xffffff);
      drawCenteredString(fontRendererObj, "\"" + options.get(target).iconName + "\"",
          guiOriginX + 98, guiOriginY + 154, 0xffffff);
    }
