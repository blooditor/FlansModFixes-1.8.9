...
public class CommonProxy {
  ...
  /**
   * Returns the list of content pack files, and on the client, adds the content pack resources and
   * models to the classpath
   */
  ...
  public Container getServerGui(int ID, EntityPlayer player, World world, int x, int y, int z) {
    switch (ID) {
      ...
      case 13:
        return new ContainerPaintjobTable(player.inventory, world,
    ...
    for (int n = 0; n < temporaryInventory.getSizeInventory(); n++) {
      ...
      if (stackInSlot != null && stackInSlot.getItem() instanceof ItemPart) {
        ...
        if (partType.category == EnumPartCategory.ENGINE && partType.worksWith
            .contains(EnumType.getFromObject(type))) {
  ...
  public void repairDriveable(EntityPlayer driver, EntityDriveable driving, DriveablePart part) {
    ...
    ArrayList<ItemStack> stacksNeeded = driving.getDriveableType()
        .getItemsRequired(part, driving.getDriveableData().engine);
    ...
    for (ItemStack stackNeeded : stacksNeeded) {
      ...
      for (int m = 0; m < temporaryInventory.getSizeInventory(); m++) {
        ...
        if (stackInSlot != null && stackInSlot.getItem() == stackNeeded.getItem()
          ...
          //Work out the amount to take from the stack
          int amountFound = Math
              .min(stackInSlot.stackSize, stackNeeded.stackSize - totalAmountFound);
          //Take it
          stackInSlot.stackSize -= amountFound;
          //Check for empty stacks
          if (stackInSlot.stackSize <= 0) {
