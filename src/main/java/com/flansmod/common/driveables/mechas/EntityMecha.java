...
public class EntityMecha extends EntityDriveable {
  ...
  public GunAnimations leftAnimations = new GunAnimations(
      null), rightAnimations = new GunAnimations(null);
  ...
  public boolean pressKey(int key, EntityPlayer player) {
    ...
    switch (key) {
      ...
      case 4: //Jump
      {
        ...
        if (onGround && (jumpDelay == 0) && (canThrustCreatively
            || data.fuelInTank > data.engine.fuelConsumption) && isPartIntact(
            EnumDriveablePart.hips)) {
          jumpDelay = 20;
      ...
      case 7: //Inventory
      {
        ...
        ((EntityPlayer) seats[0].riddenByEntity)
            .openGui(FlansMod.INSTANCE, 10, worldObj, chunkCoordX, chunkCoordY, chunkCoordZ);
        return true;
      }
      case 8: //UseR
      {
  ...
  protected boolean creative() {
    return !(seats[0].riddenByEntity instanceof EntityPlayer)
        || ((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode;
  }
  ...
  protected boolean useItem(boolean left) {
    if (left ? isPartIntact(EnumDriveablePart.leftArm) : isPartIntact(EnumDriveablePart.rightArm)) {
      ItemStack heldStack = left ? inventory.getStackInSlot(EnumMechaSlotType.leftTool)
          : inventory.getStackInSlot(EnumMechaSlotType.rightTool);
      if (heldStack == null) {
        return false;
      }
      ...
      if (heldItem instanceof ItemMechaAddon) {
        ...
        Vector3f lookOrigin = new Vector3f((float) mechaType.seats[0].x / 16F,
            (float) mechaType.seats[0].y / 16F + seats[0].riddenByEntity.getMountedYOffset(),
        ...
        Vector3f lookVector = axes.findLocalVectorGlobally(
            seats[0].looking.findLocalVectorGlobally(new Vector3f(reach, 0F, 0F)));
        ...
        MovingObjectPosition hit = worldObj
            .rayTraceBlocks(lookOrigin.toVec3(), lookTarget.toVec3());

        //MovingObjectPosition hit = ((EntityLivingBase)seats[0].riddenByEntity).rayTrace(reach, 1F);
        if (hit != null && hit.typeOfHit == MovingObjectType.BLOCK) {
          BlockPos pos = hit.getBlockPos();
          if (breakingBlock == null || breakingBlock.x != pos.getX() || breakingBlock.y != pos
              .getY() || breakingBlock.z != pos.getZ()) {
            breakingProgress = 0F;
          }
        ...
        if (delay <= 0) {
          ...
          if (bulletStack == null) {
            gunItem.Reload(heldStack, worldObj, this, driveableData, left, true, true,
                (infiniteAmmo() || creative()), true);
          }
  ...
  private void shoot(ItemStack stack, GunType gunType, ItemStack bulletStack, boolean creative,
      boolean left) {
    MechaType mechaType = getMechaType();
    ...
    Vector3f gunVector = new Vector3f(mechaType.armLength + 1.2F * mechaType.heldItemScale,
        0.5F * mechaType.heldItemScale, 0F);
    ...
    if (!worldObj.isRemote) {
      for (int k = 0; k < gunType.numBullets * bulletType.numBullets; k++) {
        ...
        float speed = gunType.getBulletSpeed(bulletType, stack);
        if (speed <= 0.0f) {
          speed = 5.0f;
        }
    ...
    if (left) {
      shootDelayLeft =
          gunType.mode == EnumFireMode.SEMIAUTO ? Math.max(gunType.GetShootDelay(stack), 5)
              : gunType.GetShootDelay(stack);
    } else {
      shootDelayRight =
          gunType.mode == EnumFireMode.SEMIAUTO ? Math.max(gunType.GetShootDelay(stack), 5)
              : gunType.GetShootDelay(stack);
  ...
  public boolean attackEntityFrom(DamageSource damagesource, float i) {
    ...
    if (damagesource.getDamageType().equals("fall")) {
      ...
      float damageToInflict = takeFallDamage ? i * ((type.fallDamageMultiplier * vulnerability()))
          * wouldBeNegativeDamage : 0;
      float blockDamageFromFalling =
          damageBlocksFromFalling ? i * (type.blockDamageFromFalling) / 10F : 0;

      driveableData.parts.get(EnumDriveablePart.hips).attack(damageToInflict, false);
      checkParts();
      FlansMod.getPacketHandler().sendToAllAround(new PacketDriveableDamage(this), posX, posY, posZ,
      ...
      driveableData.parts.get(EnumDriveablePart.core)
          .attack(i * vulnerability(), damagesource.isFireDamage());
    }
    return true;
  }
  ...
  public void onUpdate() {
    ...
    if (toggleTimer == 0 && autoRepair()) {
      for (EnumDriveablePart part : EnumDriveablePart.values()) {
        ...
        boolean hasCreativePlayer =
            seats != null && seats[0] != null && seats[0].riddenByEntity instanceof EntityPlayer
                && ((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode;
        if (thisPart != null && thisPart.health != 0 && thisPart.health < thisPart.maxHealth && (
            hasCreativePlayer || data.fuelInTank >= 10F)) {
          thisPart.health += 1;
    ...
    if (diamondDetect() != null && diamondTimer == 0 && worldObj.isRemote && seats[0] != null
        && seats[0].riddenByEntity instanceof EntityPlayer && FlansMod.proxy
        .isThePlayer((EntityPlayer) seats[0].riddenByEntity)) {
      ...
      for (float i = -30; i <= 30; i++) {
        for (float j = -30; j <= 30; j++) {
          for (float k = -30; k <= 30; k++) {
            ...
            if (i * i + j * j + k * k < sqDistance
                && worldObj.getBlockState(new BlockPos(x, y, z)).getBlock()
                == (Blocks.diamond_ore)) {
      ...
      if (sqDistance < 901) {
        PacketPlaySound.sendSoundPacket(posX, posY, posZ, FlansMod.soundRange, dimension,
            diamondDetect().detectSound, false);
    ...
    boolean driverIsLiving =
        seats[0] != null && seats[0].riddenByEntity instanceof EntityLivingBase;
    ...
    if (!worldObj.isRemote && TeamsManager.mechaLove > 0
        && ticksSinceUsed > TeamsManager.mechaLove * 20) {
      setDead();
    }
    ...
    if (seats[0] != null) {
      ...
      //}
      if (seats[0].riddenByEntity instanceof EntityLivingBase
    ...
    if (!onGround && thePlayerIsDrivingThis && Minecraft
        .getMinecraft().currentScreen instanceof GuiDriveableController && FlansMod.proxy
        .isKeyDown(4) && shouldFly() && (
        ((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode || data.fuelInTank >= (
            10F * jetPack))) {
      motionY *= 0.95;
    ...
    if (driverIsLiving) {
      ...
      if (thePlayerIsDrivingThis && Minecraft
      ...
      } else if (seats[0].riddenByEntity instanceof EntityLiving
          && !(seats[0].riddenByEntity instanceof EntityPlayer)) {
      ...
      if (Math.abs(intent.lengthSquared()) > 0.1) {
        ...
        if ((canThrustCreatively || data.fuelInTank > data.engine.fuelConsumption) && isPartIntact(
            EnumDriveablePart.hips)) {
          if (!onGround && shouldFly() && (canThrustCreatively
              || data.fuelInTank > 10F * jetPack + data.engine.fuelConsumption)) {
      ...
      if (!worldObj.isRemote) {
        ...
        if (breakingBlock != null) {
          ...
          IBlockState state = worldObj
              .getBlockState(new BlockPos(breakingBlock.x, breakingBlock.y, breakingBlock.z));
          ...
          boolean leftStackIsTool =
              leftStack != null && leftStack.getItem() instanceof ItemMechaAddon;
          boolean rightStackIsTool =
              rightStack != null && rightStack.getItem() instanceof ItemMechaAddon;
          boolean breakingBlocks =
              (leftMouseHeld && leftStackIsTool) || (rightMouseHeld && rightStackIsTool);
          ...
          if (blockHit == null || !breakingBlocks) {
            ...
            float blockHardness = blockHit.getBlockHardness(worldObj,
                new BlockPos(breakingBlock.x, breakingBlock.y, breakingBlock.z));
            ...
            if (leftStackIsTool) {
              ...
              if (leftType.function.effectiveAgainst(material)
                  && leftType.toolHardness > blockHardness) {
            ...
            if (rightStackIsTool) {
              ...
              if (rightType.function.effectiveAgainst(material)
                  && rightType.toolHardness > blockHardness) {
            ...
            //If this block's hardness is zero-ish, then the tool's power is OVER 9000!!!!
              ...
              mineSpeed /= blockHit.getBlockHardness(worldObj,
                  new BlockPos(breakingBlock.x, breakingBlock.y, breakingBlock.z));
            }
            ...
            if (breakingProgress >= 1F) {
              ...
              if (entity instanceof EntityPlayerMP) {
                int eventOutcome = ForgeHooks.onBlockBreakEvent(worldObj,
                    ((EntityPlayerMP) entity).capabilities.isCreativeMode ? GameType.CREATIVE
                        : ((EntityPlayerMP) entity).capabilities.allowEdit ? GameType.SURVIVAL
                            : GameType.ADVENTURE, (EntityPlayerMP) entity,
                    new BlockPos(breakingBlock.x, breakingBlock.y, breakingBlock.z));
              ...
              if (!cancelled) {
                ...
                if (vacuumItems) {
                  for (ItemStack stack : blockHit.getDrops(worldObj,
                      new BlockPos(breakingBlock.x, breakingBlock.y, breakingBlock.z), state, 0)) {
                    //Check for iron regarding refining
                    boolean fuelCheck = (data.fuelInTank >= 5F
                        || ((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode);
                    if (fuelCheck && refineIron() && stack.getItem() instanceof ItemBlock
                        && ((ItemBlock) stack.getItem()).block == Blocks.iron_ore) {
                    ...
                    if (fuelCheck && wasteCompact() && stack.getItem() instanceof ItemBlock && (
                        ((ItemBlock) stack.getItem()).block == Blocks.cobblestone
                            || ((ItemBlock) stack.getItem()).block == Blocks.dirt
                            || ((ItemBlock) stack.getItem()).block == Blocks.sand)) {
                      stack.stackSize = 0;
                      if (!((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode) {
                    ...
                    fuelCheck = (data.fuelInTank >= 3F * diamondMultiplier()
                        || ((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode);
                    if (fuelCheck && stack.getItem() == Items.diamond) {
                      ...
                      stack.stackSize *= MathHelper.floor_float(multiplier) + (
                          rand.nextFloat() < tailFloat(multiplier) ? 1 : 0);
                      if (!((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode) {
                        data.fuelInTank -= 3F * diamondMultiplier();
                      }
                    }
                    fuelCheck = (data.fuelInTank >= 2F * redstoneMultiplier()
                        || ((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode);
                    ...
                    fuelCheck = (data.fuelInTank >= 2F * coalMultiplier()
                        || ((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode);
                    ...
                    fuelCheck = (data.fuelInTank >= 2F * emeraldMultiplier()
                        || ((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode);
                    ...
                    fuelCheck = (data.fuelInTank >= 2F * ironMultiplier()
                        || ((EntityPlayer) seats[0].riddenByEntity).capabilities.isCreativeMode);
                    ...
                    if (autoCoal() && (stack.getItem() == Items.coal) && (data.fuelInTank + 250F
                        < type.fuelTankSize)) {
                    ...
                    if (!InventoryHelper
                        ...
                        && !worldObj.isRemote && worldObj.getGameRules()
                        .getBoolean("doTileDrops")) {
                      worldObj.spawnEntityInWorld(
                          new EntityItem(worldObj, breakingBlock.x + 0.5F, breakingBlock.y + 0.5F,
                              breakingBlock.z + 0.5F, stack));
                    }
                  }
                }
                //Destroy block
                worldObj
                    .destroyBlock(new BlockPos(breakingBlock.x, breakingBlock.y, breakingBlock.z),
                        atLeastOneEffectiveTool && !vacuumItems);
              }
            }
          }
        }
    ...
    if (!worldObj.isRemote && ticksExisted % 5 == 0) {
      FlansMod.getPacketHandler().sendToAllAround(new PacketMechaControl(this), posX, posY, posZ,
          FlansMod.driveableUpdateRange, dimension);
    }
    ...
    if (part.type == EnumDriveablePart.core) {
      for (int i = 0; i < inventory.getSizeInventory(); i++) {
        if (inventory.getStackInSlot(i) != null) {
          worldObj.spawnEntityInWorld(
              new EntityItem(worldObj, posX + midpoint.x, posY + midpoint.y, posZ + midpoint.z,
