...
public class BlockSulphuricAcid extends BlockFluidClassic {
  ...
  public static final PropertyFloat HEIGHT_NW = new PropertyFloat("height_nw", 0F,
      1F), HEIGHT_SW = new PropertyFloat("height_sw", 0F, 1F),
      HEIGHT_SE = new PropertyFloat("height_se", 0F, 1F), HEIGHT_NE = new PropertyFloat("height_ne",
      0F, 1F);
  ...
  protected BlockState createBlockState() {
    return new ExtendedBlockState(this, new IProperty[]{LEVEL},
        new IUnlistedProperty[]{HEIGHT_SW, HEIGHT_NW, HEIGHT_SE, HEIGHT_NE, FLOW_DIRECTION,
            LEVEL_CORNERS[0], LEVEL_CORNERS[1], LEVEL_CORNERS[2], LEVEL_CORNERS[3]});
  }
  ...
  public IBlockState getExtendedState(IBlockState state, IBlockAccess world, BlockPos pos) {
    ...
    if (flow11 != 1) {
      ...
      heightNW = getFluidHeightAverage(flow00, flow01, flow10, flow11);
      heightSW = getFluidHeightAverage(flow01, flow02, flow12, flow11);
      heightSE = getFluidHeightAverage(flow12, flow21, flow22, flow11);
      heightNE = getFluidHeightAverage(flow10, flow20, flow21, flow11);
    } else {
      heightNW = flow11;
      heightSW = flow11;
      heightSE = flow11;
    ...
    extState = extState
        .withProperty(FLOW_DIRECTION, (float) BlockFluidBase.getFlowDirection(world, pos));
  ...
  public float getFluidHeightForRender(IBlockAccess world, BlockPos pos) {
    ...
    if (state.getBlock() == this) {
      ...
      if (state.getValue(LEVEL) == this.getMaxRenderHeightMeta()) {
        return 0.875F;
      }
    }
    return !state.getBlock().getMaterial().isSolid() && verticalOrigin == this ? 1
        : this.getQuantaPercentage(world, pos) * 0.875F;
  }
