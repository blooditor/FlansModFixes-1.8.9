...
public class EntitySurvivor extends EntityFlansModShooter {
  ...
  public SurvivorInventory inventory = new SurvivorInventory(this);
  ...
  public EntitySurvivor(World world) {
    ...
    targetTasks
        .addTask(4, new EntityAINearestAttackableTarget(this, EntityFlansModShooter.class, true));
  ...
  private void initGunList() {
    ...
    for (GunType gun : GunType.gunList) {
      if (gun.mode == EnumFireMode.SEMIAUTO && !gun.deployable && gun.ammo.size() > 0 && !gun.shield
          && gun.usableByPlayers && gun.dungeonChance != 0) {
        validGuns.add(gun);
      }
    }
  }
  ...
  protected void dropFewItems(boolean p_70628_1_, int p_70628_2_) {
    ...
    for (int i = 0; i < numFoods; i++) {
      switch (rand.nextInt(5)) {
        ...
        case 1:
          ...
          break;
        case 2:
          dropItem(Items.mushroom_stew, 1);
          break;
        case 3:
    ...
    if (rand.nextBoolean()) {
      worldObj.spawnEntityInWorld(new EntityItem(worldObj, posX, posY, posZ,
          FlansModApocalypse.getLootGenerator().getSurvivorJournal(rand)));
    }
  }

  public EnumGunHeldState getGunHeldState() {
    return gunHeldState;
  }

  public static enum EnumGunHeldState{
    PATROL, DOWN, LOW, READY, AIM
  }
}
