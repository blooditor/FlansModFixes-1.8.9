...
public class GuiDriveableMenu extends GuiContainer {
  ...
  public void initGui() {
    ...
    //Cargo button
    GuiButton cargoButton = new GuiButton(0, width / 2 - 60, height / 2 - 71, 58, 20,
        type.numFlareSlots > 0 ? "Flares" : "Cargo");
    //cargoButton.enabled = type.numCargoSlots > 0;
    cargoButton.enabled = type.numCargoSlots > 0 || type.numFlareSlots > 0;
    ...
    GuiButton missileButton = new GuiButton(3, width / 2 + 2, height / 2 - 49, 58, 20,
        entity.getMissileInventoryName());
    missileButton.enabled = type.numMissileSlots > 0;
    buttonList.add(missileButton);

    //Mine / Bomb Button
    GuiButton bombButton = new GuiButton(5, width / 2 + 2, height / 2 - 27, 58, 20,
        entity.getBombInventoryName());
    bombButton.enabled = type.numBombSlots > 0;
    buttonList.add(bombButton);

    //Repair button
    buttonList.add(new GuiButton(4, width / 2 - 60, height / 2 - 27, 58, 20, "Health"));
  }
  ...
  protected void actionPerformed(GuiButton button) {
    ...
    if (button.id == 4) //Repair
    {
      ...
      inventory.player.openGui(FlansMod.INSTANCE, 1, world, entity.chunkCoordX, entity.chunkCoordY,
          entity.chunkCoordZ);
    }
