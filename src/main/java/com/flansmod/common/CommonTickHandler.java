...
public class CommonTickHandler {
  ...
  /**
   * List for storing replacement EntityItemCustomRenderers. Stops concurrent modifications and
   * messing up the entity list.
   */
  ...
  public void tick(TickEvent.ServerTickEvent event) {
    switch (event.phase) {
      case START: {
        ...
        try {
          MinecraftServer.getServer().worldServerForDimension(0).rainingStrength = 0;
        } catch (Exception e) {
  ...
  public void onEntitySpawn(EntityJoinWorldEvent event) {
    ...
    if (event.entity instanceof EntityItem && !(event.entity instanceof EntityItemCustomRender)) {
      ...
      if (stack != null && stack.getItem() instanceof ItemGun
          && ((ItemGun) stack.getItem()).GetType().modelString != null) {
