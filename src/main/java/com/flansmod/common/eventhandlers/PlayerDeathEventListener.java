...
public class PlayerDeathEventListener {
  ...
  public void PlayerDied(LivingDeathEvent DamageEvent) {
    if ((DamageEvent.source.getDamageType().equalsIgnoreCase("explosion") && (
        (DamageEvent.source.getSourceOfDamage() instanceof EntityGrenade) || (DamageEvent.source
            .getSourceOfDamage() instanceof EntityBullet)))
      ...
      if (DamageEvent.entityLiving instanceof EntityPlayer && !isGrenade) {
        FlansMod.getPacketHandler().sendToDimension(new PacketKillMessage(false,
                ((EntityBullet) DamageEvent.source.getSourceOfDamage()).type,
                (killedTeam == null ? "f" : killedTeam.textColour) + DamageEvent.entity
                    .getDisplayName().getFormattedText(),
                (killerTeam == null ? "f" : killedTeam.textColour) + DamageEvent.source
                    .getSourceOfDamage().getDisplayName().getFormattedText()),
            DamageEvent.entityLiving.dimension);
      }
      if (DamageEvent.entityLiving instanceof EntityPlayer && isGrenade) {
        FlansMod.getPacketHandler().sendToDimension(new PacketKillMessage(false,
                ((EntityGrenade) DamageEvent.source.getSourceOfDamage()).type,
                (killedTeam == null ? "f" : killedTeam.textColour) + DamageEvent.entity
                    .getDisplayName().getFormattedText(),
                (killerTeam == null ? "f" : killedTeam.textColour) + DamageEvent.source
                    .getSourceOfDamage().getDisplayName().getFormattedText()),
