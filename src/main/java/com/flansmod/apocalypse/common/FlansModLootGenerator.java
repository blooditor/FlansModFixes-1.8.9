...
public class FlansModLootGenerator {
    ...
    for (DriveableType type : DriveableType.types) {
      if (type instanceof VehicleType) {
        ...
        if (((PlaneType) type).mode == EnumPlaneMode.PLANE) {
          ...
          helicopters.add((PlaneType) type);
        }
  ...
  public void fillVillageChest(Random rand, TileEntityChest chest) {
    ...
    for (int i = 0; i < numParts; i++) {
      ...
      chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
          new ItemStack(part.item, 1));
    }

    //Add 1~5 random ammo
    for (int i = 0; i < numAmmo; i++) {
      ShootableType type = ShootableType.shootables.get(
          new ArrayList<Integer>(ShootableType.shootables.keySet())
              .get(rand.nextInt(ShootableType.shootables.size())));
      if (type != null && type.dungeonChance != 0) {
        chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
            new ItemStack(type.item, 1 + (type.maxStackSize > 1 && rand.nextBoolean() ? 1 : 0)));
      }
    }
    ...
    for (int i = 0; i < numFuel; i++) {
      ...
      chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
          new ItemStack(fuel.item, rand.nextInt(Math.min(fuel.stackSize - 1, 2)) + 1));
    }
    ...
    for (int i = 0; i < numFood; i++) {
      switch (rand.nextInt(4)) {
        case 0:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(Items.chicken, rand.nextInt(2) + 1));
          break;
        case 1:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(Items.porkchop, rand.nextInt(2) + 1));
          break;
        case 2:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(Items.beef, rand.nextInt(2) + 1));
          break;
        case 3:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(Items.baked_potato, rand.nextInt(3) + 1));
          break;
      }
    }
    ...
    if (rand.nextBoolean() && rand.nextBoolean()) {
      chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
          new ItemStack(MechaItemType.types.get(rand.nextInt(MechaItemType.types.size())).item));
    }

    //Add 0~1 tools
    if (rand.nextBoolean()) {
      chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()), new ItemStack(
          ToolType.tools.get(new ArrayList<String>(ToolType.tools.keySet())
              .get(rand.nextInt(ToolType.tools.size()))).item));
    }
  }
  ...
  public Block getRandomWeaponBox(Random rand) {
    if (rand.nextInt(4) == 0) {
      //Get armour box
      return ArmourBoxType.boxes.get(new ArrayList<String>(ArmourBoxType.boxes.keySet())
          .get(rand.nextInt(ArmourBoxType.boxes.size()))).block;
    } else {
      //Get weapon box
      return GunBoxType.gunBoxMap.get(new ArrayList<String>(GunBoxType.gunBoxMap.keySet())
          .get(rand.nextInt(GunBoxType.gunBoxMap.size()))).block;
    }
  }
  ...
  public PartType getRandomEngine(DriveableType type, Random rand) {
    switch (EnumType.getFromObject(type)) {
      case vehicle:
        return vehicleEngines.size() > 0 ? vehicleEngines.get(rand.nextInt(vehicleEngines.size()))
            : null;
      case plane:
  ...
  public void fillBrewingStand(Random rand, TileEntityBrewingStand tileentity) {
    for (int i = 0; i < 3; i++) {
      if (rand.nextBoolean()) {
        tileentity.setInventorySlotContents(i,
            new ItemStack(Items.potionitem, 1, potions[rand.nextInt(9)]));
      }
    }
  }
  ...
  public void fillLiquidLabChest(Random rand, TileEntityChest chest) {
    ...
    for (int i = 0; i < numItems; i++) {
      switch (rand.nextInt(10)) {
        case 0:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(Items.bowl, rand.nextInt(5) + 1));
          break;
        case 1:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
        ...
        case 2:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(Items.bucket, rand.nextInt(2) + 1));
          break;
        case 3:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
        ...
        case 7:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(FlansModApocalypse.sulphur, rand.nextInt(12) + 1));
          break;
        case 8:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              getScientistJournal(rand));
          break;
        case 9:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              getScientistJournal(rand));
  ...
  private ItemStack getRandomAttachment(Random rand) {
    AttachmentType type = AttachmentType.attachments
        .get(rand.nextInt(AttachmentType.attachments.size()));
  ...
  public void fillDyeFactoryChest(TileEntityChest chest, Random rand) {
    ...
    for (int i = 0; i < numDyes; i++) {
      chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
          new ItemStack(Items.dye, rand.nextInt(8) + 1, rand.nextInt(16)));
    }
    ...
    for (int i = 0; i < numMisc; i++) {
      switch (rand.nextInt(4)) {
        case 0:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(Items.string, rand.nextInt(5) + 1));
          break;
        case 1:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(Items.feather, rand.nextInt(5) + 1));
          break;
        case 2:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(Items.leather, rand.nextInt(8) + 1));
          break;
        case 3:
          chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()),
              new ItemStack(Items.clay_ball, rand.nextInt(32) + 1));
          break;
      }
    }
  }
  ...
  public ItemStack getScientistJournal(Random rand) {
    ...
    switch (rand.nextInt(8)) {
      ...
      case 5:
        ...
        pages.appendTag(new NBTTagString(
            "We actually did it! I cannot believe it, but we sent someone back in time! Admittedly, they ended up walking with Creepersauruses, but nonetheless, we did it!"));
        break;
      case 6:
        stack.setTagInfo("title", new NBTTagString("Research Journal: Entry 7"));
        pages.appendTag(new NBTTagString(
            "They're here! The mechas are here! If you read this, please, go back in time, destroy the creator, stop th..."));
        break;
      case 7:
  ...
  public ItemStack getSurvivorJournal(Random rand) {
    ...
    switch (rand.nextInt(1)) {
      case 0:
        stack.setTagInfo("author", new NBTTagString("Flan"));
        break;
    }
    NBTTagList pages = new NBTTagList();
    ...
    switch (rand.nextInt(8)) {
      case 0:
        ...
        pages.appendTag(new NBTTagString(
            "I have no food. My child has no food. We are going to die. Why did this have to happen?"));
        break;
      case 1:
        stack.setTagInfo("title", new NBTTagString("The Endtimes"));
        pages.appendTag(new NBTTagString(
            "It's amazing how fast your world can be torn down around you. Just three days ago, I was happily trading emeralds at the village market. Now all that is gone. I am left to wander this wasteland alone. I don't know how long I'll last, or how long I'll stay sane..."));
        break;
      case 2:
        stack.setTagInfo("title", new NBTTagString("Day 5"));
        pages.appendTag(new NBTTagString(
            "We found water today! At the bottom of a village well. We drank and bathed and filled our bottles and left. But for reference, the village was at- *bloodstains*"));
        break;
      case 3:
        stack.setTagInfo("title", new NBTTagString("Day 7"));
        pages.appendTag(new NBTTagString(
            "They got my brother! Just after we left the village, he was snatched by some sort of... robot... Also, I think the water may have been contaminated. I've been sweating an awful lot, and it's not just the heat."));
        break;
      case 4:
        stack.setTagInfo("title", new NBTTagString("Day 10"));
        pages.appendTag(new NBTTagString(
            "I have been violently ill, but have not found a new water source yet. I may have to drink more contaminated water to stay alive. I couldn't get worse, could I?"));
        break;
      case 5:
        stack.setTagInfo("title", new NBTTagString("The Wasteland"));
        pages.appendTag(new NBTTagString(
            "This world is harsh and unforgiving. I've had to make difficult choices, but they are necessary in order to survive. If I hadn't pulled the trigger, they would have done so instead. I'm sure of it."));
        break;
      case 6:
        stack.setTagInfo("title", new NBTTagString(""));
        pages.appendTag(new NBTTagString(
            "We spotted an airstrip in the distance! We're going to head over there under cover of darkness and see if we can acquire ourselves a plane. Let's get out of this terrible place."));
        break;
      case 7:
        stack.setTagInfo("title", new NBTTagString("Time Portal: Instruction Manual"));
        stack.setTagInfo("generation", new NBTTagInt(3));
        pages.appendTag(new NBTTagString(
            "The Time Portal uses the portal properties of obsidian combined with-"));
        pages.appendTag(new NBTTagString(
            "Beware! The mechas are coming! The time portal is of great importance! You must-"));
        break;
    }
