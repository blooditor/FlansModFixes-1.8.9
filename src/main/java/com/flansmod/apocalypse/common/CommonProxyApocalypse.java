...
public class CommonProxyApocalypse {
  ...
  public void init(FMLInitializationEvent event) {
    EntityRegistry.registerGlobalEntityID(EntitySurvivor.class, "Survivor",
        EntityRegistry.findGlobalUniqueEntityId(), 0x588225, 0x875C37);
    EntityRegistry
        .registerModEntity(EntitySurvivor.class, "Survivor", 117, FlansModApocalypse.INSTANCE, 250,
            20, false);

    EntityRegistry.registerGlobalEntityID(EntityTeleporter.class, "Teleporter",
        EntityRegistry.findGlobalUniqueEntityId());
    EntityRegistry
        .registerModEntity(EntityTeleporter.class, "Teleporter", 113, FlansModApocalypse.INSTANCE,
            100, 20, true);

    EntityRegistry.registerGlobalEntityID(EntityAIMecha.class, "AIMecha",
        EntityRegistry.findGlobalUniqueEntityId());
    EntityRegistry
        .registerModEntity(EntityAIMecha.class, "AIMecha", 114, FlansModApocalypse.INSTANCE, 250,
            20, false);

    EntityRegistry.registerGlobalEntityID(EntityFakePlayer.class, "FakePlayer",
        EntityRegistry.findGlobalUniqueEntityId());
    EntityRegistry
        .registerModEntity(EntityFakePlayer.class, "FakePlayer", 115, FlansModApocalypse.INSTANCE,
            250, 2, false);

    EntityRegistry.registerGlobalEntityID(EntityNukeDrop.class, "NukeDrop",
        EntityRegistry.findGlobalUniqueEntityId());
    EntityRegistry
        .registerModEntity(EntityNukeDrop.class, "NukeDrop", 116, FlansModApocalypse.INSTANCE, 250,
            20, false);
    ...
    if (event.phase == TickEvent.Phase.START) {
      ...
      if (getApocalypseCountdown() > 0) {
        if (apocalypseMecha == null || apocalypseMecha.isDead()) {
          ...
          apocalypseMecha.seats[0].looking
              .rotateGlobalYaw(apocalypseMecha.worldObj.rand.nextFloat() * 10F);
          apocalypseMecha.seats[0].looking
              .rotateGlobalPitch((float) apocalypseMecha.worldObj.rand.nextGaussian() * 3F);
          ...
          if (getApocalypseCountdown() % 20 == 0) {
            ...
            world.spawnEntityInWorld(
                new EntityNukeDrop(world, apocalypseMecha.posX + world.rand.nextGaussian() * range,
                    256, apocalypseMecha.posZ + world.rand.nextGaussian() * range));
          }
            ...
            switch (FlansModApocalypse.OPTION) {
              case DIM:
                for (int i = 0; i < placer.worldObj.playerEntities.size(); i++) {
                  if (placer.worldObj.playerEntities.get(i).dimension == 0) {
                    sendPlayerToApocalypse(placer.worldObj.playerEntities.get(i));
                  }
                }
                break;
              case DIM_OPT_IN:
              ...
              case NEARBY:
                for (Object player : placer.worldObj.playerEntities) {
                  if (((Entity) player).dimension == 0
                      && ((Entity) player).getDistanceToEntity(placer) < 50) {
      ...
      WorldServer world = MinecraftServer.getServer()
          .worldServerForDimension(FlansModApocalypse.dimensionID);
      if (world != null) {
        for (int i = 0; i < world.playerEntities.size(); i++) {
          ...
          if (world.rand.nextInt(5000) == 0) {
            ...
            tags.setString("Engine",
                FlansModApocalypse.getLootGenerator().getRandomEngine(type, world.rand).shortName);
            ...
            EntityFlyByPlane plane = new EntityFlyByPlane(world, player.posX + dX, 120,
                player.posZ + dZ, type, data);
          ...
          if (world.rand.nextInt(FlansModApocalypse.WANDERING_SURVIVOR_RARITY) == 0
              && !world.provider.isDaytime()) {
            ...
            survivor.setPosition(player.posX + dX,
                world.getTopSolidOrLiquidBlock(new BlockPos(player.posX + dX, 0, player.posZ + dZ))
                    .getY() + 1D, player.posZ + dZ);
  ...
  private void sendPlayerToApocalypse(EntityPlayer player) {
    ...
    data.entryPoints.put(player.getPersistentID(),
        new BlockPos(apocalypseMecha.posX, apocalypseMecha.posY, apocalypseMecha.posZ));
    BlockPos exitPoint = new BlockPos(apocalypseMecha.posX, 128, apocalypseMecha.posZ);
    for (; MinecraftServer.getServer().worldServerForDimension(FlansModApocalypse.dimensionID)
        .isAirBlock(exitPoint); exitPoint = exitPoint.down()) {
    }
    MinecraftServer.getServer().getConfigurationManager()
        .transferPlayerToDimension((EntityPlayerMP) player, FlansModApocalypse.dimensionID,
            new TeleporterApocalypse(
                MinecraftServer.getServer().worldServerForDimension(FlansModApocalypse.dimensionID),
                exitPoint.add(0, 1, 0)));
  ...
  public void itemPlaced(EntityJoinWorldEvent event) {
    if (!event.world.isRemote && event.entity instanceof EntityMecha
      ...
      if (engine.isAIChip) {
        ...
        FlansMod.getPacketHandler().sendTo(new PacketApocalypseCountdown(getApocalypseCountdown()),
            (EntityPlayerMP) mecha.placer);
      }
    }
  }
  ...
  public void playerDied(LivingDeathEvent event) {
    if (event.entityLiving.dimension == FlansModApocalypse.dimensionID
  ...
  public void playerRespawned(PlayerRespawnEvent event) {
    if (event.player.dimension == FlansModApocalypse.dimensionID) {
      ...
      if (pos != null) {
        ...
        pos = pos.add((int) (Math.cos(angle) * FlansModApocalypse.SPAWN_RADIUS), 128 - pos.getY(),
            (int) (Math.sin(angle) * FlansModApocalypse.SPAWN_RADIUS));
        for (; event.player.worldObj.isAirBlock(pos); pos = pos.down()) {

        }
        ((EntityPlayerMP) event.player).playerNetServerHandler
            .setPlayerLocation(pos.getX() + 0.5D, pos.getY() + 1.5D, pos.getZ() + 0.5D, 0F, 0F,
                enumset);
