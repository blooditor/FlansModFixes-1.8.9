...
public class DriveablePart {
  ...
  public DriveableHit rayTrace(EntityDriveable driveable, Vector3f origin, Vector3f motion) {
    ...
    if (motion.x != 0F) {
      if (origin.x < box.x) //Check face x = o.x
      {
        ...
        if (intersectY >= box.y && intersectY <= box.y + box.h && intersectZ >= box.z
            && intersectZ <= box.z + box.d) {
          return new DriveableHit(driveable, type, intersectTime,
              new Vector3f(box.x - origin.x, intersectY, intersectZ));
        }
      } else if (origin.x > box.x + box.w) //Check face x = o.x + d.x
      {
        ...
        if (intersectY >= box.y && intersectY <= box.y + box.h && intersectZ >= box.z
            && intersectZ <= box.z + box.d) {
          return new DriveableHit(driveable, type, intersectTime,
              new Vector3f(box.x + box.w - origin.x, intersectY, intersectZ));
        }
      }
    }
    ...
    if (motion.z != 0F) {
      if (origin.z < box.z) //Check face z = box.z
      {
        ...
        float intersectY = origin.y + motion.y * intersectTime;
        if (intersectX >= box.x && intersectX <= box.x + box.w && intersectY >= box.y
            && intersectY <= box.y + box.h) {
          return new DriveableHit(driveable, type, intersectTime,
              new Vector3f(intersectX, intersectY, box.z - origin.z));
        }
      } else if (origin.z > box.z + box.d) //Check face z = box.z + box.d
      {
        ...
        if (intersectX >= box.x && intersectX <= box.x + box.w && intersectY >= box.y
            && intersectY <= box.y + box.h) {
          return new DriveableHit(driveable, type, intersectTime,
              new Vector3f(intersectX, intersectY, box.z + box.d - origin.z));
        }
      }
    }
    ...
    if (motion.y != 0F) {
      if (origin.y < box.y) //Check face y = o.y
      {
        ...
        float intersectX = origin.x + motion.x * intersectTime;
        float intersectZ = origin.z + motion.z * intersectTime;
        if (intersectX >= box.x && intersectX <= box.x + box.w && intersectZ >= box.z
            && intersectZ <= box.z + box.d) {
          return new DriveableHit(driveable, type, intersectTime,
              new Vector3f(intersectX, box.y - origin.y, intersectZ));
        }
      } else if (origin.y > box.y + box.h) //Check face x = box.y + box.h
      {
        ...
        if (intersectX >= box.x && intersectX <= box.x + box.w && intersectZ >= box.z
            && intersectZ <= box.z + box.d) {
          return new DriveableHit(driveable, type, intersectTime,
              new Vector3f(intersectX, box.y + box.h - origin.y, intersectZ));
        }
      }
    }
   ...
   */
  public float hitByBullet(BulletType type, float damage, boolean dealDamage) {
  ...
  /**
   * Ray traces a single co-ordinate But only returns true once upon entering the box, and not while
   * in or exiting
   *
  ...
  /**
   * Ray traces a single co-ordinate Returns true if the ray is in the bounds at some point along
   * its length
   *
   * @param start The start of the ray in this co-ordinate
   * @param end   The end of the ray in this co-ordinate
