...
public class EntityGrenade extends EntityShootable implements IEntityAdditionalSpawnData {
  ...
  public EntityGrenade(World w, GrenadeType g, EntityLivingBase t) {
    ...
    if (thrower instanceof EntityPlayer
        && PlayerHandler.getPlayerData((EntityPlayer) thrower) != null) {
    ...
    if (type.throwSound != null) {
      PacketPlaySound
          .sendSoundPacket(posX, posY, posZ, FlansMod.soundRange, dimension, type.throwSound, true);
    }
  }
  ...
  public void onUpdate() {
    ...
    if (type == null || (type.despawnTime > 0 && ticksExisted > type.despawnTime)) {
      if (!(worldObj.isRemote && type != null && type.isDeployableBag)) {
        detonated = true;
        setDead();
        return;
      }
    ...
    if (worldObj.isRemote) {
      if (type.trailParticles) {
        ...
        for (int i = 0; i < 10; i++) {
          EntityFX particle = FlansModClient
              .getParticle(type.trailParticleType, worldObj, prevPosX + dX * i, prevPosY + dY * i,
                  prevPosZ + dZ * i);
    ...
    if (smoking) {
      ...
      FlansMod.getPacketHandler()
          .sendToAllAround(new PacketFlak(posX, posY, posZ, 50, type.smokeParticleType), posX, posY,
              posZ, 30, dimension);
      //
      List list = worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
          getEntityBoundingBox().expand(type.smokeRadius, type.smokeRadius, type.smokeRadius));
      for (Object obj : list) {
    ...
    if (!worldObj.isRemote) {
      ...
      if (type.livingProximityTrigger > 0 || type.driveableProximityTrigger > 0) {
        ...
        List list = worldObj.getEntitiesWithinAABBExcludingEntity(this,
            getEntityBoundingBox().expand(checkRadius, checkRadius, checkRadius));
        for (Object obj : list) {
          ...
          if (obj instanceof EntityLivingBase
            ...
            if (TeamsManager.getInstance() != null
                && TeamsManager.getInstance().currentRound != null && obj instanceof EntityPlayerMP
                && thrower instanceof EntityPlayer) {
              if (!TeamsManager.getInstance().currentRound.gametype
                  .playerAttacked((EntityPlayerMP) obj,
                      new EntityDamageSourceGun(type.shortName, this, (EntityPlayer) thrower, type,
                          false))) {
                continue;
              }
            }
    ...
    if (!stuck && !type.stickToThrower) {
      ...
      if (hit != null && hit.typeOfHit == MovingObjectType.BLOCK) {
        ...
        //The grenade cannot bounce if it detonated on impact, so hence the "else" condition
          ...
          float lambda = Math.abs(motVec.lengthSquared()) < 0.00000001F ? 1F
              : postHitMotVec.length() / motVec.length();
          ...
          Vector3f.add(angularVelocity,
              new Vector3f(rand.nextGaussian() * randomSpinner, rand.nextGaussian() * randomSpinner,
                  rand.nextGaussian() * randomSpinner), angularVelocity);
    ...
    if (type.damageVsLiving > 0 && !stuck) {
      ...
      for (Object obj : list) {
        ...
        if (obj instanceof EntityLivingBase) {
          ((EntityLivingBase) obj).attackEntityFrom(getGrenadeDamage(),
              type.damageVsLiving * motVec.lengthSquared() * 3);
        }
        if (type.explodeOnImpact) {
          detonate();
        }
  ...
  public void detonate() {
    ...
    if (!worldObj.isRemote) {
      PacketPlaySound.sendAdvancedSound(this, type.detonateSound, 12);
    }
    //Explode
    if (!worldObj.isRemote && type.explosionRadius > 0.1F) {
      if ((thrower instanceof EntityPlayer)) {
        new FlansModExplosion(worldObj, this, (EntityPlayer) thrower, type, posX, posY, posZ,
            type.explosionRadius, type.fireRadius > 0, type.smokeRadius > 0,
            type.explosionBreaksBlocks, type.explosionDamageVsLiving,
            type.explosionDamageVsDriveable, type.explosionArmorPenetration);
      } else {
        ...
        for (float j = -type.fireRadius; j < type.fireRadius; j++) {
          for (float k = -type.fireRadius; k < type.fireRadius; k++) {
            ...
            if (i * i + j * j + k * k <= type.fireRadius * type.fireRadius
                && worldObj.getBlockState(new BlockPos(x, y, z)).getBlock() == Blocks.air && rand
                .nextBoolean()) {
							/*
    ...
    if (worldObj.isRemote) {
      for (int i = 0; i < type.explodeParticles; i++) {
        worldObj.spawnParticle(FlansModClient.getParticleType(type.explodeParticleType), posX, posY,
            posZ, rand.nextGaussian(), rand.nextGaussian(), rand.nextGaussian());
      }
    }
  ...
  private DamageSource getGrenadeDamage() {
    if (thrower instanceof EntityPlayer) {
      return (new EntityDamageSourceGun(type.shortName, this, (EntityPlayer) thrower, type, false))
          .setProjectile();
    } else {
  ...
  public boolean interactFirst(EntityPlayer player) {
    ...
    if (type.isDeployableBag && !worldObj.isRemote) {
      ...
      if (type.healAmount > 0 && player.getHealth() < player.getMaxHealth()) {
        ...
        FlansMod.getPacketHandler()
            .sendToAllAround(new PacketFlak(player.posX, player.posY, player.posZ, 5, "heart"),
                new NetworkRegistry.TargetPoint(player.dimension, player.posX, player.posY,
                    player.posZ, 50F));
        used = true;
      }
      ...
      //Handle ammo
      if (type.numClips > 0 && player.getCurrentEquippedItem() != null && player
        ...
        if (gun.ammo.size() > 0) {
          ...
          int numToGive = Math
              .min(bulletToGive.maxStackSize, type.numClips * gun.numAmmoItemsInGun);
          if (player.inventory
              .addItemStackToInventory(new ItemStack(bulletToGive.item, numToGive))) {
            used = true;
          }
        }
      }
