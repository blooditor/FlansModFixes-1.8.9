...
public class GuiLandingPage extends GuiTeamsBase {
  ...
  public void initGui() {
    ...
    for (int i = 0; i < 5; i++) {
      if (data.currentLevel >= pool.slotUnlockLevels[i]) {
        buttonList.add(
            new GuiButton(i, width / 2 - WIDTH / 2 + 12 + 49 * i, height / 2 - HEIGHT / 2 + 117, 36,
                20, "Edit"));
      }
    }

    buttonList.add(
        new GuiButton(5, width / 2 - WIDTH / 2 + 202, height / 2 - HEIGHT / 2 + 162, 47, 20,
            "Play >>"));

    for (int i = 0; i < 3; i++) {
      int numBoxes = data.GetNumOfUnopenedBoxes(pool.rewardBoxes[i]);

      GuiButton button = new GuiButton(6 + i, width / 2 - WIDTH / 2 + 9 + 65 * i,
          height / 2 - HEIGHT / 2 + 187, 59, 20, "Open");
  ...
  public void drawScreen(int i, int j, float f) {
    ...
    drawModalRectWithCustomSizedTexture(guiOriginX + 106, guiOriginY + 146, 259, 164,
        (int) (92.0f * XPProgress), 16, textureX, textureY);

    // Draw text
    drawCenteredString(fontRendererObj, ClientTeamsData.motd, guiOriginX + 128, guiOriginY + 12,
        0xffffff);

    drawString(fontRendererObj, mc.thePlayer.getName(), guiOriginX + 30, guiOriginY + 150,
        0xffffff);
    drawCenteredString(fontRendererObj, "Rank " + data.currentLevel, guiOriginX + 154,
        guiOriginY + 150, 0xffffff);
  ...
  private void DrawRewardBoxPanel(LoadoutPool pool, PlayerRankData data, int x, int y, int index) {
    ...
    drawCenteredString(fontRendererObj, "x " + data.GetNumOfUnopenedBoxes(box), x + 33, y + 7,
        0xffffff);
  }
}
