    ...
    if (classMenu) {
      for (int i = 0; i < classChoices.length; i++) {
        if (classChoices[i] != null) {
          buttonList.add(
              new GuiButton(i, width / 2 - 128 + 9, height / 2 - guiHeight / 2 + 24 + 24 * i, 73,
                  20, classChoices[i].name));
        }
      }
    } else {
      if (teamChoices == null) {
      ...
      for (int i = 0; i < teamChoices.length; i++) {
        if (teamChoices[i] != null) {
          buttonList.add(
              new GuiButton(i, width / 2 - 128 + 10, height / 2 - guiHeight / 2 + 24 + 24 * i, 236,
                  20, "\u00a7" + teamChoices[i].textColour + teamChoices[i].name));
        } else {
          buttonList.add(
              new GuiButton(i, width / 2 - 128 + 10, height / 2 - guiHeight / 2 + 24 + 24 * i, 236,
                  20, "No Team / Builder"));
  ...
  public void drawScreen(int i, int j, float f) {
    ...
    if (classMenu) {
      for (int n = 0; n < classChoices.length; n++) {
        drawTexturedModalRect(width / 2 - 128, height / 2 - guiHeight / 2 + 22 + 24 * n, 0, 23, 256,
            24);
      }
    } else {
      for (int n = 0; n < teamChoices.length; n++) {
        drawTexturedModalRect(width / 2 - 128, height / 2 - guiHeight / 2 + 22 + 24 * n, 0, 48, 256,
            24);
      }
    }
    fontRendererObj
        .drawStringWithShadow(classMenu ? "Choose a Class" : "Choose a Team", width / 2 - 120,
            height / 2 - guiHeight / 2 + 8, 0xffffff);

    super.drawScreen(i, j, f);
    if (classMenu) {
      for (int n = 0; n < classChoices.length; n++) {
        for (int m = 0; m < classChoices[n].startingItems.size(); m++) {
          drawSlotInventory(classChoices[n].startingItems.get(m), width / 2 - 128 + 85 + 18 * m,
              height / 2 - guiHeight / 2 + 26 + 24 * n);
        }
      }
    }
  }
  ...
  protected void actionPerformed(GuiButton button) {
    if (classMenu) {
      FlansMod.getPacketHandler()
          .sendToServer(new PacketTeamSelect(classChoices[button.id].shortName, true));
    ...
    if (i == 1 || i == mc.gameSettings.keyBindInventory.getKeyCode()) {
      ...
      if (classMenu) {
        if (classChoices != null && classChoices.length > 0) {
          FlansMod.getPacketHandler()
              .sendToServer(new PacketTeamSelect(classChoices[0].shortName, true));
        }
      } else {
        FlansMod.getPacketHandler()
