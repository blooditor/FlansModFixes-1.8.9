...
public class ModelPlane extends ModelDriveable {
  ...
  public void render(DriveableType type) {
    ...
    for (ModelRendererTurbo[] heliMainRotorModel : heliMainRotorModels) {
      renderPart(heliMainRotorModel);
    }
    for (ModelRendererTurbo[] heliTailRotorModel : heliTailRotorModels) {
  ...
  public void render(float f5, EntityPlane plane, float f) {
    ...
    for (Propeller propeller : plane.getPlaneType().propellers) {
      if (plane.isPartIntact(propeller.planePart) && propellerModels.length > propeller.ID) {
        ...
        for (int j = 0; j < numParts; j++) {
          propellerModels[propeller.ID][j].rotateAngleX =
              angle + (j * 2F * 3.1415926535F) / (numParts);
          propellerModels[propeller.ID][j].render(f5);
        }
      }
    }
    Vector3f color = plane.driveableData.color3f;
    ...
    for (EntitySeat seat : plane.seats) {
      ...
      if (seat != null && seat.seatInfo != null && seat.seatInfo.gunName != null
          && gunModels.get(seat.seatInfo.gunName) != null && plane
          .isPartIntact(seat.seatInfo.part)) {
        float yaw =
            seat.prevLooking.getYaw() + (seat.looking.getYaw() - seat.prevLooking.getYaw()) * f;
        float pitch = seat.prevLooking.getPitch()
            + (seat.looking.getPitch() - seat.prevLooking.getPitch()) * f;
  ...
  public void renderRotor(EntityPlane plane, float f5, int i) {
    ...
    if (i >= type.heliPropellers.size() || plane
        .isPartIntact(type.heliPropellers.get(i).planePart)) {
  ...
  public void renderTailRotor(EntityPlane plane, float f5, int i) {
    PlaneType type = plane.getPlaneType();
    //If its not covered by the plane type heli propellers, render it. Otherwise, see if the part is intact
    if (i >= type.heliTailPropellers.size() || plane
        .isPartIntact(type.heliTailPropellers.get(i).planePart)) {
