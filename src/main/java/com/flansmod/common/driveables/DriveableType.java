...
public abstract class DriveableType extends PaintableType {
  ...
  /**
   * The list of bullet types that can be used in this driveable for the main gun (tank shells,
   * plane bombs etc)
   */
  ...
  /**
   * If true, then this vehicle harvests blocks from the harvester hitbox and places them in the
   * inventory
   */
   ...
   * Inventory sizes
   */
  public int numFlareSlots, numCargoSlots, numBombSlots, numMissileSlots;
  /**
   * The fuel tank size
   */
   ...
   *
   */
  public float turretRotationSpeed = 2.5F * AIM_SPEED_MULTIPLIER;
  ...
  public void read(String[] split, TypeFile file) {
    ...
    try {
      ...
      //Movement Variables
        ...
        turretOrigin = new Vector3f(Float.parseFloat(split[1]) / 16F,
            Float.parseFloat(split[2]) / 16F, Float.parseFloat(split[3]) / 16F);
      ...
      if (split[0].equals("TurretRotationSpeed")) {
        turretRotationSpeed = Float.parseFloat(split[1]) * AIM_SPEED_MULTIPLIER;
      ...
      //Wheels
        ...
        wheelPositions[Integer.parseInt(split[1])] = new DriveablePosition(
            new Vector3f(Float.parseFloat(split[2]) / 16F, Float.parseFloat(split[3]) / 16F,
      ...
      //Harvesting
        ...
        } else if (split[1].equals("Spade") || split[1].equals("Shovel") || split[1]
            .equals("Excavator")) {
      ...
      //Cargo / Payload
      else if (split[0].equals("FlareSlots")) {
        numFlareSlots = numCargoSlots = Integer.parseInt(split[1]);
      } else if (split[0].equals("CargoSlots")) {
        numCargoSlots = numFlareSlots > 0 ? numFlareSlots : Integer.parseInt(split[1]);
      ...
      //Backwards compatibility stuff
        ...
        shootPointsPrimary.add(new DriveablePosition(
            new Vector3f(Float.parseFloat(split[1]) / 16F, Float.parseFloat(split[2]) / 16F,
                Float.parseFloat(split[3]) / 16F), EnumDriveablePart.core));
      } else if (split[0].equals("BarrelPosition")) {
        primary = EnumWeaponType.SHELL;
        shootPointsPrimary.add(new DriveablePosition(
            new Vector3f(Float.parseFloat(split[1]) / 16F, Float.parseFloat(split[2]) / 16F,
                Float.parseFloat(split[3]) / 16F), EnumDriveablePart.turret));
      ...
      //Health
        ...
        EnumDriveablePart part = EnumDriveablePart.getPart(split[1]);
        CollisionBox box = new CollisionBox(Integer.parseInt(split[2]), Integer.parseInt(split[3]),
      ...
      //Driver Position
        ...
        seats[0].rotatedOffset = new Vector3f(Integer.parseInt(split[1]) / 16F,
            Integer.parseInt(split[2]) / 16F, Integer.parseInt(split[3]) / 16F);
      } else if (split[0].equals("DriverAimSpeed")) {
        seats[0].aimingSpeed = new Vector3f(Float.parseFloat(split[1]) * AIM_SPEED_MULTIPLIER,
            Float.parseFloat(split[2]) * AIM_SPEED_MULTIPLIER, Float.parseFloat(split[3])
            * AIM_SPEED_MULTIPLIER);
      } else if (split[0].equals("RotatedPassengerOffset")) {
        seats[Integer.parseInt(split[1])].rotatedOffset = new Vector3f(Integer.parseInt(split[2])
      ...
      //Passengers / Gunner Seats
        ...
        seats[Integer.parseInt(split[1])].gunOrigin = new Vector3f(Float.parseFloat(split[2]) / 16F,
      ...
      //Sound
        ...
        FlansMod.proxy.loadSound(contentPack, "driveables", split[1]);
      } else if (split[0].equals("ShootMainSound") || split[0].equals("ShootSoundPrimary")
          || split[0].equals("ShellSound") || split[0].equals("BombSound")) {
      ...
      } else if (split[0].equalsIgnoreCase("AddParticle") || split[0]
          .equalsIgnoreCase("AddEmitter")) {
  ...
  public abstract EntityDriveable createDriveable(World world, double x, double y, double z,
      DriveableData data);
  ...
  /**
   * Find the items needed to rebuild a part. The returned array is disconnected from the template
   * items it has looked up
   */
