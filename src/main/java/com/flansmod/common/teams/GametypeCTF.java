...
public class GametypeCTF extends Gametype {
  ...
  public void playerKilled(EntityPlayerMP player, DamageSource source) {
    ...
    if (player.riddenByEntity instanceof EntityFlag) {
      Team flagTeam = teamsManager
          .getTeam(((EntityFlag) player.riddenByEntity).getBase().getOwnerID());
      player.riddenByEntity.mountEntity(null);
      TeamsManager.messageAll(
          "\u00a7f" + player.getName() + " dropped the \u00a7" + flagTeam.textColour + flagTeam.name
              + "\u00a7f flag");
    }
  }
    ...
    if (object instanceof EntityFlag) {
      ...
      if (flag.getBase().getOwnerID() > 1) {
        ...
        if (playerTeam != null && playerTeam != Team.spectators
            && flag.getBase().getMap() == teamsManager.currentRound.map) {
          ...
          if (playerTeam == flagTeam) {
            ...
            if (flag.ridingEntity == null && !flag.isHome) {
              ...
              playerData.score += 2;
              TeamsManager.messageAll(
                  "\u00a7f" + player.getName() + " returned the \u00a7" + flagTeam.textColour
            ...
            //They have another flag in hand
              ...
              if (otherFlagTeam != null && otherFlagTeam != Team.spectators
                ...
                otherFlag.reset();
                TeamsManager.messageAll(
                    "\u00a7f" + player.getName() + " captured the \u00a7" + otherFlagTeam.textColour
                        + otherFlagTeam.name + "\u00a7f flag");
              }
            }
          }
          //If the player is clicking the other team flag
            ...
            if (flag.ridingEntity == player) {
              ...
              TeamsManager.messageAll(
                  "\u00a7f" + player.getName() + " dropped the \u00a7" + flagTeam.textColour
                      + flagTeam.name + "\u00a7f flag");
              ...
              TeamsManager.messageAll(
                  "\u00a7f" + player.getName() + " picked up the \u00a7" + flagTeam.textColour
                      + flagTeam.name + "\u00a7f flag");
              flag.isHome = false;
            }
  ...
  public Vec3 getSpawnPoint(EntityPlayerMP player) {
    ...
    ArrayList<ITeamBase> bases = teamsManager.currentRound.map
        .getBasesPerTeam(teamsManager.currentRound.getTeamID(data.newTeam));
