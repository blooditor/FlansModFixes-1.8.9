...
public class ModelAAGun extends ModelBase {
  ...
  public void renderBase(float f, float f1, float f2, float f3, float f4, float f5,
      EntityAAGun aa) {
  ...
  public void renderGun(float f, float f1, float f2, float f3, float f4, float f5, EntityAAGun aa) {
    ...
    for (ModelRendererTurbo aGunModel : gunModel) {
      ...
      aGunModel.rotateAngleZ =
          -aa.gunPitch / 180F * 3.141592653589793238462643383279502884197169399F;
      aGunModel.render(f5);
    }

    for (ModelRendererTurbo aGunsightModel : gunsightModel) {
      aGunsightModel.rotateAngleZ =
          -aa.gunPitch / 180F * 3.141592653589793238462643383279502884197169399F;
    ...
    for (int i = 0; i < barrelModel.length; i++) {
      for (int j = 0; j < barrelModel[i].length; j++) {
        barrelModel[i][j].setPosition(
            -aa.barrelRecoil[i] * (float) (Math.cos(-aa.gunPitch * 3.14159265358979F / 180F))
                + barrelX,
            -aa.barrelRecoil[i] * (float) (Math.sin(-aa.gunPitch * 3.14159265358979F / 180F))
                + barrelY, barrelZ);
        barrelModel[i][j].rotateAngleZ =
            -aa.gunPitch / 180F * 3.141592653589793238462643383279502884197169399F;
  ...
  public void flipAll() {
    for (ModelRendererTurbo aBaseModel : baseModel) {
      ...
      aBaseModel.setRotationPoint(aBaseModel.rotationPointX, -aBaseModel.rotationPointY,
          -aBaseModel.rotationPointZ);
    }
    for (ModelRendererTurbo aSeatModel : seatModel) {
      aSeatModel.doMirror(false, true, true);
      aSeatModel.setRotationPoint(aSeatModel.rotationPointX, -aSeatModel.rotationPointY,
          -aSeatModel.rotationPointZ);
    }
    for (ModelRendererTurbo aGunModel : gunModel) {
      aGunModel.doMirror(false, true, true);
      aGunModel.setRotationPoint(aGunModel.rotationPointX, -aGunModel.rotationPointY,
          -aGunModel.rotationPointZ);
    }
    for (ModelRendererTurbo aGunsightModel : gunsightModel) {
      aGunsightModel.doMirror(false, true, true);
      aGunsightModel.setRotationPoint(aGunsightModel.rotationPointX, -aGunsightModel.rotationPointY,
          -aGunsightModel.rotationPointZ);
    }
    for (ModelRendererTurbo[] aBarrelModel : barrelModel) {
      for (int j = 0; j < aBarrelModel.length; j++) {
        ...
        aBarrelModel[j]
            .setRotationPoint(aBarrelModel[j].rotationPointX, -aBarrelModel[j].rotationPointY,
                -aBarrelModel[j].rotationPointZ);
      }
    }
    for (ModelRendererTurbo[] anAmmoModel : ammoModel) {
      for (int j = 0; j < anAmmoModel.length; j++) {
        ...
        anAmmoModel[j]
            .setRotationPoint(anAmmoModel[j].rotationPointX, -anAmmoModel[j].rotationPointY,
                -anAmmoModel[j].rotationPointZ);
      }
    }
  }
