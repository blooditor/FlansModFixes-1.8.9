...
import com.flansmod.apocalypse.common.entity.EntitySurvivor.EnumGunHeldState;
...
public class EntityFlansModShooter extends EntityMob implements IRangedAttackMob {
  ...
  EnumGunHeldState gunHeldState = EnumGunHeldState.PATROL;
  ...
  public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData data) {
    ...
    this.setCanPickUpLoot(
        this.rand.nextFloat() < 0.55F * difficulty.getClampedAdditionalDifficulty());
    ...
    if (stack != null && stack.getItem() instanceof ItemGun) {
      ...
      if (type.useLoopingSounds && loopedSoundDelay <= 0 && minigunSpeed > 0.1F && !reloading) {
        ...
        PacketPlaySound.sendSoundPacket(posX, posY, posZ, FlansMod.soundRange, dimension,
            shouldPlayWarmupSound ? type.warmupSound : type.loopedSound, false);
   ...
   */
  public boolean reload(ItemStack gunStack, GunType gunType, World world, Entity entity,
    ...
    for (int i = 0; i < gunType.numAmmoItemsInGun; i++) {
      ...
      if (bulletStack == null || bulletStack.getItemDamage() == bulletStack.getMaxDamage()
        ...
        for (int j = 0; j < ammoStacks.length; j++) {
          ...
          if (searchingStack != null && searchingStack.getItem() instanceof ItemShootable && gunType
              .isAmmo(((ItemShootable) (searchingStack.getItem())).type)) {
  ...
  private void shoot(ItemStack stack, GunType gunType, World world, ItemStack bulletStack,
      Entity entity, boolean left, EntityLivingBase target) {
    ItemGun item = (ItemGun) gunType.item;
    ShootableType bullet = ((ItemShootable) bulletStack.getItem()).type;
    // Play a sound if the previous sound has finished
    if (!world.isRemote && soundDelay <= 0 && gunType.shootSound != null) {
      AttachmentType barrel = gunType.getBarrel(stack);
      boolean silenced = gunType.silenced || barrel != null && barrel.silencer;
      ...
      PacketPlaySound.sendAdvancedSound(this, gunType, bullet, silenced, true);
    ...
    if (!world.isRemote) {
      ...
      for (int k = 0; k < gunType.numBullets * bullet.numBullets; k++) {
        ...
        Vector3f direction = new Vector3f(target.posX - posX,
            (target.posY + target.getEyeHeight()) - (posY + getEyeHeight()), target.posZ - posZ)
            .normalise(null);
        Vector3f.add(direction, new Vector3f(rand.nextFloat() * direction.x * inaccuracy,
            rand.nextFloat() * direction.y * inaccuracy,
            rand.nextFloat() * direction.z * inaccuracy), direction);
        ...
        float speed = gunType.getBulletSpeed(bullet, stack);

        if (shootableItem.type != null) {
  ...
  public void readAiState(int type) {
    switch (type) {
      case 0:
        gunHeldState = EnumGunHeldState.PATROL;
        break;
      case 1:
        gunHeldState = EnumGunHeldState.LOW;
        break;
      case 2:
      case 3:
        gunHeldState = EnumGunHeldState.AIM;
        break;
    }
  }
  @Override
