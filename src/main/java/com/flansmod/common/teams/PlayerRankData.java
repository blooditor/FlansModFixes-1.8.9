...
public class PlayerRankData {
  ...
  public void readFromBuf(ByteBuf data) {
    ...
    for (int i = 0; i < numRewardBoxes; i++) {
      rewardBoxData
          .add(RewardBoxInstance.CreateClientRewardBoxInstance(data.readInt(), data.readInt()));
    }
  }
  ...
  public void readFromNBT(NBTTagCompound tags) {
    ...
    if (rewardTags != null) {
      for (int i = 0; i < rewardTags.tagCount(); i++) {
        ...
        rewardBoxData
            .add(RewardBoxInstance.CreateRewardBoxInstanceFromNBT(boxHash, unlockHash, type));
      }
    }
  }
