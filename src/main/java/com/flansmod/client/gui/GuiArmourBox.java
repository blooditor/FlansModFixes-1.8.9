  ...
  public void drawScreen(int i, int j, float f) {
    ...
    for (int row = 0; row < 3; row++) {
      for (int col = 0; col < 9; col++) {
        drawSlotInventory(inventory.getStackInSlot(col + (row + 1) * 9), m + 8 + col * 18,
            n + 100 + row * 18);
      }
    }
    for (int col = 0; col < 9; col++) {
  ...
  private void drawRecipe(FontRenderer fontrenderer, int m, int n, int q) {
    ...
    if (page != null) {
      ...
      for (int i = 0; i < 2; i++) {
        //Iterate over y
        for (int j = 0; j < 2; j++) {
          drawSlotInventory(new ItemStack(page.armours[i * 2 + j].item), m + 9 + 83 * i,
              n + 44 + 22 * j);
          ...
          for (int p = 0; p < (numParts < 3 ? numParts : 3); p++) {
            drawSlotInventory(page.requiredStacks[i * 2 + j].get(startPart + p),
  ...
  protected void mouseClicked(int i, int j, int k) {
    ...
    if (k == 0 || k == 1) {
      ...
      for (int x = 0; x < 2; x++) {
        //Iterate over y
        for (int y = 0; y < 2; y++) {
          if (type.pages.get(page).armours[x * 2 + y] != null && m > 7 + 83 * x && m < 27 + 83 * x
              && n > 42 + 22 * y && n < 62 + 22 * y) {
