...
public class InventoryGunModTable extends InventoryBasic {
  ...
  public void markDirty() {
    ...
    if (gunStack != lastGunStack) {
      ...
      setInventorySlotContents(1,
          ItemStack.loadItemStackFromNBT(attachmentTags.getCompoundTag("barrel")));
      setInventorySlotContents(2,
          ItemStack.loadItemStackFromNBT(attachmentTags.getCompoundTag("scope")));
      setInventorySlotContents(3,
          ItemStack.loadItemStackFromNBT(attachmentTags.getCompoundTag("stock")));
      setInventorySlotContents(4,
          ItemStack.loadItemStackFromNBT(attachmentTags.getCompoundTag("grip")));
      genericScroll = 0;
      for (int i = 0; i < Math.min(gunType.numGenericAttachmentSlots, 8); i++) {
        setInventorySlotContents(5 + i,
            ItemStack.loadItemStackFromNBT(attachmentTags.getCompoundTag("generic_" + i)));
      }
      busy = false;
    }
    //Else we changed an attachment
      ...
      for (int i = 0; i < gunType.numGenericAttachmentSlots; i++) {
        if (i >= genericScroll * 4 && i < genericScroll * 4 + 8) {
          writeAttachmentTags(attachmentTags, getStackInSlot(i - genericScroll * 4 + 5),
              "generic_" + i);
        } else {
          attachmentTags
              .setTag("generic_" + i, getStackInSlot(0).getTagCompound().getTag("generic_" + i));
        }
      }
  ...
  public void writeAttachmentTags(NBTTagCompound attachmentTags, ItemStack attachmentStack,
