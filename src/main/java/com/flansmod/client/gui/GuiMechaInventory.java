...
public class GuiMechaInventory extends GuiContainer {
  ...
  protected void drawGuiContainerBackgroundLayer(float f, int i1, int j1) {
    ...
    for (int row = 0; row < (numRows > 3 ? 3 : numRows); row++) {
      drawTexturedModalRect(j + 185, k + 24 + 19 * row, 181, 97,
    ...
    if (fuelInTank > 0) {
      drawTexturedModalRect(width / 2 - 18,
          height / 2 + 45 - (int) ((94 * fuelInTank) / fuelTankSize), 350, 20, 15,
          (int) ((94 * fuelInTank) / fuelTankSize));
    }
  ...
  public void drawTexturedModalRect(int par1, int par2, int par3, int par4, int par5, int par6) {
    ...
    worldrenderer.addVertexWithUV(par1, par2 + par6, this.zLevel,
        (float) (par3) * f, (float) (par4 + par6) * f1);
    worldrenderer
        .addVertexWithUV(par1 + par5, par2 + par6, this.zLevel,
            (float) (par3 + par5) * f, (float) (par4 + par6) * f1);
    worldrenderer.addVertexWithUV(par1 + par5, par2, this.zLevel,
        (float) (par3 + par5) * f, (float) (par4) * f1);
    worldrenderer.addVertexWithUV(par1, par2, this.zLevel,
        (float) (par3) * f, (float) (par4) * f1);
    tessellator.draw();
  }
  ...
  public void initGui() {
    ...
    buttonList.add(new GuiButton(1, width / 2 - 68, height / 2 + 63, 68, 20, "Health"));
  }

  @Override
  protected void actionPerformed(GuiButton button) {
    if (button.id == 0) {
      inventory.player.openGui(FlansMod.INSTANCE, 6, world, mecha.chunkCoordX, mecha.chunkCoordY,
          mecha.chunkCoordZ);
    }
    if (button.id == 1) {
      inventory.player.openGui(FlansMod.INSTANCE, 1, world, mecha.chunkCoordX, mecha.chunkCoordY,
          mecha.chunkCoordZ);
    }
  }
