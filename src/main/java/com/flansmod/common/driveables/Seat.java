...
public class Seat {
  ...
  /**
   * The x, y and z positions of the seat within the plane in model co-ordinates x is forwards, y is
   * up and z is left
   */
  public int x, y, z;
  /**
  ...
  public int id;
  /**
   * Limits for the look vector of the seat. Range is -360 to 360. Thus any range should lie in here
   * without having to wrap
   */
  public float minYaw = -360F, maxYaw = 360F;
  /**
   * Limits for the look vector of the seat. Range is -90 to 90, but don't go beyond +/-89 or the
   * view will mess up at the poles
   */
  public float minPitch = -89F, maxPitch = 89F;
  /**
   * The gun this seat requires. As of 1.6.2, seats and planes will require specific guns as opposed
   * to being completely open to anything
   */
  public GunType gunType;
  /**
   * The name of the gun model this seat is connected to. Gun model names are specified in the model
   * files
   */
  ...
  public int gunnerID;
  /**
   * For turret mounted seats on tanks, the seat will be positioned differently according to this
   * offset and the yaw of the turret
   */
  public Vector3f rotatedOffset = new Vector3f();
  /**
   * Yaw/Pitch rotation speeds (Yaw/Pitch/z) where Z is ignored
   */
  public Vector3f aimingSpeed = new Vector3f(DriveableType.AIM_SPEED_MULTIPLIER,DriveableType.AIM_SPEED_MULTIPLIER,DriveableType.AIM_SPEED_MULTIPLIER);
  /**
  ...
  public boolean enableDriveBy = false; //if the player is able to shoot while seated
  ...
  /**
   * Type file constructor. Line from type file should be of one of the following forms Passenger ID
   * x y z Passenger ID x y z minYaw maxYaw minPitch maxPitch Passenger ID x y z minYaw maxYaw
   * minPitch maxPitch gunType.shortName gunName
   */
