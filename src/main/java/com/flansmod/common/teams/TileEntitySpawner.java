...
public class TileEntitySpawner extends TileEntity implements ITeamObject, ITickable {
  ...
  public void update() {
    ...
    if (worldObj.getBlockState(pos).getValue(TYPE).intValue() == 1) {
      return;
    }
    ...
    if (currentDelay == 0) {
      ...
      for (int i = 0; i < stacksToSpawn.size(); i++) {
        if (worldObj.getBlockState(pos).getValue(TYPE).intValue() == 2) {
          ...
          if (stack != null && stack.getItem() instanceof ItemPlane) {
            spawnedEntity = ((ItemPlane) stack.getItem())
                .spawnPlane(worldObj, pos.getX() + 0.5F, pos.getY() + 0.5F, pos.getZ() + 0.5F,
                    stack);
          }
          if (stack != null && stack.getItem() instanceof ItemVehicle) {
            spawnedEntity = ((ItemVehicle) stack.getItem())
                .spawnVehicle(worldObj, pos.getX() + 0.5F, pos.getY() + 0.5F, pos.getZ() + 0.5F,
                    stack);
          }
          if (stack != null && stack.getItem() instanceof ItemAAGun) {
            spawnedEntity = ((ItemAAGun) stack.getItem())
                .spawnAAGun(worldObj, pos.getX() + 0.5F, pos.getY(), pos.getZ() + 0.5F, stack);
          }
        } else {
  ...
  public int getTeamID() {
    if (worldObj.isRemote) {
      ...
      return base == null ? 0 : base.getOwnerID();
    }
  }

  @Override
  public void onBaseSet(int newTeamID) {
    FlansMod.packetHandler.sendToDimension(getDescriptionPacket(),
        worldObj == null ? dimension : worldObj.provider.getDimensionId());
  }
  ...
  public boolean isSpawnPoint() {
    if (worldObj.getBlockState(pos).getProperties().containsKey(TYPE)) {
      int metadata = worldObj.getBlockState(pos).getValue(TYPE).intValue();
      return metadata == 1;
    }
