...
public class EntityAIMecha extends EntityMecha {
  ...
  public EntityAIMecha(World world, double x, double y, double z, MechaType type,
  ...
  protected void moveAI(Vector3f actualMotion) {
    ...
    if (target == null && (this.ticksExisted + this.getEntityId()) % targetAcquireInterval == 0) {
      ...
      for (Object obj : worldObj.getEntitiesWithinAABBExcludingEntity(this,
          getEntityBoundingBox().expand(targetingRange, targetingRange, targetingRange))) {
    ...
    if (!worldObj.isRemote && target != null) {
      Vec3 rightArmOrigin =
          usingLeft ? axes.findLocalVectorGlobally(getMechaType().leftArmOrigin).toVec3()
              .addVector(posX, posY, posZ)
              : axes.findLocalVectorGlobally(getMechaType().rightArmOrigin).toVec3()
                  .addVector(posX, posY, posZ);
      Vec3 targetOrigin = new Vec3(target.posX, target.posY + target.getEyeHeight() / 2D,
          target.posZ);
      ...
      if (seats != null && seats[0] != null) {
        seats[0].looking
            .setAngles(0F, -(float) Math.atan2(dY, Math.sqrt(dX * dX + dZ * dZ)) * 180F / 3.14159F,
                0F);
        seats[0].prevLooking
            .setAngles(0F, -(float) Math.atan2(dY, Math.sqrt(dX * dX + dZ * dZ)) * 180F / 3.14159F,
                0F);
      ...
      }
      {
        ...
        if (hit == null || hit.typeOfHit != MovingObjectPosition.MovingObjectType.BLOCK
            || dX * dX + dY * dY + dZ * dZ
            < blockHitX * blockHitX + blockHitY * blockHitY + blockHitZ * blockHitZ) {
          useItem(usingLeft);
          if (rand.nextInt(5) == 0) {
            usingLeft = !usingLeft;
          }
        ...
        //Otherwise, move closer
          ...
          if (Math.abs(intent.lengthSquared()) > 0.1) {
            ...
            intent.scale(
                (type.moveSpeed * data.engine.engineSpeed * speedMultiplier()) * (4.3F / 20F));
  ...
  private boolean isBetterTarget(Entity currentTarget, double distToCurrentTarget,
      Entity potentialTarget, double distToPotentialTarget) {
    return potentialTarget instanceof EntityPlayer && distToPotentialTarget < distToCurrentTarget
        && distToPotentialTarget < targetingRange * targetingRange;
  }
