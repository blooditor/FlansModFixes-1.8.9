...
public class InfoType {
  ...
  /**
   * The probability that this item will appear in a dungeon chest. Scaled so that each chest is
   * likely to have a fixed number of Flan's Mod items. Must be greater than or equal to 0, and
   * should probably not exceed 100
   */
  public int dungeonChance = 1;
  public boolean craftable; //TODO disable for all default parts
  ...
  public void read(String[] split, TypeFile file) {
    try {
      ...
      craftable = Read(split, "craftable", craftable);
      ...
      if (split[0].equals("Colour") || split[0].equals("Color")) {
        colour =
            (Integer.parseInt(split[1]) << 16) + ((Integer.parseInt(split[2])) << 8) + ((Integer
                .parseInt(split[3])));
      }
  ...
  protected boolean KeyMatches(String[] split, String key) {
    return split != null && split.length > 1 && key != null && split[0].toLowerCase()
    ...
    if (KeyMatches(split, key)) {
      if (split.length == 2) {
        try {
          ...
          InfoType.LogError(shortName,
              "Incorrect format for " + key + ". Passed in value is not an integer");
        }
      } else {
        InfoType.LogError(shortName,
            "Incorrect format for " + key + ". Should be \"" + key + " <integer value>\"");
      }
    }
    ...
    if (KeyMatches(split, key)) {
      if (split.length == 2) {
        try {
          ...
          InfoType.LogError(shortName,
              "Incorrect format for " + key + ". Passed in value is not an float");
        }
      } else {
        InfoType.LogError(shortName,
            "Incorrect format for " + key + ". Should be \"" + key + " <float value>\"");
      }
    }
  ...
  protected String Read(String[] split, String key, String currentValue) {
    if (KeyMatches(split, key)) {
      if (split.length == 2) {
        ...
        InfoType.LogError(shortName,
            "Incorrect format for " + key + ". Should be \"" + key + " <singleWord>\"");
      }
    }
  ...
  protected String ReadAndConcatenateMultipleStrings(String[] split, String key,
      String currentValue) {
    if (KeyMatches(split, key)) {
      if (split.length > 1) {
        currentValue = split[1];
        ...
        InfoType.LogError(shortName,
            "Incorrect format for " + key + ". Should be \"" + key + " <long string>\"");
      }
    }
    ...
    if (KeyMatches(split, key)) {
      if (split.length == 2) {
        try {
          ...
          InfoType.LogError(shortName,
              "Incorrect format for " + key + ". Passed in value is not an boolean");
        }
      } else {
        InfoType.LogError(shortName,
            "Incorrect format for " + key + ". Should be \"" + key + " <true/false>\"");
      }
    }
  ...
  public void addRecipe(Item par1Item) {
    ...
    try {
      if (!shapeless) {
        ...
        int rows = 3;
        // First column
        if (((String) recipe[0]).charAt(0) == ' ' && ((String) recipe[1]).charAt(0) == ' '
            && ((String) recipe[2]).charAt(0) == ' ') {
          for (int i = 0; i < 3; i++) {
        ...
        if (((String) recipe[0]).charAt(last) == ' ' && ((String) recipe[1]).charAt(last) == ' '
            && ((String) recipe[2]).charAt(last) == ' ') {
          for (int i = 0; i < 3; i++) {
        ...
        // Bottom row
        if (recipe[rows - 1].equals(" ") || recipe[rows - 1].equals("  ") || recipe[rows - 1]
            .equals("   ")) {
          Object[] newRecipe = new Object[recipe.length - 1];
  ...
  public static ItemStack getRecipeElement(String s, int amount, int damage, String requester) {
    ...
    for (Object object : Item.itemRegistry) {
      ...
      if (item != null && item.getUnlocalizedName() != null && (
          item.getUnlocalizedName().equals("item." + s) || item.getUnlocalizedName()
              .equals("tile." + s))) {
  ...
  public void addDungeonLoot() {
    if (dungeonChance > 0) {
      ...
      addToRandomChest(stack,
          (float) (FlansMod.dungeonLootChance * dungeonChance) / (float) totalDungeonChance);
    }
  }
      ...
      switch (random.nextInt(10)) {
        case 0:
          ChestGenHooks.addItem(ChestGenHooks.DUNGEON_CHEST,
              new WeightedRandomChestContent(new ItemStack(this.item), 1, 1, chance));
          break;
        case 1:
