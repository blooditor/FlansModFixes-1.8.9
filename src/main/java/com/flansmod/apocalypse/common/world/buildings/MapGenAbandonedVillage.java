...
public class MapGenAbandonedVillage extends MapGenStructure {
  ...
  public static List<BiomeGenBase> villageSpawnBiomes = Arrays
      .asList(BiomeGenApocalypse.canyon, BiomeGenApocalypse.desert);
  ...
  protected boolean canSpawnStructureAtCoords(int x, int z) {
    ...
    return xAdjusted == xScaled && zAdjusted == zScaled && worldObj.getWorldChunkManager()
        .areBiomesViable(xAdjusted * 16 + 8, zAdjusted * 16 + 8, 0, villageSpawnBiomes);
  }
  ...
  public static class Start extends StructureStart {
    ...
    public Start(World world, Random rand, int x, int z, int i) {
      ...
      StructureAbandonedVillagePieces.Start start = new StructureAbandonedVillagePieces.Start(
          world.getWorldChunkManager(), 0, rand, (x << 4) + 2, (z << 4) + 2, list, i);
    ...
    public void generateStructure(World world, Random rand, StructureBoundingBox boundingBox) {
      ...
      while (iterator.hasNext()) {
        ...
        if (structurecomponent.getBoundingBox().intersectsWith(boundingBox) && !structurecomponent
