...
public class GuiPaintjobTable extends GuiContainer {
  ...
  private static int[] baseColours = new int[]{0x000000, 0xffffff, 0xff0000, 0xff5500, 0xffaa00,
      0xffff00, 0xaaff00, 0x55ff00, 0x00ff00, 0x00ff55, 0x00ffaa, 0x00ffff, 0x00aaff, 0x0055ff,
      0x0000ff, 0x5500ff, 0xaa00ff, 0xff00ff};
  ...
  public void updateScreen() {
    ...
    if (movingFlatTextureWindow) {
      ...
      flatTextureWindowY =
          this.height - Mouse.getEventY() * this.height / this.mc.displayHeight - 1 - guiTop;
    }
  ...
  protected void drawGuiContainerForegroundLayer(int x, int y) {
    ...
    if (customModeTransitionTimer <= 0.999f) {
      ...
      fontRendererObj
          .drawString("Inventory", GetMainPageX() + 8, GetMainPageY() + (ySize - 94) + 2, 0x404040);
      fontRendererObj
          .drawString("Paintjob Table", GetMainPageX() + 8, GetMainPageY() + 6, 0x404040);
    }
  ...
  protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
    ...
    if (customModeTransitionTimer <= 0.999f) {
      ...
      // Inventory box
      drawModalRectWithCustomSizedTexture(xOrigin, yOrigin + 122, 0, 114, xSize, 142, textureX,
          textureY);
      ...
      if (hoveringOver != null) {
        ...
        if (numDyes != 0 && !inventory.player.capabilities.isCreativeMode) {
          ...
          for (int n = 0; n < numDyes; n++) {
            ...
            for (int s = 0; s < inventory.getSizeInventory(); s++) {
              ...
              if (stack != null && stack.getItem() == Items.dye
                  && stack.getItemDamage() == hoveringOver.dyesNeeded[n].getItemDamage()) {
          ...
          if (numDyes == 1) {
            drawModalRectWithCustomSizedTexture(originX, originY, (haveDyes[0] ? 379 : 356), 0, 22,
                22, textureX, textureY);
          } else {
            //First slot
            drawModalRectWithCustomSizedTexture(originX, originY, 256, (haveDyes[0] ? 23 : 0), 20,
                22, textureX, textureY);
            //Middle slots
            for (int s = 1; s < numDyes - 1; s++) {
              drawModalRectWithCustomSizedTexture(originX + 2 + 18 * s, originY, 277,
                  (haveDyes[s] ? 23 : 0), 18, 22, textureX, textureY);
            }
            //Last slot
            drawModalRectWithCustomSizedTexture(originX + 2 + 18 * (numDyes - 1), originY, 296,
                (haveDyes[numDyes - 1] ? 23 : 0), 20, 22, textureX, textureY);
          }

          for (int s = 0; s < numDyes; s++) {
            itemRender
                .renderItemIntoGUI(hoveringOver.dyesNeeded[s], originX + 3 + s * 18, originY + 3);
            itemRender.renderItemOverlayIntoGUI(this.fontRendererObj, hoveringOver.dyesNeeded[s],
                originX + 3 + s * 18, originY + 3, null);
          }
        }
      }
    ...
    if (customModeTransitionTimer >= 0.001f) {
      ...
      // Palette
      drawModalRectWithCustomSizedTexture(xOrigin, yOrigin + 200, 224, 206, 288, 50, textureX,
          textureY);
      ...
      for (int x = 0; x < paletteSizeX; x++) {
        for (int y = 0; y < paletteSizeY; y++) {
          ...
          GlStateManager.color(scale * ((colour >> 16) & 0xff), scale * ((colour >> 8) & 0xff),
              scale * ((colour >> 0) & 0xff));

          drawModalRectWithCustomSizedTexture(xOrigin + 8 + 9 * x, yOrigin + 200 + 8 + 9 * y, 0, 0,
              7, 7, textureX, textureY);
        }
      }
      ...
      drawModalRectWithCustomSizedTexture(xOrigin + 172, yOrigin + 208, 0, 0, 34, 34, textureX,
          textureY);

      // Slider bars
      for (int n = 0; n < componentBarLength; n++) {
        GlStateManager.color((float) n / componentBarLength, green, blue);
        drawModalRectWithCustomSizedTexture(xOrigin + 212 + n, yOrigin + 208, 0, 0, 1, 10, textureX,
            textureY);
      }
      for (int n = 0; n < componentBarLength; n++) {
        GlStateManager.color(red, (float) n / componentBarLength, blue);
        drawModalRectWithCustomSizedTexture(xOrigin + 212 + n, yOrigin + 220, 0, 0, 1, 10, textureX,
            textureY);
      }
      for (int n = 0; n < componentBarLength; n++) {
        GlStateManager.color(red, green, (float) n / componentBarLength);
        drawModalRectWithCustomSizedTexture(xOrigin + 212 + n, yOrigin + 232, 0, 0, 1, 10, textureX,
            textureY);
      }
      ...
      // Sliders
      drawModalRectWithCustomSizedTexture(xOrigin + 212 + (int) (red * componentBarLength),
          yOrigin + 207, 317, 21, 3, 12, textureX, textureY);
      drawModalRectWithCustomSizedTexture(xOrigin + 212 + (int) (green * componentBarLength),
          yOrigin + 219, 317, 21, 3, 12, textureX, textureY);
      drawModalRectWithCustomSizedTexture(xOrigin + 212 + (int) (blue * componentBarLength),
          yOrigin + 231, 317, 21, 3, 12, textureX, textureY);

      for (int n = 0; n < 3; n++) {
        drawModalRectWithCustomSizedTexture(xOrigin + 290, yOrigin + 200 + 17 * n, 401, 0, 78, 16,
            textureX, textureY);
      }
      ...
      {
        drawModalRectWithCustomSizedTexture(xFlatOrigin, yFlatOrigin, 242, 54, 64 + 7, 152,
            textureX, textureY);
        drawModalRectWithCustomSizedTexture(xFlatOrigin + 64 + 7, yFlatOrigin, 242 + 270 - 64 - 7,
            54, 64 + 7, 152, textureX, textureY);

        bindWorkingTexture();

        drawModalRectWithCustomSizedTexture(xFlatOrigin + 7, yFlatOrigin + 17, 0, 0, 128, 128,
            dynamicTextureX, dynamicTextureY);
      }
  ...
  public void bindWorkingTexture() {
    mc.getTextureManager().bindTexture(
        mc.getTextureManager().getDynamicTextureLocation("customPaintjob", dynamicTexture));
  }
    ...
    if (gunStack != null && gunStack.getItem() instanceof IPaintableItem) {
      ...
      try {
        String imageLocation =
            "Flan/" + paintableType.contentPack + "/assets/flansmod/skins/" + paintjob.textureName
                + ".png";
  ...
  public void handleInput() throws IOException {
    ...
    if (inCustomMode) {
        ...
        if (pixelX >= 0 && pixelX < 128 && pixelY >= 0 && pixelY < 128) {
          for (int i = -2; i < 2; i++) {
            for (int j = -2; j < 2; j++) {
              ...
              dynamicTexture.getTextureData()[px + py * dynamicTextureX] =
                  0xff000000 + currentColour;
              copyImageToTexture();
            }
          }
        }
      ...
      if (mouseXInGUI >= flatTexOriginX - 64 + 7 && mouseXInGUI <= flatTexOriginX + 64 + 14
          && mouseYInGUI >= flatTexOriginY - 4 && mouseYInGUI <= flatTexOriginY + 6) {
  ...
  public void handleMouseInput() throws IOException {
    ...
    if (inCustomMode) {
      ...
      for (int x = 0; x < paletteSizeX; x++) {
        for (int y = 0; y < paletteSizeY; y++) {
          if (mouseXInGUI >= xOrigin + 8 + 9 * x && mouseXInGUI < xOrigin + 15 + 9 * x
              && mouseYInGUI >= yOrigin + 208 + 9 * y && mouseYInGUI < yOrigin + 215 + 9 * y) {
      ...
      if (Mouse.getEventButton() == 0 && Mouse.getEventButtonState()) {
        if (mouseXInGUI >= xOrigin + 212 && mouseXInGUI < xOrigin + 212 + componentBarLength
            && mouseYInGUI >= yOrigin + 208 && mouseYInGUI < yOrigin + 218) {
        ...
        if (mouseXInGUI >= xOrigin + 212 && mouseXInGUI < xOrigin + 212 + componentBarLength
            && mouseYInGUI >= yOrigin + 220 && mouseYInGUI < yOrigin + 230) {
        ...
        if (mouseXInGUI >= xOrigin + 212 && mouseXInGUI < xOrigin + 212 + componentBarLength
            && mouseYInGUI >= yOrigin + 232 && mouseYInGUI < yOrigin + 242) {
