...
public class EntityAAGun extends Entity implements IEntityAdditionalSpawnData {
  ...
  public void setPosition(double d, double d1, double d2) {
    ...
    setEntityBoundingBox(AxisAlignedBB
        .fromBounds(d - f, (d1 - yOffset), d2 - f, d + f, (d1 - yOffset) + f1, d2 + f));
  }
  ...
  public boolean attackEntityFrom(DamageSource damagesource, float i) {
    if (damagesource.damageType.equals("player")) {
      ...
      } else if (TeamsManager.canBreakGuns && !worldObj.isRemote) {
        setDead();
      }
    } else {
      setBeenAttacked();
  ...
  public void onUpdate() {
    ...
    if (worldObj.isRemote && riddenByEntity != null && riddenByEntity == FMLClientHandler.instance()
        .getClient().thePlayer) {
      checkForShooting();
    }
      ...
      for (int i = 0; i < type.numBarrels; i++) {
        ...
        if (ammo[i] == null && riddenByEntity != null && riddenByEntity instanceof EntityPlayer) {
          ...
          if (slot >= 0) {
            ...
            PacketPlaySound
                .sendSoundPacket(posX, posY, posZ, 50, dimension, type.reloadSound, true);
          }
        }
      }
    }
    ...
    if (!worldObj.isRemote && reloadTimer <= 0 && shootDelay <= 0) {
      if (mouseHeld && riddenByEntity != null && riddenByEntity instanceof EntityPlayer) {
        ...
        for (int j = 0; j < type.numBarrels; j++) {
          if (shootDelay <= 0 && ammo[j] != null && (!type.fireAlternately
              || type.fireAlternately && currentBarrel == j)) {
            // Fire
            ...
            Vec3 origin = rotate(
                type.barrelX[currentBarrel] / 16D - type.barrelZ[currentBarrel] / 16D,
        ...
        for (int j = 0; j < type.numBarrels; j++) {
          ...
          if (shootDelay <= 0 && ammo[ammoSlot] != null && (!type.fireAlternately
              || type.fireAlternately && currentBarrel == ammoSlot)) {
            // Fire
    ...
    if (!worldObj.isRemote) {
      FlansMod.getPacketHandler()
          .sendToAllAround(new PacketAAGunAngles(this), posX, posY, posZ, 50F, dimension);
    }
  }
  ...
  public Entity getValidTarget() {
    ...
    for (Object obj : worldObj.getEntitiesWithinAABBExcludingEntity(this,
        getEntityBoundingBox().expand(type.targetRange, type.targetRange, type.targetRange))) {
      Entity candidateEntity = (Entity) obj;

      if ((type.targetMobs && candidateEntity instanceof EntityMob) || (type.targetPlayers
        ...
        if (candidateEntity.getDistanceToEntity(this) < type.targetRange) {
          if (candidateEntity instanceof EntityPlayer) {
            ...
            if (TeamsManager.enabled && TeamsManager.getInstance().currentRound != null
                && placer != null) {
              PlayerData placerData = PlayerHandler
                  .getPlayerData(placer, worldObj.isRemote ? Side.CLIENT : Side.SERVER);
              PlayerData candidateData = PlayerHandler.getPlayerData((EntityPlayer) candidateEntity,
                  worldObj.isRemote ? Side.CLIENT : Side.SERVER);
              if (candidateData.team == Team.spectators || candidateData.team == null) {
                continue;
              }
              if (!TeamsManager.getInstance().currentRound.gametype
                  .playerCanAttack((EntityPlayerMP) placer, placerData.team,
  ...
  protected void writeEntityToNBT(NBTTagCompound nbttagcompound) {
    ...
    nbttagcompound.setString("Placer", placer == null ? placerName : placer.getName());
  }
      ...
      for (int i = 0; i < (type.shareAmmo ? 1 : type.numBarrels); i++) {
        if (ammo[i] == null) {
          ...
          if (slot >= 0) {
            ...
            reloadTimer = type.reloadTime;
            worldObj.playSoundAtEntity(this, type.reloadSound, 1.0F,
