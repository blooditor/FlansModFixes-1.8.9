...
public class GametypeZombies extends Gametype {
  ...
  public void tick() {
    ...
    if (teamsManager.roundTimeLeft + humanPrepTime - 200
        == teamsManager.currentRound.timeLimit * 20 * 60) {
      TeamsManager.messageAll(
          "\u00a7cThe \u00a74zombies \u00a7care upon us! 10 seconds to \u00a74infection\u00a7c!");
    }

    //Human prep time over
    if (teamsManager.roundTimeLeft + humanPrepTime
        == teamsManager.currentRound.timeLimit * 20 * 60) {
    ...
    if (teamsManager.roundTimeLeft + humanPrepTime < teamsManager.currentRound.timeLimit * 20 * 60
        && teamsManager.roundTimeLeft % 200 == 0
        && teamsManager.currentRound.teams[1].members.size() == 0) {
      zombifySomeone();
    }
  }
  ...
  public void zombifySomeone() {
    if (teamsManager.currentRound.teams[0].members.size() > 0) {
      EntityPlayer zombifiedPlayer = TeamsManager.getPlayer(
          teamsManager.currentRound.teams[0].members
              .get(rand.nextInt(teamsManager.currentRound.teams[0].members.size())));
      TeamsManager.messageAll("\u00a74" + zombifiedPlayer.getName()
          + "\u00a7c was infected with the \u00a74zombie plague\u00a7c!");
  ...
  @Override
  public boolean playerCanAttack(EntityPlayerMP attacker, Team attackerTeam, EntityPlayerMP victim,
      Team victimTeam) {
  ...
  public boolean teamHasWon(Team team) {
    ...
    //Zombies win if all the humans are dead
      ...
      return teamsManager.roundTimeLeft + humanPrepTime
          <= teamsManager.currentRound.timeLimit * 20 * 60
          && teamsManager.currentRound.teams[0].members.size() == 0;
    }
    return false;
  }
  ...
  //Zombies can't loot
  public boolean playerCanLoot(ItemStack stack, InfoType infoType, EntityPlayer player,
      Team playerTeam) {
