...
public class RenderMecha extends Render implements IItemRenderer {
  ...
  public void render(EntityMecha mecha, double d, double d1, double d2, float f, float f1) {
    ...
    if (mecha.isPartIntact(EnumDriveablePart.leftArm)) {
      ...
      if (mecha.seats[0] != null) {
        smoothedPitch = mecha.seats[0].prevLooking.getPitch()
      ...
      GL11.glTranslatef(0F + type.leftHandModifierY, -type.armLength - type.leftHandModifierX,
          0F + type.leftHandModifierZ);
    ...
    if (mecha.isPartIntact(EnumDriveablePart.rightArm)) {
      ...
      //Move to the end of the arm and render the held item
      GL11.glTranslatef(0F + type.rightHandModifierY, -type.armLength - type.rightHandModifierX,
          0F + type.rightHandModifierZ);
      GL11.glScalef(modelScale, modelScale, modelScale);
    ...
    if (FlansMod.DEBUG) {
      ...
      for (DriveablePart part : mecha.getDriveableData().parts.values()) {
        ...
        renderOffsetAABB(new AxisAlignedBB(part.box.x / 16F, part.box.y / 16F, part.box.z / 16F,
            (part.box.x + part.box.w) / 16F, (part.box.y + part.box.h) / 16F,
            (part.box.z + part.box.d) / 16F), 0, 0, 0);
      }
    ...
    if (mecha.isPartIntact(EnumDriveablePart.hips)) {
      ...
      if (model != null) {
        ...
        float legsYaw =
            (float) Math.sin(((mecha.ticksExisted) + f1) / type.legSwingTime) * mecha.legSwing;
  ...
  private void renderItem(EntityMecha mecha, ItemStack stack, int par3, boolean leftHand,
      float dT) {
    GL11.glPushMatrix();
    ...
    if (item instanceof ItemMechaAddon) {
      ...
      if (toolType.model != null) {
        ...
        GL11.glPopMatrix();
        toolType.model.renderSaw(mecha, dT,
      ...
      ClientProxy.gunRenderer.renderGun(stack, gunType, 1F / 16F, model,
          leftHand ? mecha.leftAnimations : mecha.rightAnimations, 0F);
    } else {
    ...
    switch (type) {
      ...
      case ENTITY:
        return Minecraft.getMinecraft().gameSettings.fancyGraphics && item != null && item
            .getItem() instanceof ItemMecha && ((ItemMecha) item.getItem()).type.model != null;
      default:
        break;
    }
    return false;
    ...
    if (item != null && item.getItem() instanceof ItemMecha) {
      ...
      if (mechaType.model != null) {
        ...
        Minecraft.getMinecraft().renderEngine
            .bindTexture(FlansModResourceHandler.getTexture(mechaType));
