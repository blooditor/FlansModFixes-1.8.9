...
public abstract class PaintableType extends InfoType {
  ...
  /**
   * The default paintjob for this gun. This is created automatically in the load process from
   * existing info
   */
    ...
    try {
      ...
      if (KeyMatches(split, "Paintjob")) {
        ...
        for (int i = 0; i < (split.length - 3) / 2; i++) {
          dyeStacks[i] = new ItemStack(Items.dye, Integer.parseInt(split[i * 2 + 4]),
              getDyeDamageValue(split[i * 2 + 3]));
        }
  ...
  public Paintjob getPaintjob(int i) {
    if (this instanceof GunType && ((GunType) this).shield) {
      return paintjobs.get(0);//TODO
    }
    return i < paintjobs.size() ? paintjobs.get(i) : paintjobs.get(0);
  }
  ...
  public static ResourceLocation GetCustomPaintjobSkinResource(ItemStack stack) {
    ...
    if (!FlansModResourceHandler.HasResourceForHash(customPaintHash)) {
      FlansModResourceHandler
          .CreateSkinResourceFromByteArray(tags.getByteArray("Skin"), tags.getInteger("SkinWidth"),
              tags.getInteger("SkinHeight"), customPaintHash);
      FlansModResourceHandler
          .CreateIconResourceFromByteArray(tags.getByteArray("Icon"), tags.getInteger("IconWidth"),
              tags.getInteger("IconHeight"), customPaintHash);
    }
