...
public class PlayerClass extends InfoType implements IPlayerClass {
  ...
  /**
   * This loads the items once for clients connecting to remote servers, since the clients can't
   * tell what attachments a gun has in the GUI and they need to load it at least once
   */
  @Override
  ...
  /**
   * In the loading phase item IDs are all up in the air and so too are NBT tags regarding
   * ItemStacks So to avoid guns with attachments having their attachments replaced with incorrect
   * ones, random guns and other silly things, we read the relevant lines here, as the world loads
   */
  @Override
      ...
      for (String[] split : startingItemStrings) {
        ...
        for (Object object : Item.itemRegistry) {
          ...
          if (item != null && item.getUnlocalizedName() != null && (
              item.getUnlocalizedName().equals(itemNames[0]) || (
                  item.getUnlocalizedName().split("\\.").length > 1 && item.getUnlocalizedName()
                      .split("\\.")[1].equals(itemNames[0])))) {
            matchingItem = item;
          }
        }
        ...
        if (matchingItem == null) {
          FlansMod.log("Tried to add " + split[1] + " to player class " + shortName
              + " but the item did not exist");
          return;
        }
        if (split.length > 2) {
