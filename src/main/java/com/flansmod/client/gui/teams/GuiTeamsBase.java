...
public class GuiTeamsBase extends GuiScreen {
  ...
  private static final ResourceLocation loudoutBoxes = new ResourceLocation("flansmod",
      "gui/LandingPage.png");
  private static final ResourceLocation ranks = new ResourceLocation("flansmod", "gui/Ranks.png");
  private static final ResourceLocation loadoutEditor = new ResourceLocation("flansmod",
  ...
  protected void DrawLoadoutPanel(LoadoutPool pool, PlayerRankData data, int i, int j, int n) {
    ...
    if (data.currentLevel >= pool.slotUnlockLevels[n]) {
      ...
      drawCenteredString(fontRendererObj, "at " + pool.slotUnlockLevels[n], i + 23, j + 40,
          0xffffff);
    }
  ...
  protected void DrawRankIcon(int rank, int prestige, int x, int y, boolean doubleSize) {
    ...
    if (doubleSize) {
      drawModalRectWithCustomSizedTexture(guiOriginX + x, guiOriginY + y, rank * 32, prestige * 32,
          32, 32, 1024, 512);
    } else {
      drawModalRectWithCustomSizedTexture(guiOriginX + x, guiOriginY + y, rank * 16, prestige * 16,
          16, 16, 512, 256);
    }
  }
