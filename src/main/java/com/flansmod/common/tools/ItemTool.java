...
public class ItemTool extends ItemFood implements IFlanItem {
  ...
  public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer) {
    if (type.foodness > 0) {
      ...
      if (type.toolLife > 0 && type.destroyOnEmpty && itemstack.getItemDamage() == itemstack
          .getMaxDamage()) {
        itemstack.stackSize--;
      }
      ...
      PlayerData data = PlayerHandler
          .getPlayerData(entityplayer, world.isRemote ? Side.CLIENT : Side.SERVER);
      ...
      Vec3 posVec = new Vec3(entityplayer.posX,
          entityplayer.posY + 1.62D - entityplayer.getYOffset(), entityplayer.posZ);
      Vec3 lookVec = posVec
      ...
      if (type.healDriveables) {
        ...
        for (int i = 0; i < world.loadedEntityList.size(); i++) {
          ...
          if (obj instanceof EntityDriveable) {
            ...
            //Raytrace
            DriveablePart part = driveable.raytraceParts(new Vector3f(posVec),
            ...
            if (part != null && part.maxHealth > 0) {
              ...
              if (part.health < part.maxHealth && (type.toolLife == 0
                ...
                //If the tool is damagable and is destroyed upon being used up, then destroy it
                if (type.toolLife > 0 && type.destroyOnEmpty
                    && itemstack.getItemDamage() == itemstack.getMaxDamage()) {
                  itemstack.stackSize--;
                }
                //Our work here is done. Let's be off
      ...
      if (!world.isRemote && type.healPlayers) {
        ...
        List list = world.getEntitiesWithinAABB(EntityLivingBase.class, new AxisAlignedBB(
            Math.min(posVec.xCoord, lookVec.xCoord), Math.min(posVec.yCoord, lookVec.yCoord),
            Math.min(posVec.zCoord, lookVec.zCoord),
        ...
        for (Object aList : list) {
          ...
          MovingObjectPosition hit = checkEntity.getEntityBoundingBox()
              .calculateIntercept(posVec, lookVec);
          //If it hit, heal it
          if (hit != null) {
        ...
        if (hitLiving != null) {
          ...
          FlansMod.getPacketHandler().sendToAllAround(
              new PacketFlak(hitLiving.posX, hitLiving.posY, hitLiving.posZ, 5, "heart"),
              new NetworkRegistry.TargetPoint(hitLiving.dimension, hitLiving.posX, hitLiving.posY,
                  hitLiving.posZ, 50F));
